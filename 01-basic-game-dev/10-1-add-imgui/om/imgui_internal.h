...
typedef int ImGuiLayoutType; // enum: horizontal or vertical             // enum
                             // ImGuiLayoutType_
typedef int ImGuiButtonFlags; // flags: for ButtonEx(), ButtonBehavior()  //
                              // enum ImGuiButtonFlags_
typedef int ImGuiItemFlags; // flags: for PushItemFlag()                // enum
                            // ImGuiItemFlags_
typedef int ImGuiSeparatorFlags; // flags: for Separator() - internal        //
                                 // enum ImGuiSeparatorFlags_
typedef int ImGuiSliderFlags;    // flags: for SliderBehavior()              //
...
IMGUI_API int ImTextCountUtf8BytesFromStr(
    const ImWchar* in_text,
    const ImWchar* in_text_end); // return number of bytes to express string as
                                 // UTF-8 code-points
...
IMGUI_API char* ImStrchrRange(const char* str_begin,
                              const char* str_end,
                              char        c);
...
// Helpers: Math
// We are keeping those not leaking to the user by default, in the case the user
// has implicit cast operators between ImVec2 and its own types (when
// IM_VEC2_CLASS_EXTRA is defined)
...
static inline void ImSwap(int& a, int& b)
{
    int tmp = a;
    a       = b;
    b       = tmp;
}
static inline void ImSwap(float& a, float& b)
{
    float tmp = a;
    a         = b;
    b         = tmp;
}
static inline int ImLerp(int a, int b, float t)
{
    return (int)(a + (b - a) * t);
}
...
// We call C++ constructor on own allocated memory via the placement "new(ptr0)
// Type()" syntax. Defining a custom placement new() with a dummy parameter
...
inline void* operator new(size_t, ImNewPlacementDummy, void* ptr)
{
    return ptr;
}
inline void operator delete(void*, ImNewPlacementDummy, void*) {
} // This is only required so we can use the symetrical new()
#define IM_PLACEMENT_NEW(_PTR) new (ImNewPlacementDummy(), _PTR)
#define IM_NEW(_TYPE)                                                          \
    new (ImNewPlacementDummy(), ImGui::MemAlloc(sizeof(_TYPE))) _TYPE
...
// for Dear ImGui.
#define IMGUI_PAYLOAD_TYPE_DOCKABLE                                            \
    "_IMDOCK" // ImGuiWindow* // [Internal] Docking/tabs
...
struct IMGUI_API ImGuiTextEditState
{
    ...
    ImVector<ImWchar> Text; // edit buffer, we need to persist but can't
    ...
    ImGuiTextEditState() { memset(static_cast<void*>(this), 0, sizeof(*this)); }
...
struct ImGuiNextWindowData
{
    ...
    ImGuiNextWindowData()
    {
        PosCond = SizeCond = ContentSizeCond = CollapsedCond =
            SizeConstraintCond = FocusCond = 0;
...
struct ImGuiContext
{
    ...
    ImGuiWindow*
        MovingWindow; // Track the child window we clicked on to move a window.
    ...
    ImVector<ImGuiPopupRef> CurrentPopupStack; // Which level of BeginPopup() we
    ...
    // Render
    ImDrawData RenderDrawData; // Main ImDrawData instance to pass render
    ...
    float  DragCurrentValue; // Currently dragged value, always float, not
    ...
    ImVec2 OsImePosRequest, OsImePosSet; // Cursor position request & last
                                         // passed to the OS Input Method Editor

    // Settings
    float
        SettingsDirtyTimer; // Save .ini Settings on disk when time reaches zero
    ImVector<ImGuiWindowSettings>
        SettingsWindows; // .ini settings for ImGuiWindow
    ...
    ImGuiTextBuffer*
        LogClipboard; // Else log to clipboard. This is pointer so our GImGui
    ...
    int WantCaptureMouseNextFrame; // explicit capture via CaptureInputs() sets
                                   // those flags
...
// window, inherited from parent on first Begin().
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_Disabled =
        1 << 2, // false    // FIXME-WIP: Disable interactions but doesn't
                // affect visuals. Should be: grey out and disable interactions
                // with widgets that affect data + view widgets (WIP)
...
// ImGuiDrawContext is quite tenuous and could be reconsidered.
struct IMGUI_API ImGuiDrawContext
{
    ...
    float IndentX; // Indentation / start position from left of window
                   // (increased by TreePush/TreePop, etc.)
    float GroupOffsetX;
    float ColumnsOffsetX; // Offset to the current column (if ColumnsCurrent >
...
struct IMGUI_API ImGuiWindow
{
    ...
    int       HiddenFrames;
    ImGuiCond SetWindowPosAllowFlags;       // store condition flags for next
                                            // SetWindowPos() call.
    ImGuiCond SetWindowSizeAllowFlags;      // store condition flags for next
                                            // SetWindowSize() call.
    ImGuiCond SetWindowCollapsedAllowFlags; // store condition flags for next
                                            // SetWindowCollapsed() call.
    ...
    ImRect ClipRect; // = DrawList->clip_rect_stack.back(). Scissoring /
                     // clipping rectangle. x1, y1, x2, y2.
    ImRect WindowRectClipped; // = WindowRect just after setup in Begin(). ==
    ...
    ImGuiWindow*
        RootWindow; // Generally point to ourself. If we are a child window,
                    // this is pointing to the first non-child parent window.
    ImGuiWindow*
        RootNonPopupWindow; // Generally point to ourself. Used to display
                            // TitleBgActive color and for selecting which
                            // window to use for NavWindowing
...
struct ImGuiItemHoveredDataBackup
{
    ...
    void Backup()
    {
        ...
        LastItemRectHoveredRect = window->DC.LastItemRectHoveredRect;
    }
    void Restore() const
    {
        ImGuiWindow* window                = GImGui->CurrentWindow;
...
// - You are calling ImGui functions after ImGui::Render() and before the next
...
VerticalSeparator(); // Vertical separator, for menu bars (use current line
                     // height). not exposed because it is misleading what it
                     // doesn't have an effect on regular layout.
IMGUI_API bool SplitterBehavior(ImGuiID       id,
                                const ImRect& bb,
                                ImGuiAxis     axis,
                                float*        size1,
                                float*        size2,
...
// NB: All position are in absolute pixels coordinates (never using window
// coordinates internally) AVOID USING OUTSIDE OF IMGUI.CPP! NOT FOR PUBLIC
// CONSUMPTION. THOSE FUNCTIONS ARE A MESS. THEIR SIGNATURE AND BEHAVIOR WILL
// CHANGE, THEY NEED TO BE REFACTORED INTO SOMETHING DECENT.
...
IMGUI_API bool ArrowButton(ImGuiID          id,
                           ImGuiDir         dir,
                           ImVec2           padding,
                           ImGuiButtonFlags flags = 0);

IMGUI_API bool SliderBehavior(const ImRect&    frame_bb,
                              ImGuiID          id,
                              float*           v,
                              float            v_min,
                              float            v_max,
...
IMGUI_API bool SliderFloatN(const char* label,
                            float*      v,
                            int         components,
                            float       v_min,
                            float       v_max,
                            ...
                            float       power);
IMGUI_API bool SliderIntN(const char* label,
                          int*        v,
                          int         components,
                          int         v_min,
                          int         v_max,
...
IMGUI_API bool DragBehavior(const ImRect& frame_bb,
                            ImGuiID       id,
                            float*        v,
                            float         v_speed,
                            float         v_min,
                            ...
                            int           decimal_precision,
                            float         power);
IMGUI_API bool DragFloatN(const char* label,
                          float*      v,
                          int         components,
                          float       v_speed,
                          float       v_min,
                          ...
                          const char* display_format,
                          float       power);
IMGUI_API bool DragIntN(const char* label,
                        int*        v,
                        int         components,
                        float       v_speed,
                        int         v_min,
...
IMGUI_API bool InputTextEx(const char*           label,
                           ...
                           void*                 user_data = NULL);
IMGUI_API bool InputFloatN(const char*         label,
                           float*              v,
                           int                 components,
                           int                 decimal_precision,
                           ImGuiInputTextFlags extra_flags);
IMGUI_API bool InputIntN(const char*         label,
                         int*                v,
                         int                 components,
                         ImGuiInputTextFlags extra_flags);
IMGUI_API bool InputScalarEx(const char*         label,
                             ...
                             ImGuiInputTextFlags extra_flags);
IMGUI_API bool InputScalarAsWidgetReplacement(const ImRect& aabb,
                                              const char*   label,
                                              ImGuiDataType data_type,
                                              void*         data_ptr,
                                              ImGuiID       id,
...
IMGUI_API bool TreeNodeBehaviorIsOpen(
    ImGuiID            id,
    ImGuiTreeNodeFlags flags = 0); // Consume previous SetNextTreeNodeOpened()
...
// Shade functions
IMGUI_API void ShadeVertsLinearColorGradientKeepAlpha(ImDrawVert* vert_start,
                                                      ImDrawVert* vert_end,
                                                      ImVec2      gradient_p0,
                                                      ImVec2      gradient_p1,
                                                      ImU32       col0,
                                                      ImU32       col1);
IMGUI_API void ShadeVertsLinearAlphaGradientForLeftToRightText(
    ImDrawVert* vert_start,
    ImDrawVert* vert_end,
    float       gradient_p0_x,
    float       gradient_p1_x);
IMGUI_API void ShadeVertsLinearUV(ImDrawVert*   vert_start,
                                  ImDrawVert*   vert_end,
                                  const ImVec2& a,
                                  const ImVec2& b,
                                  const ImVec2& uv_a,
...
IMGUI_API void ImFontAtlasBuildMultiplyCalcLookupTable(
    unsigned char out_table[256], float in_multiply_factor);
