...
struct ImColor; // Helper functions to create a color that can be converted to
                // either u32 or float4
struct ImGuiIO; // Main configuration and I/O between your application and ImGui
struct ImGuiOnceUponAFrame; // Simple helper for running a block of code not
                            // more than once a frame, used by
                            // IMGUI_ONCE_UPON_A_FRAME macro
struct ImGuiStorage;        // Simple custom key value storage
struct ImGuiStyle;          // Runtime data for styling/colors
struct ImGuiTextFilter;     // Parse and apply text filters. In format
                            // "aaaaa[,bbbb][,ccccc]"
struct ImGuiTextBuffer;     // Text buffer for logging/accumulating text
struct ImGuiTextEditCallbackData; // Shared state of ImGui::InputText() when
                                  // using custom ImGuiTextEditCallback
                                  // (rare/advanced use)
struct ImGuiSizeCallbackData;     // Structure used to constraint window size in
                              // custom ways when using custom ImGuiSizeCallback
                              // (rare/advanced use)
...
typedef int
    ImGuiCol; // enum: a color identifier for styling     // enum ImGuiCol_
typedef int
    ImGuiCond; // enum: a condition for Set*()             // enum ImGuiCond_
typedef int
            ImGuiKey; // enum: a key identifier (ImGui-side enum) // enum ImGuiKey_
typedef int ImGuiMouseCursor; // enum: a mouse cursor identifier          //
                              // enum ImGuiMouseCursor_
typedef int ImGuiStyleVar; // enum: a variable identifier for styling  // enum
                           // ImGuiStyleVar_
typedef int ImDrawCornerFlags; // flags: for ImDrawList::AddRect*() etc.   //
                               // enum ImDrawCornerFlags_
typedef int ImDrawListFlags; // flags: for ImDrawList                    // enum
                             // ImDrawListFlags_
typedef int ImGuiColorEditFlags; // flags: for ColorEdit*(), ColorPicker*()  //
                                 // enum ImGuiColorEditFlags_
typedef int ImGuiColumnsFlags;   // flags: for *Columns*()                   //
                                 // enum ImGuiColumnsFlags_
typedef int ImGuiDragDropFlags;  // flags: for *DragDrop*()                  //
                                 // enum ImGuiDragDropFlags_
typedef int ImGuiComboFlags; // flags: for BeginCombo()                  // enum
                             // ImGuiComboFlags_
typedef int ImGuiFocusedFlags;    // flags: for IsWindowFocused()             //
                                  // enum ImGuiFocusedFlags_
typedef int ImGuiHoveredFlags;    // flags: for IsItemHovered() etc.          //
                                  // enum ImGuiHoveredFlags_
typedef int ImGuiInputTextFlags;  // flags: for InputText*()                  //
                                  // enum ImGuiInputTextFlags_
typedef int ImGuiSelectableFlags; // flags: for Selectable()                  //
                                  // enum ImGuiSelectableFlags_
typedef int ImGuiTreeNodeFlags;   // flags: for TreeNode*(),CollapsingHeader()//
                                  // enum ImGuiTreeNodeFlags_
typedef int ImGuiWindowFlags;     // flags: for Begin*()                      //
...
// IMGUI_ONCE_UPON_A_FRAME          // Execute a block of code once per frame
// only (convenient for creating UI within deep-nested code that runs multiple
// times)
...
#ifdef IM_VEC2_CLASS_EXTRA // Define constructor and implicit cast operators in
...
// ImGui end-user API
// In a namespace so that user can add extra functions in a separate file (e.g.
// Value() helpers for your vector or common types)
namespace ImGui
{
// Main
...
IMGUI_API             ImDrawData*
                      GetDrawData(); // same value as passed to your io.RenderDrawListsFn() function.
...
IMGUI_API void Render();   // ends the ImGui frame, finalize the draw data, then
                           // call your io.RenderDrawListsFn() function if set.
IMGUI_API void
EndFrame(); // ends the ImGui frame. automatically called by Render(), so most
            // likely don't need to ever call that yourself directly. If you
            // don't need to render you may call EndFrame() but you'll have
            // wasted CPU already. If you don't need to render, better to not
            // create any imgui windows instead!
...
IMGUI_API bool Begin(
    ...
    ImGuiWindowFlags flags =
        0); // push window to the stack and start appending to it. see .cpp for
            // details. return false when window is collapsed, so you can early
            // out in your code. 'bool* p_open' creates a widget on the
            // upper-right to close the window (which sets your bool to false).
IMGUI_API void
...
IMGUI_API       ImDrawList*
                GetWindowDrawList(); // get rendering command-list if you want to append your
                                     // own draw primitives
IMGUI_API ImVec2
...
IMGUI_API void   SetWindowFontScale(
      float scale); // per-window font scale. Adjust IO.FontGlobalScale if you
                    // want to scale all windows
...
IMGUI_API void SetNextWindowContentSize(
    const ImVec2& size); // set next window content size (~ enforce the range of
                         // scrollbars). not including window decorations (title
                         // bar, menu bar, etc.). set an axis to 0.0f to leave
...
IMGUI_API void SetWindowSize(
    ...
    ImGuiCond     cond = 0); // (not recommended) set current window size - call
                         // within Begin()/End(). set to ImVec2(0,0) to force an
                         // auto-fit. prefer using SetNextWindowSize(), as this
...
IMGUI_API void SetStateStorage(
    ImGuiStorage*
        tree); // replace tree state storage with our own (if you want to
...
IMGUI_API const ImVec4& GetStyleColorVec4(
    ImGuiCol idx); // retrieve style color as stored in ImGuiStyle structure.
                   // use to feed back into PushStyleColor(), otherwhise use
...
IMGUI_API void PushItemWidth(
    float item_width); // width of items for the common item+label case, pixels.
                       // 0.0f = default to ~2/3 of windows width, >0.0f: width
                       // in pixels, <0.0f align xx pixels to the right of
...
IMGUI_API void PushTextWrapPos(
    float wrap_pos_x =
        0.0f); // word-wrapping for Text*() commands. < 0.0f: no wrapping; 0.0f:
               // wrap to end of window (or column); > 0.0f: wrap at
...
IMGUI_API void SetCursorScreenPos(
    const ImVec2& pos); // cursor position in absolute screen coordinates
                        // [0..io.DisplaySize]
IMGUI_API void
AlignTextToFramePadding(); // vertically align/lower upcoming text to
                           // FramePadding.y so that it will aligns to upcoming
                           // widgets (call if you have text on a line before
                           // regular widgets)
...
IMGUI_API void SetColumnOffset(
    int column_index,
    float
        offset_x); // set position of column line (in pixels, from the left side
                   // of the contents region). pass -1 to use current column
IMGUI_API int GetColumnsCount();

// ID scopes
// If you are creating widgets in a loop you most likely want to push a unique
// identifier (e.g. object pointer, loop index) so ImGui can differentiate them.
// You can also use the "##foobar" syntax within widget label to distinguish
// them from each others. Read "A primer on the use of labels/IDs" in the FAQ
// for more details.
IMGUI_API void PushID(const char* str_id); // push identifier into the ID stack.
                                           // IDs are hash of the entire stack!
...
IMGUI_API bool InvisibleButton(
    const char*   str_id,
    const ImVec2& size); // button behavior without the visuals, useful to build
                         // custom behaviors using the public api (along with
...
// selection state however you want it. The old Combo() api are helpers over
// BeginCombo()/EndCombo() which are kept available for convenience purpose.
...
IMGUI_API bool SliderFloat(
    ...
    const char* display_format = "%.3f",
    float power = 1.0f); // adjust display_format to decorate the value with a
                         // prefix or a suffix for in-slider labels or unit
                         // display. Use power!=1.0 for logarithmic sliders
...
IMGUI_API bool TreeNode(
    const char* label); // if returning 'true' the node is open and the tree id
                        // is pushed into the id stack. user is responsible for
                        // calling TreePop().
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(
    2); // read the FAQ about why and how to use ID. to align arbitrary text at
...
IMGUI_API void TreePush(
    const char* str_id); // ~ Indent()+PushId(). Already called by TreeNode()
                         // when returning true, but you can call Push/Pop
                         // yourself for layout purpose
...
IMGUI_API bool ListBoxHeader(
    ...
    const ImVec2& size = ImVec2(
        0, 0)); // use if you want to reimplement ListBox() will custom data or
                // interactions. make sure to call ListBoxFooter() afterwards.
...
IMGUI_API void SetTooltip(const char* fmt, ...) IM_FMTARGS(
    1); // set text tooltip under mouse-cursor, typically use with
        // ImGui::IsItemHovered(). overidde any previous call to SetTooltip().
IMGUI_API void SetTooltipV(const char* fmt, va_list args) IM_FMTLIST(1);
IMGUI_API void
BeginTooltip(); // begin/append a tooltip window. to create full-featured
                // tooltip (with any kind of contents).
IMGUI_API void EndTooltip();

// Menus
IMGUI_API bool
BeginMainMenuBar(); // create and append to a full screen menu-bar. only call
                    // EndMainMenuBar() if this returns true!
IMGUI_API void EndMainMenuBar();
IMGUI_API bool
BeginMenuBar(); // append to menu-bar of current window (requires
                // ImGuiWindowFlags_MenuBar flag set on parent window). only
                // call EndMenuBar() if this returns true!
...
IMGUI_API void OpenPopup(
    const char*
        str_id); // call to mark popup as open (don't call every frame!). popups
                 // are closed when user click outside, or if
...
IMGUI_API bool BeginPopup(
    const char*      str_id,
    ImGuiWindowFlags flags =
        0); // return true if the popup is open, and you can start outputting to
            // it. only call EndPopup() if BeginPopup() returned true!
IMGUI_API bool BeginPopupContextItem(
    ...
    int         mouse_button =
        1); // helper to open and begin popup when clicked on last item. if you
            ...
            // you want to use that on a non-interactive item such as Text() you
...
IMGUI_API bool BeginPopupContextWindow(
    ...
    bool also_over_items =
        true); // helper to open and begin popup when clicked on current window.
...
IMGUI_API bool BeginPopupModal(
    ...
    ImGuiWindowFlags flags =
        0); // modal dialog (regular window with title bar, block interactions
            // behind the modal window, can't close the modal window by clicking
            // outside)
...
IMGUI_API bool SetDragDropPayload(
    ...
    ImGuiCond cond =
        0); // type is a user defined string of maximum 8 characters. Strings
...
// Focus
// (FIXME: Those functions will be reworked after we merge the navigation branch
// + have a pass at focusing/tabbing features.) (Prefer using
// "SetItemDefaultFocus()" over "if (IsWindowAppearing()) SetScrollHere()" when
// applicable, to make your code more forward compatible when navigation branch
// is merged)
IMGUI_API void
SetItemDefaultFocus(); // make last item the default focused item of a window
                       // (WIP navigation branch only). Pleaase use instead of
                       // SetScrollHere().
...
IMGUI_API bool IsItemActive(); // is the last item active? (e.g. button being
                               // held, text field being edited- items that
...
IMGUI_API bool IsItemVisible(); // is the last item visible? (aka not out of
                                // sight due to clipping/scrolling.)
...
IMGUI_API bool IsKeyDown(
    int user_key_index); // is key being held. == io.KeysDown[user_key_index].
                         ...
                         // entry of io.KeyDown[]. Use your own indices/enums
...
IMGUI_API bool IsMouseHoveringRect(
    ...
    bool clip =
        true); // is mouse hovering given bounding rect (in screen space).
               // clipped by current clipping settings. disregarding of
...
IMGUI_API ImVec2
          GetMousePosOnOpeningCurrentPopup(); // retrieve backup of mouse positioning at
                                    // the time of opening popup we have
                                    // BeginPopup() into
...
IMGUI_API void CaptureKeyboardFromApp(
    bool capture =
        true); // manually override io.WantCaptureKeyboard flag next frame (said
               // flag is entirely left for your application handle). e.g. force
               // capture keyboard when your widget is being hovered.
IMGUI_API void CaptureMouseFromApp(
    bool capture =
        true); // manually override io.WantCaptureMouse flag next frame (said
               // flag is entirely left for your application handle).
...
// Internal context access - if you want to use multiple context, share context
// between modules (e.g. DLL). There is a default context created and active by
// default. All contexts share a same ImFontAtlas by default. If you want
// different font atlas, you can new() them and overwrite the GetIO().Fonts
// variable of an ImGui context.
...
enum ImGuiWindowFlags_
{
    ...
    // ImGuiWindowFlags_ShowBorders          = 1 << 7,   // Show borders around
    // windows and items (OBSOLETE! Use e.g. style.FrameBorderSize=1.0f to
    // enable borders).
    ...
    ImGuiWindowFlags_ResizeFromAnySide =
        1 << 17, // (WIP) Enable resize from any corners and borders. Your
                 // back-end needs to honor the different values of
                 // io.MouseCursor set by imgui.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackCharFilter =
        1
        << 9, // Call user function to filter character. Modify data->EventChar
              // to replace/filter input, or return 1 to discard character.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover =
        1 << 1, // By default, when dragging we clear data so that
                // IsItemHovered() will return true, to avoid subsequent user
...
// using 8-characters long strings. Types starting with '_' are defined by Dear
// ImGui.
#define IMGUI_PAYLOAD_TYPE_COLOR_3F                                            \
    "_COL3F" // float[3]     // Standard type for colors, without alpha. User
             // code may use this type.
...
enum ImGuiCol_
{
    ...
    ImGuiCol_ModalWindowDarkening, // darken entire screen when a modal window
                                   // is active
...
// Obsolete names (will be removed)
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    //, ImGuiCol_ComboBg = ImGuiCol_PopupBg     // ComboBg has been merged with
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_Uint8 =
        1 << 16, // [DataType]   // ColorEdit, ColorPicker, ColorButton:
                 // _display_ values formatted as 0..255.
    ImGuiColorEditFlags_Float =
        1 << 17, // [DataType]   // ColorEdit, ColorPicker, ColorButton:
                 // _display_ values formatted as 0.0f..1.0f floats instead of
                 // 0..255 integers. No round-trip of value via integers.
    ImGuiColorEditFlags_PickerHueBar =
        1 << 18, // [PickerMode] // ColorPicker: bar for Hue, rectangle for
                 // Sat/Value.
    ImGuiColorEditFlags_PickerHueWheel =
        1 << 19, // [PickerMode] // ColorPicker: wheel for Hue, triangle for
                 // Sat/Value.
    // Internals/Masks
...
// functions All those functions treat 0 as a shortcut to ImGuiCond_Always. From
// the point of view of the user use this as an enum (don't combine multiple
// values into flags).
enum ImGuiCond_
{
    ...
    ImGuiCond_FirstUseEver =
        1 << 2, // Set the variable if the window has no saved data (if doesn't
                // exist in the .ini file)
    ImGuiCond_Appearing =
        1 << 3 // Set the variable if the window is appearing after being
...
struct ImGuiStyle
{
    ...
    ImVec2 DisplayWindowPadding;   // Window positions are clamped to be visible
                                   // within the display area by at least this
                                   // amount. Only covers regular windows.
    ImVec2 DisplaySafeAreaPadding; // If you cannot see the edge of your screen
                                   // (e.g. on a TV) increase the safe area
                                   // padding. Covers popups/tooltips as well
                                   // regular windows.
...
struct ImGuiIO
{
    ...
    ImVec2 DisplaySize; // <unset>              // Display size, in pixels. For
                        // clamping windows positions.
    float DeltaTime; // = 1.0f/60.0f         // Time elapsed since last frame,
                     // in seconds.
    float IniSavingRate; // = 5.0f               // Maximum time between saving
                         // positions/sizes to .ini file, in seconds.
    const char* IniFilename; // = "imgui.ini"        // Path to .ini file. NULL
                             // to disable .ini saving.
    ...
    int KeyMap[ImGuiKey_COUNT]; // <unset>              // Map of indices into
                                // the KeysDown[512] entries array
    ...
    ImVec2 DisplayVisibleMin; // <unset> (0.0f,0.0f)  // If you use DisplaySize
                              // as a virtual space larger than your screen, set
                              // DisplayVisibleMin/Max to the visible area.
    ImVec2 DisplayVisibleMax; // <unset> (0.0f,0.0f)  // If the values are the
                              // same, we defaults to Min=(0.0f) and
                              // Max=DisplaySize

    // Advanced/subtle behaviors
    bool OptMacOSXBehaviors; // = defined(__APPLE__) // OS X style: Text editing
    ...
    bool OptCursorBlink; // = true               // Enable blinking cursor, for
                         // users who consider it annoying.
    ...
    // Alternatively you can keep this to NULL and call GetDrawData() after
    // Render() to get the same pointer. See example applications if you are
    ...
    // Optional: override memory allocations. MemFreeFn() may be called with a
    // NULL pointer. (default to posix malloc/free)
    ...
    bool MouseDown[5]; // Mouse buttons: left, right, middle + extras. ImGui
                       // itself mostly only uses left button
                       // (BeginPopupContext** are using right button). Others
                       // buttons allows us to track if the mouse is being used
                       // by your application + available to user as a
                       // convenience via IsMouse** API.
    float MouseWheel;  // Mouse wheel: 1 unit scrolls about 5 lines text.
    bool  MouseDrawCursor; // Request ImGui to draw a mouse cursor for you (if
                           // you are on a platform without a mouse cursor).
    bool KeyCtrl;          // Keyboard modifier pressed: Control
    bool KeyShift;         // Keyboard modifier pressed: Shift
    bool KeyAlt;           // Keyboard modifier pressed: Alt
    bool KeySuper;         // Keyboard modifier pressed: Cmd/Super/Windows
    bool KeysDown[512]; // Keyboard keys that are pressed (in whatever storage
                        // order you naturally have access to keyboard data)
    ...
    bool WantCaptureMouse; // When io.WantCaptureMouse is true, do not dispatch
                           // mouse input data to your main application. This is
                           // set by ImGui when it wants to use your mouse (e.g.
                           // unclicked mouse is hovering a window, or a widget
                           // is active).
    bool WantCaptureKeyboard; // When io.WantCaptureKeyboard is true, do not
                              // dispatch keyboard input data to your main
                              // application. This is set by ImGui when it wants
                              // to use your keyboard inputs.
    ...
    bool WantMoveMouse; // [BETA-NAV] MousePos has been altered, back-end should
    ...
    int MetricsActiveWindows;  // Number of visible root windows (exclude child
                               // windows)
    ImVec2
    ...
    float  MouseClickedTime[5]; // Time of last click (used to figure out
                                // double-click)
...
bool Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use,
           ...
           ImGuiWindowFlags flags = 0); // Use SetNextWindowSize() instead if
                                        // you want to set a window size.
static inline bool IsRootWindowOrAnyChildHovered()
{
    return IsItemHovered(ImGuiHoveredFlags_RootAndChildWindows);
}
...
// windows implementation of STL with debug enabled is absurdly slow, so let's
// bypass it so our code runs fast in debug). Our implementation does NOT call
// C++ constructors/destructors. This is intentional and we do not require it.
// Do not use this class as a straight std::vector replacement in your code!
template <typename T>
class ImVector
{
public:
    ...
    inline void reserve(int new_capacity)
    {
        if (new_capacity <= Capacity)
            return;
        T* new_data =
            (value_type*)ImGui::MemAlloc((size_t)new_capacity * sizeof(T));
        if (Data)
        {
            memcpy((void*)new_data, Data, (size_t)Size * sizeof(T));
        }
        ImGui::MemFree(Data);
        Data     = new_data;
...
// each Window. We use it to e.g. store collapse state for a tree (Int 0/1),
// store color edit options. This is optimized for efficient reading (dichotomy
// into a contiguous buffer), rare writing (typically tied to user interactions)
// You can use it as custom user storage for temporary values. Declare your own
...
// Shared state of InputText(), passed to callback when a
// ImGuiInputTextFlags_Callback* flag is used and the corresponding callback is
// triggered.
struct ImGuiTextEditCallbackData
{
    ImGuiInputTextFlags
        EventFlag; // One of ImGuiInputTextFlags_Callback* // Read-only
    ImGuiInputTextFlags
    ...
    ImWchar EventChar; // Character input                      // Read-write
                       // (replace character or set to zero)

    // Completion,History,Always events:
    // If you modify the buffer contents make sure you update 'BufTextLen' and
    // set 'BufDirty' to true.
    ImGuiKey EventKey; // Key pressed (Up/Down/TAB)            // Read-only
    char*    Buf; // Current text buffer                  // Read-write (pointed
                  // data only, can't replace the actual pointer)
    ...
    // NB: Helper functions for text manipulation. Calling those function loses
    // selection.
...
// at io.DisplaySize), however you are totally free to apply whatever
// transformation matrix to want to the data (if you apply such transformation
// you'll want to apply it to ClipRect as well) Important: Primitives are always
// added to the list and not culled (culling is done at higher-level by ImGui::
// functions), if you use this API a lot consider coarse culling your drawn
// objects.
struct ImDrawList
{
    ...
    IMGUI_API void PushClipRect(
        ...
        bool intersect_with_current_clip_rect =
            false); // Render-level scissoring. This is passed down to your
                    ...
                    // clipping. Prefer using higher-level ImGui::PushClipRect()
    ...
    IMGUI_API void AddTriangle(const ImVec2& a, const ImVec2& b,
    ...
    IMGUI_API void AddCircleFilled(const ImVec2& centre, float radius,
                                   ImU32 col, int num_segments = 12);
    ...
    IMGUI_API void AddImageQuad(
        ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b,
        const ImVec2& c, const ImVec2& d, const ImVec2& uv_a = ImVec2(0, 0),
        const ImVec2& uv_b = ImVec2(1, 0), const ImVec2& uv_c = ImVec2(1, 1),
        const ImVec2& uv_d = ImVec2(0, 1), ImU32 col = 0xFFFFFFFF);
    IMGUI_API void AddImageRounded(
        ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b,
        const ImVec2& uv_a, const ImVec2& uv_b, ImU32 col, float rounding,
...
struct ImDrawData
{
    ...
    IMGUI_API void
    DeIndexAllBuffers(); // For backward compatibility or convenience: convert
                         // all buffers from indexed to de-indexed, in case you
                         // cannot render indexed. Note: this is slow and most
                         // likely a waste of resources. Always prefer indexed
                         // rendering!
...
struct ImFontConfig
{
    ...
    int OversampleH, OversampleV; // 3, 1     // Rasterize at higher quality for
                                  // sub-pixel positioning. We don't use
...
// Sharing a texture for multiple fonts allows us to reduce the number of draw
// calls during rendering. We also add custom graphic data into the texture that
// serves for ImGui.
//  1. (Optional) Call AddFont*** functions. If you don't call any, the default
//  font will be loaded for you.
//  2. Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve
//  pixels data.
//  3. Upload the pixels data into a texture within your graphics system.
//  4. Call SetTexID(my_tex_id); and pass the pointer/identifier to your
//  texture. This value will be passed back to you during rendering to identify
//  the texture.
// IMPORTANT: If you pass a 'glyph_ranges' array to AddFont*** functions, you
// need to make sure that your array persist up until the ImFont is build (when
// calling GetTextData*** or Build()). We only copy the pointer, not the data.
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont* AddFontFromMemoryTTF(
        ...
        const ImWchar*      glyph_ranges =
            NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas!
                   // Will be deleted after Build(). Set
                   // font_cfg->FontDataOwnedByAtlas to false to keep ownership.
    ...
    IMGUI_API ImFont* AddFontFromMemoryCompressedBase85TTF(
        ...
        const ImWchar*      glyph_ranges =
            NULL); // 'compressed_font_data_base85' still owned by caller.
                   ...
                   // parameter.
    IMGUI_API void
    ClearTexData(); // Clear the CPU-side texture data. Saves RAM once the
                    // texture has been copied to graphics memory.
    IMGUI_API void
                   ClearInputData(); // Clear the input TTF data (inc sizes, glyph ranges)
    IMGUI_API void ClearFonts(); // Clear the ImGui-side font data (glyphs
    ...
    // SetTexID(). RGBA32 format is provided for convenience and compatibility,
    // but note that unless you use CustomRect to draw color data, the RGB
    // pixels emitted from Fonts will all be white (~75% of waste). Pitch =
    // Width * BytesPerPixels
    ...
    IMGUI_API const ImWchar*
                    GetGlyphRangesChinese(); // Default + Japanese + full set of about 21000 CJK
                                             // Unified Ideographs
    IMGUI_API const ImWchar*
    ...
    IMGUI_API int AddCustomRectFontGlyph(
        ...
        const ImVec2& offset =
            ImVec2(0, 0)); // Id needs to be < 0x10000 to register a rectangle
                           // to map into a specific font.
