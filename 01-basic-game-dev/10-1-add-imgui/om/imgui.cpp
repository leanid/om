...
// https://github.com/ocornut/imgui/issues/1269 Developed by Omar Cornut and
// every direct or indirect contributors to the GitHub. This library is free but
// I need your support to sustain development and maintenance. If you work for a
// company, please consider financial support, see Readme. For individuals:
...
/*
 ...
 without a label? (Yes). A primer on labels/IDs.
   - How can I tell when Dear ImGui wants my mouse/keyboard inputs VS when I can
 ...
 Korean, Cyrillic?
   - How can I preserve my Dear ImGui context across reloading a DLL? (loss of
 ...
 but a typical frame won't allocate anything)
 ...
 - Add the Dear ImGui source files to your projects, using your preferred build
 system. It is recommended you build the .cpp files as part of your project and
 not as a library.
 ...
 - Init: retrieve the ImGuiIO structure with ImGui::GetIO() and fill the fields
 marked 'Settings': at minimum you need to set io.DisplaySize (application
 resolution). Later on you will fill your keyboard mapping, clipboard handlers,
 and other advanced features but for a basic integration you don't need to worry
 about it all.
 - Init: call io.Fonts->GetTexDataAsRGBA32(...), it will build the font atlas
 texture, then load the texture pixels into graphics memory.
 - Every frame:
    - In your main loop as early a possible, fill the IO fields marked 'Input'
 (e.g. mouse position, buttons, keyboard info, etc.)
    - Call ImGui::NewFrame() to begin the frame
    - You can use any ImGui function you want between NewFrame() and Render()
    - Call ImGui::Render() as late as you can to end the frame and finalize
 render data. it will call your io.RenderDrawListFn handler. (Even if you don't
 render, call Render() and ignore the callback, or call EndFrame() instead.
 Otherwhise some features will break)
 - All rendering information are stored into command-lists until ImGui::Render()
 is called.
 - Dear ImGui never touches or knows about your GPU state. the only function
 that knows about GPU is the RenderDrawListFn handler that you provide.
 - Effectively it means you can create widgets at any time in your code,
 regardless of considerations of being in "update" vs "render" phases of your
 own application.
 - Refer to the examples applications in the examples/ folder for instruction on
 ...
 - A minimal application skeleton may be:
     ...
     io.RenderDrawListsFn = MyRenderFunction;  // Setup a render function, or
 set to NULL and call GetDrawData() after Render() to access render data.
     // TODO: Fill others settings of the io structure later.

     // Load texture atlas (there is a default font so you don't need to care
 about choosing a font yet) unsigned char* pixels; int width, height;
     io.Fonts->GetTexDataAsRGBA32(pixels, &width, &height);
     // TODO: At this points you've got the texture data and you need to upload
 that your your graphic system: MyTexture* texture =
 MyEngine::CreateTextureFromMemoryPixels(pixels, width, height,
 TEXTURE_TYPE_RGBA)
     // TODO: Store your texture pointer/identifier (whatever your engine uses)
 in 'io.Fonts->TexID'. This will be passed back to your via the renderer.
 ...
 to those fields from your Windows message loop handlers, etc.) ImGuiIO& io =
 ImGui::GetIO(); io.DeltaTime = 1.0f/60.0f; io.MousePos = mouse_pos;
        io.MouseDown[0] = mouse_button_0;
        io.MouseDown[1] = mouse_button_1;

        // Call NewFrame(), after this point you can use ImGui::* functions
 anytime ImGui::NewFrame();
 ...
 graphics API supports it.
                 // If your engine doesn't support scissoring yet, you will get
 some small glitches (some elements outside their bounds) which you can fix
 later. MyEngineScissor((int)pcmd->ClipRect.x, (int)pcmd->ClipRect.y,
 ...
 'io.WantCaptureMouse'/'io.WantCaptureKeyboard'/'io.WantTextInput' flags are
 updated. They tell you if ImGui intends to use your inputs. So for example, if
 'io.WantCaptureMouse' is set you would typically want to hide mouse inputs from
 the rest of your application. Read the FAQ below for more information about
 those flags.
 ...
 generally minor and easy to fix. Here is a change-log of API breaking changes,
 if you are using one of the functions listed, expect to have to fix some code.
 Also read releases logs https://github.com/ocornut/imgui/releases for more
 details.

 - 2018/01/11 (1.54) - obsoleted IsAnyWindowHovered() in favor of
 IsWindowHovered(ImGuiHoveredFlags_AnyWindow). Kept redirection function (will
 obsolete).
 - 2018/01/11 (1.54) - obsoleted IsAnyWindowFocused() in favor of
 IsWindowFocused(ImGuiFocusedFlags_AnyWindow). Kept redirection function (will
 obsolete).
 - 2018/01/03 (1.54) - renamed ImGuiSizeConstraintCallback to ImGuiSizeCallback,
 ImGuiSizeConstraintCallbackData to ImGuiSizeCallbackData.
 - 2017/12/29 (1.54) - removed CalcItemRectClosestPoint() which was weird and
 ...
 default theme it will not affect you. However if your TitleBg/TitleBgActive
 alpha was <1.0f you need to tweak your custom theme to readjust for the fact
 that we don't draw a WindowBg background behind the title bar. This helper
 function will convert an old TitleBg/TitleBgActive color into a new one with
 the same visual output, given the OLD color and the OLD WindowBg color. ImVec4
 ...
 changed! ImGui_XXXX_RenderDrawLists(ImDrawList** const cmd_lists, int
 cmd_lists_count) became: ImGui_XXXX_RenderDrawLists(ImDrawData* draw_data).
                              argument   'cmd_lists'        ->
 'draw_data->CmdLists' argument   'cmd_lists_count'  ->
 'draw_data->CmdListsCount' ImDrawList 'commands'         -> 'CmdBuffer'
 ...
 uncompressed pixels. this sequence: const void* png_data; unsigned int
 png_size; ImGui::GetDefaultFontData(NULL, NULL, &png_data, &png_size);
                           // <Copy to GPU>
                       became:
 ...
 fonts and manage the texture buffer for internal needs. it is now recommended
 that you sample the font texture with bilinear interpolation. (1.30) - added
 texture identifier in ImDrawCmd passed to your render function (we can now
 render images). make sure to set io.Fonts->TexID. (1.30) - removed
 IO.PixelCenterOffset (unnecessary, can be handled in user projection matrix)
 ...
 Q: How can I help?
 A: - If you are experienced enough with Dear ImGui and with C/C++, look at the
 todo list and see how you want/can help!
    - Become a Patron/donate! Convince your company to become a Patron or
 provide serious funding for development time! See http://www.patreon.com/imgui

 Q: What is ImTextureID and how do I display an image?
 A: ImTextureID is a void* used to pass renderer-agnostic texture references
 around until it hits your render function. Dear ImGui knows nothing about what
 those bits represent, it just passes them around. It is up to you to decide
 what you want the void* to carry! It could be an identifier to your OpenGL
 texture (cast GLuint to void*), a pointer to your custom engine material (cast
 MyMaterial* to void*), etc. At the end of the chain, your renderer takes this
 void* to cast it back into whatever it needs to select a current texture to
 render. Refer to examples applications, where each renderer (in a
 imgui_impl_xxxx.cpp file) is treating ImTextureID as a different thing. (c++
 tip: OpenGL uses integers to identify textures. You can safely store an integer
 into a void*, just cast it to void*, don't take it's address!) To display a
 custom image/texture within an ImGui window, you may use ImGui::Image(),
 ImGui::ImageButton(), ImDrawList::AddImage() functions. Dear ImGui will
 generate the geometry and draw calls using the ImTextureID that you passed and
 which your renderer can use. It is your responsibility to get textures uploaded
 to your GPU.
 ...
 label? A: Yes. A primer on the use of labels/IDs in Dear ImGui..

   - Elements that are not clickable, such as Text() items don't need an ID.

   - Interactive widgets require state to be carried over multiple frames (most
 typically Dear ImGui often needs to remember what is the "active" widget). to
 do so they need a unique ID. unique ID are typically derived from a string
 label, an integer index or a pointer.

       Button("OK");        // Label = "OK",     ID = hash of "OK"
       Button("Cancel");    // Label = "Cancel", ID = hash of "Cancel"

   - ID are uniquely scoped within windows, tree nodes, etc. so no conflict can
 happen if you have two buttons called "OK" in two different windows or in two
 different locations of a tree.
 ...
 within string itself. This helps solving the simpler collision cases. Use "##"
 to pass a complement to the ID that won't be visible to the end-user:

       Button("Play");         // Label = "Play",   ID = hash of "Play"
       Button("Play##foo1");   // Label = "Play",   ID = hash of "Play##foo1"
 (different from above) Button("Play##foo2");   // Label = "Play",   ID = hash
 of "Play##foo2" (different from above)
 ...
 include varying information in a window title bar (and windows are uniquely
 identified by their ID.. obviously) Use "###" to pass a label that isn't part
 of ID:
 ...
 same Window. This is the most convenient way of distinguishing ID if you are
 iterating and creating many UI elements. You can push a pointer, a string or an
 integer value. Remember that ID are formed from the concatenation of everything
 in the ID stack!
 ...
 indices or pointers as ID. e.g. when displaying a single object that may change
 over time (dynamic 1-1 relationship), using a static string as ID will preserve
 your node open/closed state when the targeted object change. e.g. when
 displaying a list of objects, using indices or pointers as ID will preserve the
 node open/closed state differently. experiment and see what makes more sense!

 Q: How can I tell when Dear ImGui wants my mouse/keyboard inputs VS when I can
 pass them to my application? A: You can read the
 'io.WantCaptureMouse'/'io.WantCaptureKeyboard'/'ioWantTextInput' flags from the
 ImGuiIO structure.
    - When 'io.WantCaptureMouse' or 'io.WantCaptureKeyboard' flags are set you
 ...
 on-screen keyboard, if available (e.g. on a mobile phone, or console OS).
    Preferably read the flags after calling ImGui::NewFrame() to avoid them
 lagging by one frame. But reading those flags before calling NewFrame() is also
 generally ok, as the bool toggles fairly rarely and you don't generally expect
 to interact with either Dear ImGui or your application during the same frame
 when that transition occurs. Dear ImGui is tracking dragging and widget
 activity that may occur outside the boundary of a window, so
 'io.WantCaptureMouse' is more accurate and correct than checking if a window is
 hovered. (Advanced note: text input releases focus on Return 'KeyDown', so the
 following Return 'KeyUp' event that your application receive will typically
     have 'io.WantCaptureKeyboard=false'. Depending on your application logic it
 may or not be inconvenient. You might want to track which key-downs were for
 Dear ImGui, e.g. with an array of bool, and filter out the corresponding
 key-ups.)

 Q: How can I load a different font than the default? (default is an embedded
 version of ProggyClean.ttf, rendered at size 13) A: Use the font atlas to load
 the TTF/OTF file you want:
 ...
 'extra_fonts/README.txt' for instructions and useful header files.
 ...
 // Add a specific character
      builder.AddRanges(io.Fonts->GetGlyphRangesJapanese()); // Add one of the
 default ranges builder.BuildRanges(&ranges);                          // Build
 the final result (ordered ranges with all the unique characters submitted)
      io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels, NULL,
 ranges.Data);
 ...
 UTF-8 or load text data from file already saved as UTF-8.
    ...
    Text input: it is up to your application to pass the right character code to
 io.AddInputCharacter(). The applications in examples/ are doing that. For
 languages using IME, on Windows you can copy the Hwnd of your application to
 io.ImeWindowHandle. The default implementation of io.ImeSetInputScreenPosFn()
 on Windows will set your IME position correctly.

 Q: How can I preserve my Dear ImGui context across reloading a DLL? (loss of
 the global/static variables) A: Create your own context 'ctx = CreateContext()'
 + 'SetCurrentContext(ctx)' and your own font atlas 'ctx->GetIO().Fonts = new
 ImFontAtlas()' so you don't rely on the default globals.

 Q: How can I use the drawing facilities without an ImGui window? (using
 ImDrawList API) A: The easiest way is to create a dummy window. Call Begin()
 with NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flag, zero
 background alpha, then retrieve the ImDrawList* via GetWindowDrawList() and
 draw to it in any way you like. You can also perfectly create a standalone
 ImDrawList instance _but_ you need ImGui to be initialized because ImDrawList
 pulls from ImGui data to retrieve the coordinates of the white pixel.
...
#pragma GCC diagnostic ignored "-Wcast-qual" // warning: cast from type 'xxxx'
                                             ...
                                             // qualifiers
#pragma GCC diagnostic ignored "-Wformat-nonliteral" // warning: format not a
...
static inline void DataTypeFormatString(ImGuiDataType data_type,
                                        void*         data_ptr,
...
// to a different context by calling ImGui::SetCurrentContext() If you are
// hot-reloading this code in a DLL you will lose the static/global variables.
// Create your own context+font atlas instead of relying on those default (see
// FAQ entry "How can I preserve my ImGui context across reloading a DLL?").
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowBorderSize = 0.0f; // Thickness of border around windows. Generally
                             // set to 0.0f or 1.0f. Other values not well
                             // tested.
    ...
    ChildBorderSize = 1.0f;  // Thickness of border around child windows.
                             // Generally set to 0.0f or 1.0f. Other values not
                             // well tested.
    ...
    ItemInnerSpacing = ImVec2(4, 4); // Horizontal and vertical spacing between
                                     // within elements of a composed widget
                                     // (e.g. a slider and its label)
    TouchExtraPadding =
        ImVec2(0, 0); // Expand reactive bounding box for touch-based system
                      // where touch position is not accurate enough.
                      // Unfortunately we don't sort widgets so priority on
                      // overlap will always be given to the first widget. So
    ...
    DisplaySafeAreaPadding =
        ImVec2(4, 4); // If you cannot see the edge of your screen (e.g. on a
    ...
    CurveTessellationTol = 1.25f; // Tessellation tolerance when using
...
#ifdef __APPLE__
    OptMacOSXBehaviors = true; // Set Mac OS X style defaults based on __APPLE__
                               // compile time flag
#else
           ...
           *in_text)
    {
        ...
        if (c < 0x80)
            *buf_out++ = (char)c;
            ...
            buf_out +=
                ImTextCharToUtf8(buf_out, (int)(buf_end - buf_out - 1), c);
    }
    *buf_out = 0;
    return (int)(buf_out - buf);
}
...
ImU32 ImGui::GetColorU32(ImU32 col)
{
    ...
    a     = (int)(a * style_alpha); // We don't need to clamp 0..255 because
...
void ImGui::ColorConvertRGBtoHSV(
...
void ImGui::ColorConvertHSVtoRGB(
...
static ImVector<ImGuiStorage::Pair>::iterator LowerBound(
    ImVector<ImGuiStorage::Pair>& data, ImGuiID key)
{
...
void ImGuiTextBuffer::appendfv(const char* fmt, va_list args)
{
    ...
    int len = ImFormatStringV(NULL, 0, fmt, args); // FIXME-OPT: could do a
                                                   // first pass write attempt,
                                                   // likely successful on first
                                                   // pass.
    if (len <= 0)
        return;
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.PrevLineHeight =
        (line_height - GImGui->Style.ItemSpacing.y); // If we end up needing
                                                     // more accurate data (to
                                                     // e.g. use SameLine) we
                                                     // may as well make the
                                                     // clipper have a fourth
                                                     // step to let user process
                                                     // and display the last
                                                     // item in their list.
    if (window->DC.ColumnsSet)
        window->DC.ColumnsSet->CellMinY =
            window->DC.CursorPos
...
void ImGuiListClipper::Begin(int count, float items_height)
{
    ...
    if (ItemsHeight > 0.0f)
    {
        ...
        if (DisplayStart > 0)
            SetCursorPosYAndSetupDummyPrevLine(StartPosY +
                                                   DisplayStart * ItemsHeight,
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 2) // Step 2: dummy step only required if an explicit
                     // items_height was passed to constructor or Begin() and
                     // user still call Step(). Does nothing and switch to
                     // Step 3.
    {
...
{
    ImGuiID   seed     = IDStack.back();
    const int r_rel[4] = { (int)(r_abs.Min.x - Pos.x),
                           (int)(r_abs.Min.y - Pos.y),
                           (int)(r_abs.Max.x - Pos.x),
...
void ImGui::SetHoveredID(ImGuiID id)
{
    ...
    g.HoveredIdTimer        = (id != 0 && g.HoveredIdPreviousFrame == id)
...
// Typically, widgets that spread over available surface declares their minimum
                                  ...
                                  bool         tab_stop)
{
    ...
    if (tab_stop && (g.ActiveId == id) &&
        ...
        window->FocusIdxTabRequestNext =
            window->FocusIdxTabCounter +
            (g.IO.KeyShift ? (allow_keyboard_focus ? -1 : 0)
                           : +1); // Modulo on index will be applied at the end
                                  // of frame once we've got the total counter
                                  // of items.
...
void ImGui::NewFrame()
{
    ...
    // Check user data
    // (We pass an error message in the assert expression as a trick to get it
    // visible to programmers who are not using a debugger, as most assert
    // handlers display their argument)
    ...
    IM_ASSERT(g.IO.Fonts->Fonts.Size > 0 && "Font Atlas not created. Did you "
                                            "call io.Fonts->GetTexDataAsRGBA32 "
                                            "/ GetTexDataAsAlpha8 ?");
    IM_ASSERT(g.IO.Fonts->Fonts[0]->IsLoaded() &&
              "Font Atlas not created. Did you call "
    ...
    if (g.DragDropActive && g.DragDropPayload.DataFrameCount + 1 < g.FrameCount)
    {
        ...
        memset(
            &g.DragDropPayloadBufLocal, 0, sizeof(g.DragDropPayloadBufLocal));
    }
    ...
    memcpy(g.IO.KeysDownDurationPrev,
    ...
    // component, but in reality we test for -256000.0f) we cancel out movement
    // in MouseDelta
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        if (g.IO.MouseClicked[i])
        {
            if (g.Time - g.IO.MouseClickedTime[i] < g.IO.MouseDoubleClickTime)
            {
                ...
                g.IO.MouseClickedTime[i] = -FLT_MAX; // so the third click isn't
                                                     // turned into a
                                                     // double-click
            }
            else
        ...
        else if (g.IO.MouseDown[i])
        {
            ...
            g.IO.MouseDragMaxDistanceAbs[i].x =
                ImMax(g.IO.MouseDragMaxDistanceAbs[i].x,
                      mouse_delta.x < 0.0f ? -mouse_delta.x : mouse_delta.x);
            g.IO.MouseDragMaxDistanceAbs[i].y =
                ImMax(g.IO.MouseDragMaxDistanceAbs[i].y,
                      mouse_delta.y < 0.0f ? -mouse_delta.y : mouse_delta.y);
            g.IO.MouseDragMaxDistanceSqr[i] = ImMax(
                g.IO.MouseDragMaxDistanceSqr[i], ImLengthSqr(mouse_delta));
        }
    }
    ...
    if (g.MovingWindowMoveId && g.MovingWindowMoveId == g.ActiveId)
    {
        ...
        if (g.IO.MouseDown[0])
        {
            ...
            if (g.MovingWindow->RootWindow->PosFloat.x != pos.x ||
                g.MovingWindow->RootWindow->PosFloat.y != pos.y)
    ...
    g.HoveredWindow =
        (g.MovingWindow && !(g.MovingWindow->Flags & ImGuiWindowFlags_NoInputs))
            ? g.MovingWindow
            : FindHoveredWindow(g.IO.MousePos);
    ...
    // Update the WantCaptureMouse/WantCAptureKeyboard flags, so user can
    // capture/discard the inputs away from the rest of their application. When
    // clicking outside of a window we assume the click is owned by the
    ...
    if (g.WantCaptureMouseNextFrame != -1)
        ...
        g.IO.WantCaptureMouse = (mouse_avail_to_imgui &&
    ...
    g.IO.WantTextInput =
        (g.WantTextInputNextFrame != -1) ? (g.WantTextInputNextFrame != 0) : 0;
        ...
        !g.HoveredWindow->Collapsed)
    {
        ...
        else if (!g.IO.KeyCtrl)
        {
            ...
            // If a child window has the ImGuiWindowFlags_NoScrollWithMouse
            // flag, we give a chance to scroll its parent (unless either
            // ImGuiWindowFlags_NoInputs or ImGuiWindowFlags_NoScrollbar are
            // also set).
            ImGuiWindow* scroll_window = window;
            while (
                (scroll_window->Flags & ImGuiWindowFlags_ChildWindow) &&
            ...
            if (!(scroll_window->Flags & ImGuiWindowFlags_NoScrollWithMouse) &&
                !(scroll_window->Flags & ImGuiWindowFlags_NoInputs))
            {
                float scroll_amount = 5 * scroll_window->CalcFontSize();
                scroll_amount       = (float)(int)ImMin(
                    scroll_amount,
                    (scroll_window->ContentsRegionRect.GetHeight() +
                     scroll_window->WindowPadding.y * 2.0f) *
                        0.67f);
                SetWindowScrollY(scroll_window,
                                 scroll_window->Scroll.y -
                                           ...
                                           ImGuiTextBuffer*      buf)
{
    ...
    for (int i = 0; i != g.SettingsWindows.Size; i++)
    {
        ...
        if (const char* p = strstr(name, "###")) // Skip to the "###" marker if
                                                 // any. We don't skip past to
...
void ImGui::Shutdown()
{
    ...
    // Cleanup of other data are conditional on actually having initialize
    // ImGui.
    if (!g.Initialized)
        return;
...
{
    // For convenience and to make the code simpler, we'll write zero
    // terminators inside the buffer. So let's create a writable copy.
                                    ...
                                    ImGuiWindow*            window)
{
    ...
    if (window->Active)
    {
        ...
        if (count > 1)
            qsort(window->DC.ChildWindows.begin(),
                  (size_t)count,
                  sizeof(ImGuiWindow*),
                  ChildWindowComparer);
        for (int i = 0; i < count; i++)
                                    ...
                                    ImDrawList*            draw_list)
{
    ...
    // Check that draw_list doesn't use more vertices than indexable in a single
    // draw call (default ImDrawIdx = unsigned short = 2 bytes = 64K vertices
    // per ImDrawList = per window) If this assert triggers because you are
    // drawing lots of stuff manually: A) Make sure you are coarse clipping,
    // because ImDrawList let all your vertices pass. You can use thre Metrics
    // window to inspect draw list contents. B) If you need/want meshes with
    // more than 64K vertices, uncomment the '#define ImDrawIdx unsigned int'
    // line in imconfig.h to set the index size to 4 bytes.
...
void ImGui::EndFrame()
{
    ...
    if (g.ActiveId == 0 && g.HoveredId == 0)
    {
                 ...
                 ->Appearing) // Unless we just made a window/popup appear
        {
            ...
            if (g.IO.MouseClicked[0])
            {
                ...
                {
                    // Set ActiveId even if the _NoMove flag is set, without it
                    // dragging away from a window with _NoMove would activate
                    // hover on other windows.
                    FocusWindow(g.HoveredWindow);
                    SetActiveID(g.HoveredWindow->MoveId, g.HoveredWindow);
                    g.ActiveIdClickOffset =
                        g.IO.MousePos - g.HoveredRootWindow->Pos;
            ...
            // NewFrame->CloseInactivePopups to trigger)
            if (g.IO.MouseClicked[1])
            {
                ...
                CloseInactivePopups(hovered_window_above_modal ? g.HoveredWindow
                                                               : modal);
            }
        }
    }
    ...
    for (int i = 0; i != g.Windows.Size; i++)
    {
        ImGuiWindow* window = g.Windows[i];
        if (window->Active &&
            (window->Flags & ImGuiWindowFlags_ChildWindow)) // if a child is
                                                            // active its parent
                                                            // will add it
            continue;
...
void ImGui::Render()
{
    ...
    // practice that you don't create windows in the first place, or
    // consistently respond to Begin() returning false.
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.RenderDrawData.CmdListsCount > 0 &&
            g.IO.RenderDrawListsFn != NULL)
...
void ImGui::RenderFrame(
    ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
    ImGuiContext& g      = *GImGui;
    ImGuiWindow*  window = g.CurrentWindow;
...
void ImGui::RenderBullet(ImVec2 pos)
{
    ...
    window->DrawList->AddCircleFilled(
        pos, GImGui->FontSize * 0.20f, GetColorU32(ImGuiCol_Text), 8);
}
                           ...
                           float       wrap_width)
{
    ...
    ImVec2 text_size = font->CalcTextSizeA(
        font_size, FLT_MAX, wrap_width, text, text_display_end, NULL);
                               ...
                               float repeat_rate)
{
    ...
    return CalcTypematicPressedRepeatAmount(
        t, t - g.IO.DeltaTime, repeat_delay, repeat_rate);
}
...
bool ImGui::IsKeyPressed(int user_key_index, bool repeat)
{
    ...
    if (repeat && t > g.IO.KeyRepeatDelay)
        return GetKeyPressedAmount(
                   user_key_index, g.IO.KeyRepeatDelay, g.IO.KeyRepeatRate) > 0;
    return false;
}
...
bool ImGui::IsItemClicked(int mouse_button)
{
    return IsMouseClicked(mouse_button) &&
           IsItemHovered(ImGuiHoveredFlags_Default);
}
                           ...
                           bool             override_previous_tooltip)
{
    ...
    if (override_previous_tooltip)
        if (ImGuiWindow* window = FindWindowByName(window_name))
            if (window->Active)
            {
                // Hide previous tooltips. We can't easily "reset" the content
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ...
    popup_ref.OpenPopupPos =
        g.IO.MousePos; // NB: In the Navigation branch OpenPopupPos doesn't use
                       // the mouse position, hence the separation here.
    ...
    else
    {
        ...
        // CloseInactivePopups(). This is equivalent to what ClosePopupToLevel()
        // does.
        // if (g.OpenPopupStack[current_stack_size].PopupId == id)
...
static void CloseInactivePopups(ImGuiWindow* ref_window)
{
    ...
    if (n < g.OpenPopupStack.Size) // This test is not required but it allows to
                                   // set a convenient breakpoint on the block
                                   // below
        ClosePopupToLevel(n);
}
                            ...
                            ImGuiWindowFlags flags)
{
    ...
    if (g.NextWindowData.PosCond == 0)
        SetNextWindowPos(
            g.IO.DisplaySize * 0.5f, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
...
bool ImGui::OpenPopupOnItemClick(const char* str_id, int mouse_button)
{
        ...
        IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup))
    {
        ImGuiID id = str_id ? window->GetID(str_id)
                            : window->DC.LastItemId; // If user hasn't passed an
                                                     // ID, we can use the
                                                     // LastItemID. Using
                                                     // LastItemID as a Popup ID
                                                     // won't conflict!
        IM_ASSERT(id != 0); // However, you cannot pass a NULL str_id if the
                            // last item has no identifier (e.g. a Text() item)
        OpenPopupEx(id);
        return true;
    }
    return false;
...
bool ImGui::BeginPopupContextItem(const char* str_id, int mouse_button)
{
    ...
    ImGuiID      id     = str_id ? window->GetID(str_id)
                                 : window->DC.LastItemId; // If user hasn't passed an ID,
                                                 ...
                                                 // ID won't conflict!
    IM_ASSERT(id != 0); // However, you cannot pass a NULL str_id if the last
                        // item has no identifier (e.g. a Text() item)
    if (IsMouseReleased(mouse_button) &&
        IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup))
        OpenPopupEx(id);
                         ...
                         ImGuiWindowFlags extra_flags)
{
    ...
    if (size.x <= 0.0f)
        size.x = ImMax(content_avail.x + size.x, 4.0f); // Arbitrary minimum
                                                        // child size (0.0f
                                                        // causing too much
                                                        // issues)
    if (size.y <= 0.0f)
    ...
    if (name)
        ...
        ImFormatString(
            title, IM_ARRAYSIZE(title), "%s/%08X", parent_window->Name, id);
                       ...
                       ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return BeginChildEx(
        str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
...
void ImGui::EndChild()
{
    ...
    {
        // When using auto-filling child window, we don't provide full
        // width/height to ItemSize so that it doesn't feed back into automatic
        // size-fitting.
        ImVec2 sz = GetWindowSize();
        if (window->AutoFitChildAxises &
            (1 << ImGuiAxis_X)) // Arbitrary minimum zero-ish child size of 4.0f
    ...
    ImGuiPopupPositionPolicy policy = ImGuiPopupPositionPolicy_Default)
{
    ...
    // point around the cursor.) r_outer = the visible area rectangle, minus
    // safe area padding. If our popup size won't fit because of safe area
    // padding we ignore it.
    ImVec2 safe_padding = style.DisplaySafeAreaPadding;
    ImRect r_outer(GetVisibleRect());
    r_outer.Expand(ImVec2(
    ...
    {
        const ImGuiDir dir_prefered_order[ImGuiDir_Count_] = {
            ImGuiDir_Down, ImGuiDir_Right, ImGuiDir_Left, ImGuiDir_Up
        };
        for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_Count_;
             n++)
        {
                                    ...
                                    ImGuiWindowFlags flags)
{
    ...
    if (!(flags & ImGuiWindowFlags_NoSavedSettings))
    {
        ...
        // Use SetWindowPos() or SetNextWindowPos() with the appropriate
        // condition flag to change the initial position of a window.
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    {
        // Tooltip always resize. We keep the spacing symmetric on both axises
...
static void CalcResizePosSizeFromAnyCorner(ImGuiWindow*  window,
                                           ...
                                           ImVec2*       out_size)
{
    ImVec2 pos_min          = ImLerp(corner_target,
                            window->Pos,
                            corner_norm); // Expected window upper-left
    ImVec2 pos_max          = ImLerp(window->Pos + window->Size,
                            corner_target,
...
static ImRect GetBorderRect(ImGuiWindow* window,
                            int          border_n,
                            float        perp_padding,
                            float        thickness)
{
                                      ...
                                      ImU32         resize_grip_col[4])
{
    ...
    const int resize_border_count =
        (flags & ImGuiWindowFlags_ResizeFromAnySide) ? 4 : 0;
         ...
         resize_grip_n++)
    {
        ...
        ButtonBehavior(resize_rect,
                       window->GetID((void*)(intptr_t)resize_grip_n),
                       &hovered,
                       &held,
        ...
        else if (held)
        {
            ...
            ImVec2 corner_target =
                ...
                resize_rect.GetSize() * grip.CornerPos; // Corner of the window
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        bool        hovered, held;
        ImRect      border_rect =
            GetBorderRect(window, border_n, grip_hover_size, BORDER_SIZE);
        ...
        if (held)
        {
            ...
            CalcResizePosSizeFromAnyCorner(
                window, border_target, border_posn, &pos_target, &size_target);
        }
    }
    PopID();
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (!window)
    {
        ImVec2 size_on_first_use =
            (g.NextWindowData.SizeCond != 0)
                ...
                : ImVec2(0.0f, 0.0f); // Any condition flag will do since we are
    ...
    if (flags & ImGuiWindowFlags_Popup)
    {
        ...
        window_just_activated_by_user |=
            (window->PopupId != popup_ref.PopupId); // We recycle popups so
                                                    // treat window as activated
                                                    // if popup id changed
    ...
    if (g.NextWindowData.PosCond)
    {
        ...
        {
            SetWindowPos(
                window, g.NextWindowData.PosVal, g.NextWindowData.PosCond);
        }
    ...
    if (g.NextWindowData.SizeCond)
    {
        ...
        SetWindowSize(
            window, g.NextWindowData.SizeVal, g.NextWindowData.SizeCond);
        g.NextWindowData.SizeCond = 0;
    }
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Lock window rounding, border size and rounding so that altering the
        // border sizes for children doesn't have side-effects.
        ...
        window->WindowBorderSize = (flags & ImGuiWindowFlags_ChildWindow)
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) &&
            ...
            window->WindowPadding = ImVec2(0.0f,
        ...
        // Update contents size from last frame for auto-fitting (unless
        // explicitly specified)
        window->SizeContents = CalcSizeContents(window);

        // Hide popup/tooltip window when re-opening while we measure size
        // (because we recycle the windows)
        ...
        window->Size     = window->Collapsed ? window->TitleBarRect().GetSize()
                                             : window->SizeFull;
        if ((flags & ImGuiWindowFlags_ChildWindow) &&
            !(flags & ImGuiWindowFlags_Popup))
        {
            IM_ASSERT(window_size_x_set_by_api &&
                      window_size_y_set_by_api); // Submitted by BeginChild()
            window->Size = window->SizeFull;
        }
        ...
        const bool window_pos_with_pivot =
            (window->SetWindowPosVal.x != FLT_MAX && window->HiddenFrames == 0);
        ...
        else if (flags & ImGuiWindowFlags_ChildMenu)
        {
            ...
            // menu item, and then our FindBestPopupWindowPos() function will
            // move the new menu outside the parent bounds. This is how we end
            // up with child menus appearing (most-commonly) on the right of the
            // parent menu.
            IM_ASSERT(window_pos_set_by_api);
            float horizontal_overlap =
                style.ItemSpacing.x; // We want some overlap to convey the
                                     // relative depth of each popup (currently
                                     // the amount of overlap it is hard-coded
                                     // to style.ItemSpacing.x, may need to
                                     // introduce another style value).
            ...
            if (parent_menu->DC.MenuBarAppending)
                rect_to_avoid =
                    ImRect(-FLT_MAX,
                           ...
                           FLT_MAX,
                           parent_menu->Pos.y + parent_menu->TitleBarHeight() +
                               parent_menu->MenuBarHeight());
            else
                rect_to_avoid = ImRect(
                    parent_menu->Pos.x + horizontal_overlap,
                    -FLT_MAX,
                    parent_menu->Pos.x + parent_menu->Size.x -
                        horizontal_overlap - parent_menu->ScrollbarSizes.x,
                    FLT_MAX);
            window->PosFloat =
                FindBestWindowPosForPopup(window->PosFloat,
                                          window->Size,
                 ...
                 window_just_appearing_after_hidden_for_resize)
        {
            ImRect rect_to_avoid(window->PosFloat.x - 1,
                                 window->PosFloat.y - 1,
                                 window->PosFloat.x + 1,
                                 window->PosFloat.y + 1);
            window->PosFloat =
        ...
        if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api)
        {
            ...
            ImRect rect_to_avoid(ref_pos.x - 16,
                                 ref_pos.y - 8,
                                 ref_pos.x + 24,
                                 ref_pos.y + 24); // FIXME: Completely
                                                  // hard-coded. Store boxes in
                                                  // mouse cursor data? Scale?
                                                  // Center on cursor hit-point?
            window->PosFloat =
                FindBestWindowPosForPopup(ref_pos,
                                          window->Size,
                                          &window->AutoPosLastDirection,
                                          rect_to_avoid);
            if (window->AutoPosLastDirection == ImGuiDir_None)
                window->PosFloat =
                    ref_pos + ImVec2(2, 2); // If there's not enough room, for
                                            // tooltip we prefer avoiding the
                                            // cursor at all cost even if it
                                            // means that part of the tooltip
                                            // won't be visible.
        }
            ...
            !(flags & ImGuiWindowFlags_Tooltip))
        {
            if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 &&
                ...
                g.IO.DisplaySize.y > 0.0f) // Ignore zero-sized display
                                           // explicitly to avoid losing
                                           // positions if a window manager
                                           // reports zero-sized window when
                                           // initializing or minimizing.
            {
                ImVec2 padding = ImMax(style.DisplayWindowPadding,
                                       style.DisplaySafeAreaPadding);
                window->PosFloat =
                    ImMax(window->PosFloat + window->Size, padding) -
                    window->Size;
                window->PosFloat =
                    ImMin(window->PosFloat, g.IO.DisplaySize - padding);
            }
        }
        window->Pos = ImFloor(window->PosFloat);
        ...
        const int resize_grip_count =
            (flags & ImGuiWindowFlags_ResizeFromAnySide) ? 2 : 1; // 4
        const float grip_draw_size = (float)(int)ImMax(
            g.FontSize * 1.35f,
            window->WindowRounding + 1.0f + g.FontSize * 0.2f);
        if (!window->Collapsed)
            UpdateManualResize(window,
                               size_auto_fit,
                               &border_held,
                               resize_grip_count,
                               &resize_grip_col[0]);
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) &&
            !(flags & ImGuiWindowFlags_Popup))
            PushClipRect(
                parent_window->ClipRect.Min, parent_window->ClipRect.Max, true);
        else
        ...
        const bool  window_is_focused =
            want_focus || (g.NavWindow && window->RootNonPopupWindow ==
                                              g.NavWindow->RootNonPopupWindow);
        if (window->Collapsed)
        {
            // Title bar only
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(
                    title_bar_rect.Min,
                    title_bar_rect.Max,
                    GetColorU32(window_is_focused ? ImGuiCol_TitleBgActive
                                                  : ImGuiCol_TitleBg),
                    window_rounding,
            ...
            if (flags & ImGuiWindowFlags_MenuBar)
            {
                ...
                menu_bar_rect.ClipWith(window->Rect()); // Soft clipping, in
                                                        // particular child
                                                        // window don't have
                                                        // minimum size covering
                                                        // the menu bar so this
                                                        // is useful for them.
                window->DrawList->AddRectFilled(
                    menu_bar_rect.Min,
                    menu_bar_rect.Max,
            ...
            if (!(flags & ImGuiWindowFlags_NoResize))
            {
                     ...
                     resize_grip_n++)
                {
                    ...
                    const ImVec2 corner = ImLerp(window->Pos,
                                                 window->Pos + window->Size,
                                                 grip.CornerPos);
            ...
            {
                ImRect border =
                    GetBorderRect(window, border_held, grip_draw_size, 0.0f);
                window->DrawList->AddLine(border.Min,
                                          border.Max,
            ...
            if (style.FrameBorderSize > 0 &&
                ...
                window->DrawList->AddLine(
                    title_bar_rect.GetBL() + ImVec2(style.WindowBorderSize, -1),
        ...
        // Update ContentsRegionMax. All the variable it depends on are set
        ...
        window->MenuColumns.Update(
            3, style.ItemSpacing.x, window_just_activated_by_user);
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            {
                RenderTriangle(window->Pos + style.FramePadding,
                               window->Collapsed ? ImGuiDir_Right
                                                 : ImGuiDir_Down,
                               1.0f);
            }
            ...
            if (p_open != NULL)
            {
                ...
                const float rad =
                    (window->TitleBarHeight() - PAD * 2.0f) * 0.5f;
            ...
            clip_rect.Max.x =
                ...
                (p_open
                     ? title_bar_rect.GetHeight() - 3
                     : style.FramePadding.x); // Match the size of CloseButton()
            RenderTextClipped(text_r.Min,
                              text_r.Max,
                              name,
                              NULL,
                              &text_size,
        ...
        */
        ...
        // we will end up with a null clipping rectangle which is the correct
        // behavior.
        window->InnerRect.Min.x =
            title_bar_rect.Min.x + window->WindowBorderSize;
        window->InnerRect.Min.y = title_bar_rect.Max.y +
        ...
        // After Begin() we fill the last item / hovered data using the title
        // bar data. Make that a standard behavior (to allow usage of context
        // menus on title bar only, etc.).
        window->DC.LastItemId   = window->MoveId;
        window->DC.LastItemRect = title_bar_rect;
        window->DC.LastItemRectHoveredRect =
    ...
    ImRect      clip_rect;
    clip_rect.Min.x = ImFloor(
        0.5f + window->InnerRect.Min.x +
        ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - border_size)));
    clip_rect.Min.y = ImFloor(0.5f + window->InnerRect.Min.y);
    clip_rect.Max.x = ImFloor(
    ...
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
        ...
        if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) &&
            window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
            window->Collapsed |= (window->WindowRectClipped.Min.x >=
                                      window->WindowRectClipped.Max.x ||
                                  window->WindowRectClipped.Min.y >=
                                      window->WindowRectClipped.Max.y);

        // We also hide the window from rendering because we've already added
        // its border to the command list. (we could perform the check earlier
                  ...
                  ImGuiWindowFlags flags)
{
    ...
    // however this was very misleading because in most cases it would only
    // affect the window when it didn't have storage in the .ini file.
    if (size_on_first_use.x != 0.0f || size_on_first_use.y != 0.0f)
        SetNextWindowSize(size_on_first_use, ImGuiCond_FirstUseEver);

    // Old API feature: we could override the window background alpha with a
    // parameter. This is actually tricky to reproduce manually because: (1)
    // there are multiple variants of WindowBg (popup, tooltip, etc.) and (2)
    // you can't call PushStyleColor before Begin and PopStyleColor just after
    // Begin() because of how CheckStackSizes() behave. The user-side solution
    // is to do backup = GetStyleColorVec4(ImGuiCol_xxxBG),
    // PushStyleColor(ImGuiCol_xxxBg), Begin, PushStyleColor(ImGuiCol_xxxBg,
    // backup), [...], PopStyleColor(), End(); PopStyleColor() - which is super
    // awkward. The alpha override was rarely used but for now we'll leave the
    // Begin() variant around for a bit. We may either lift the constraint on
    // CheckStackSizes() either add a SetNextWindowBgAlpha() helper that does it
    // magically.
    ImGuiContext&  g               = *GImGui;
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    bb.Expand(ImVec2(
        ...
        -ImClamp(
            (float)(int)((bb.Max.y - bb.Min.y - 2.0f) * 0.5f), 0.0f, 3.0f)));
    ...
    if (horizontal)
        ...
        grab_rect = ImRect(
            ...
            ImMin(ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels,
                  window_rect.Max.y));
    window->DrawList->AddRectFilled(
        grab_rect.Min, grab_rect.Max, grab_col, style.ScrollbarRounding);
}
...
void ImGui::BringWindowToBack(ImGuiWindow* window)
{
    ...
    for (int i = 0; i < g.Windows.Size; i++)
        ...
        {
            memmove(
                &g.Windows[1], &g.Windows[0], (size_t)i * sizeof(ImGuiWindow*));
            g.Windows[0] = window;
            break;
        }
}
...
void ImGui::FocusWindow(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_Popup) // FIXME: This statement should
                                                // be unnecessary. Need further
...
void ImGui::PopStyleVar(int count)
{
    ...
    while (count > 0)
    {
        ...
        if (info->Type == ImGuiDataType_Float)
            (*(float*)info->GetVarPtr(&g.Style)) = backup.BackupFloat[0];
        else if (info->Type == ImGuiDataType_Float2)
            (*(ImVec2*)info->GetVarPtr(&g.Style)) =
                ImVec2(backup.BackupFloat[0], backup.BackupFloat[1]);
        else if (info->Type == ImGuiDataType_Int)
            (*(int*)info->GetVarPtr(&g.Style)) = backup.BackupInt[0];
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond)
{
    ...
    window->DC.CursorPos +=
        (window->Pos - old_pos); // As we happen to move the window while it is
                                 // being appended to (which is a bad idea -
                                 // will smear) let's at least offset the cursor
    window->DC.CursorMaxPos +=
        (window->Pos - old_pos); // And more importantly we need to adjust this
                                 // so size calculation doesn't get affected.
}
...
void ImGui::SetScrollY(float scroll_y)
{
    ...
    window->ScrollTarget.y = scroll_y + window->TitleBarHeight() +
                             window->MenuBarHeight(); // title bar height
                                                      // canceled out when using
                                                      // ScrollTargetRelY
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio)
{
    ...
    // Minor hack to to make scrolling to top/bottom of window take account of
    // WindowPadding, it looks more right to the user this way
    if (center_y_ratio <= 0.0f &&
        window->ScrollTarget.y <= window->WindowPadding.y)
        window->ScrollTarget.y = 0.0f;
    else if (center_y_ratio >= 1.0f &&
...
// FIXME-NAV: This function is a placeholder for the upcoming Navigation branch
// + Focusing features. In the current branch this function will only set the
// scrolling, in the navigation branch it will also set your navigation cursor.
// Prefer using "SetItemDefaultFocus()" over "if (IsWindowAppearing())
// SetScrollHere()" when applicable.
...
void ImGui::AlignTextToFramePadding()
{
    ...
    ImGuiContext& g              = *GImGui;
    window->DC.CurrentLineHeight = ImMax(
        window->DC.CurrentLineHeight, g.FontSize + g.Style.FramePadding.y * 2);
    window->DC.CurrentLineTextBaseOffset =
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    const char* value_text_end =
        value_text_begin +
        ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    RenderTextClipped(value_bb.Min,
                      value_bb.Max,
                      value_text_begin,
                      value_text_end,
                      NULL,
                           ...
                           ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_PressedOnDragDropHold) && g.DragDropActive &&
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
        style.FramePadding.y <
            window->DC.CurrentLineTextBaseOffset) // Try to vertically align
                                                  // buttons that are
                                                  // smaller/have no padding so
                                                  // that text baseline matches
                                                  // (bit hacky, since it
                                                  // shouldn't be a flag)
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
    ...
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_CloseButtonActive
...
// [Internal]
bool ImGui::ArrowButton(ImGuiID          id,
                        ImGuiDir         dir,
                        ImVec2           padding,
                        ImGuiButtonFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos,
                    window->DC.CursorPos +
                        ImVec2(g.FontSize + padding.x * 2.0f,
                               g.FontSize + padding.y * 2.0f));
    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
                        ...
                        const ImVec4& tint_col)
{
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float)frame_padding,
                                                         (float)frame_padding)
                                                : style.FramePadding;
    const ImRect bb(window->DC.CursorPos,
    ...
    if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(
            image_bb.Min, image_bb.Max, GetColorU32(bg_col));
    window->DrawList->AddImage(user_texture_id,
                               image_bb.Min,
                               image_bb.Max,
                               uv0,
                             ...
                             const char*        label_end)
{
    ...
    const ImRect interact_bb =
        display_frame ? bb
                      : ImRect(bb.Min.x,
                               bb.Min.y,
                               bb.Min.x + text_width + style.ItemSpacing.x * 2,
                               bb.Max.y);
    ...
    if (pressed && !(flags & ImGuiTreeNodeFlags_Leaf))
    {
        ...
        if (g.DragDropActive && is_open) // When using Drag and Drop "hold to
                                         // open" we keep the node highlighted
                                         // after opening, but never close it
                                         // again.
            toggled = false;
        if (toggled)
        {
    ...
    if (display_frame)
    {
        // Framed type
        RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
        RenderTriangle(bb.Min + ImVec2(padding.x, text_base_offset_y),
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderTriangle(
                bb.Min +
                    ImVec2(padding.x, g.FontSize * 0.15f + text_base_offset_y),
                is_open ? ImGuiDir_Down : ImGuiDir_Right,
                0.70f);
        if (g.LogEnabled)
                        ...
                        va_list            args)
{
    ...
    return TreeNodeBehavior(
        window->GetID(str_id), flags, g.TempBuffer, label_end);
}
                        ...
                        va_list            args)
{
    ...
    const char*   label_end =
        ...
        ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    return TreeNodeBehavior(
        window->GetID(ptr_id), flags, g.TempBuffer, label_end);
}
                                        ...
                                        int           buf_size)
{
    if (data_type == ImGuiDataType_Int)
    {
        if (decimal_precision < 0)
            ...
            ImFormatString(
                buf, buf_size, "%.*d", decimal_precision, *(int*)data_ptr);
    }
    else if (data_type == ImGuiDataType_Float)
    {
        if (decimal_precision < 0)
            ImFormatString(buf,
                           buf_size,
                           "%f",
                           *(float*)data_ptr); // Ideally we'd have a minimum
                                               // decimal precision of 1 to
                                               // visually denote that it is a
                                               // float, while hiding
                                               // non-significant digits?
        else
            ImFormatString(
                buf, buf_size, "%.*f", decimal_precision, *(float*)data_ptr);
    }
}
                                    ...
                                    const char*   scalar_format)
{
    ...
    if (data_type == ImGuiDataType_Int)
    {
        ...
        {
            if (sscanf(buf, "%f", &arg1f) == 1 && arg1f != 0.0f)
                *v = (int)(arg0i / arg1f);
        } // Divide
        else
        {
            if (sscanf(buf, scalar_format, &arg0i) == 1)
                *v = arg0i;
                                           ...
                                           int           decimal_precision)
{
    ...
    DataTypeFormatString(
        data_type, data_ptr, decimal_precision, buf, IM_ARRAYSIZE(buf));
    ...
    {
        IM_ASSERT(g.ActiveId == id); // InputText ID expected to match the
                                     // Slider ID (else we'd need to store them
                                     // both, which is also possible)
...
{
    static const float min_steps[10] = { 1.0f,        0.1f,       0.01f,
    ...
    float v, float v_min, float v_max, float power, float linear_zero_pos)
{
    ...
    if (is_non_linear)
    {
        ...
        {
            const float f =
                1.0f - (v_clamped - v_min) / (ImMin(0.0f, v_max) - v_min);
        ...
        else
        {
            ...
            return linear_zero_pos +
                   powf(f, 1.0f / power) * (1.0f - linear_zero_pos);
        }
    }
                           ...
                           ImGuiSliderFlags flags)
{
    ...
    if (decimal_precision != 0)
        ...
        grab_sz =
            ImMin(ImMax(1.0f * (slider_sz / ((v_min < v_max ? v_max - v_min
                                                            : v_min - v_max) +
                                             1.0f)),
                        style.GrabMinSize),
    ...
    if (v_min * v_max < 0.0f)
    {
        ...
        const float linear_dist_min_to_0 =
            powf(fabsf(0.0f - v_min), 1.0f / power);
        const float linear_dist_max_to_0 =
            powf(fabsf(v_max - 0.0f), 1.0f / power);
    ...
    // Draw
    float grab_t = SliderBehaviorCalcRatioFromValue(
                        ...
                        float       power)
{
    ...
    if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
        return InputScalarAsWidgetReplacement(
            frame_bb, label, ImGuiDataType_Float, v, id, decimal_precision);
                         ...
                         float         power)
{
    ...
    // Actual slider behavior + render grab
    bool value_changed = SliderBehavior(frame_bb,
                                        id,
                                        v,
                                        v_min,
                                        v_max,
                        ...
                        float       v_degrees_max)
{
    ...
    bool  value_changed = SliderFloat(
        label, &v_deg, v_degrees_min, v_degrees_max, "%.0f deg", 1.0f);
    *v_rad = v_deg * (2 * IM_PI) / 360.0f;
    return value_changed;
}

bool ImGui::SliderInt(
    const char* label, int* v, int v_min, int v_max, const char* display_format)
{
    if (!display_format)
        display_format = "%.0f";
    float v_f           = (float)*v;
    bool  value_changed = SliderFloat(
        label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
    *v = (int)v_f;
    return value_changed;
}
                       ...
                       const char*   display_format)
{
    if (!display_format)
        display_format = "%.0f";
    float v_f           = (float)*v;
    bool  value_changed = VSliderFloat(
        label, size, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
    *v = (int)v_f;
    return value_changed;
}
                         ...
                         float         power)
{
    ...
    const ImU32 frame_col =
        GetColorU32(g.ActiveId == id    ? ImGuiCol_FrameBgActive
                    : g.HoveredId == id ? ImGuiCol_FrameBgHovered
                                        : ImGuiCol_FrameBg);
    RenderFrame(
        frame_bb.Min, frame_bb.Max, frame_col, true, style.FrameRounding);
    ...
    if (g.ActiveId == id)
    {
        if (g.IO.MouseDown[0])
        {
            ...
            if (v_speed == 0.0f && (v_max - v_min) != 0.0f &&
                (v_max - v_min) < FLT_MAX)
            ...
            if (fabsf(adjust_delta) > 0.0f)
            {
                if (fabsf(power - 1.0f) > 0.001f)
                {
                    ...
                    float v1 =
                        powf(v0_abs, 1.0f / power) + (adjust_delta * v0_sign);
                    float v1_abs = v1 >= 0.0f ? v1 : -v1;
                    float v1_sign =
                        v1 >= 0.0f ? 1.0f : -1.0f; // Crossed sign line
                    v_cur =
                        powf(v1_abs, power) * v0_sign * v1_sign; // Reapply sign
                }
                else
                {
                    v_cur += adjust_delta;
                      ...
                      float       power)
{
    ...
    const bool value_changed = DragBehavior(
        frame_bb, id, v, v_speed, v_min, v_max, decimal_precision, power);
...
{
    return DragFloatN(
        label, v, 2, v_speed, v_min, v_max, display_format, power);
}
...
{
    return DragFloatN(
        label, v, 3, v_speed, v_min, v_max, display_format, power);
}
...
{
    return DragFloatN(
        label, v, 4, v_speed, v_min, v_max, display_format, power);
}
                    ...
                    const char* display_format)
{
    ...
    float v_f           = (float)*v;
    bool  value_changed = DragFloat(
        label, &v_f, v_speed, (float)v_min, (float)v_max, display_format);
    *v = (int)v_f;
    return value_changed;
}
    ...
    if (values_count > 0)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            const ImVec2 tp1 = ImVec2(
                t1,
                1.0f - ImSaturate((v1 - scale_min) / (scale_max - scale_min)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y),
                         window->DC.CursorPos +
                             ImVec2(0, style.FramePadding.y) + label_size);
    if (label_size.x > 0)
    {
                                     ...
                                     bool            stop_on_new_line)
{
    ...
    if (out_offset)
        *out_offset =
            ImVec2(line_width, text_size.y + line_height); // offset allow for
                                                           // the possibility of
                                                           // sitting after a
                                                           // trailing \n
                                      ...
                                      int                  line_start_idx)
{
    ...
    const ImVec2   size = InputTextCalcTextSizeW(text + line_start_idx,
                                               text + obj->CurLenW,
                                               &text_remaining,
                                               NULL,
                                               true);
...
void ImGuiTextEditCallbackData::InsertChars(int         pos,
                                            const char* new_text,
                                            const char* new_text_end)
{
    ...
    if (BufTextLen != pos)
        memmove(
            Buf + pos + new_text_len, Buf + pos, (size_t)(BufTextLen - pos));
...
// FIXME: Rather messy function partly because we are doing UTF8 > u16 > UTF8
// conversions on the go to more easily handle stb_textedit calls. Ideally we
// should stay in UTF-8 all the time. See
                        ...
                        void*                 user_data)
{
    ...
    if (focus_requested || user_clicked || user_scrolled)
    {
        if (g.ActiveId != id)
        {
            ...
            edit_state.Text.resize(buf_size + 1); // wchar count <= UTF-8 count.
                                                  // we use +1 to make sure that
                                                  // .Data isn't NULL so it
                                                  // doesn't crash.
            edit_state.InitialText.resize(buf_size + 1); // UTF-8. we use +1 to
                                                         // make sure that .Data
                                                         // isn't NULL so it
                                                         // doesn't crash.
            ImStrncpy(
                edit_state.InitialText.Data, buf, edit_state.InitialText.Size);
            const char* buf_end = NULL;
            edit_state.CurLenW  = ImTextStrFromUtf8(edit_state.Text.Data,
                                                   edit_state.Text.Size,
                                                   buf,
                                                   NULL,
                                                   &buf_end);
            edit_state.CurLenA =
                (int)(buf_end - buf); // We can't get the result from
                                      // ImFormatString() above because it is
                                      // not UTF-8 aware. Here we'll cut off
                                      // malformed UTF-8.
    ...
    if (g.ActiveId == id)
    {
        ...
        const bool osx_double_click_selects_words =
            io.OptMacOSXBehaviors; // OS X style: Double click selects by word
                                   // instead of selecting whole text
        ...
        {
            stb_textedit_click(
                &edit_state, &edit_state.StbState, mouse_x, mouse_y);
            edit_state.CursorAnimReset();
        }
        ...
        if (io.InputCharacters[0])
        {
            ...
            // need to allow CTRL+ALT as some keyboards (e.g. German) use AltGR
            // - which is Alt+Ctrl - to input certain characters.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_shortcut_key_only =
            (io.OptMacOSXBehaviors ? (io.KeySuper && !io.KeyCtrl)
                                   : (io.KeyCtrl && !io.KeySuper)) &&
            !io.KeyAlt && !io.KeyShift; // OS X style: Shortcuts using Cmd/Super
                                        // instead of Ctrl
        const bool is_wordmove_key_down =
            io.OptMacOSXBehaviors ? io.KeyAlt
                                  : io.KeyCtrl; // OS X style: Text editing
        ...
        else if (IsKeyPressedMap(ImGuiKey_Backspace) && is_editable)
        {
            if (!edit_state.HasSelection())
            {
                if (is_wordmove_key_down)
                    edit_state.OnKeyPressed(STB_TEXTEDIT_K_WORDLEFT |
                                            STB_TEXTEDIT_K_SHIFT);
                else if (io.OptMacOSXBehaviors && io.KeySuper && !io.KeyAlt &&
                         !io.KeyCtrl)
        ...
        else if (is_paste)
        {
            ...
            if (const char* clipboard = GetClipboardText())
            {
                ...
                for (const char* s = clipboard; *s;)
                {
                    ...
                    if (c == 0)
                        break;
                    if (c >= 0x10000 || !InputTextFilterCharacter(
                                            &c, flags, callback, user_data))
                        continue;
                ...
                if (event_flag)
                {
                    ...
                    if (callback_data.BufDirty)
                    {
                        IM_ASSERT(callback_data.BufTextLen ==
                                  (int)strlen(callback_data.Buf)); // You need
                                                                   // to
                                                                   // maintain
                                                                   // BufTextLen
                                                                   // if you
                                                                   ...
                                                                   // text!
                        edit_state.CurLenW =
                            ImTextStrFromUtf8(edit_state.Text.Data,
                                              edit_state.Text.Size,
                                              callback_data.Buf,
                                              NULL);
                        edit_state.CurLenA =
                            callback_data.BufTextLen; // Assume correct length
                                                      // and valid UTF-8 from
                                                      // user, saves us an extra
                                                      // strlen()
                        edit_state.CursorAnimReset();
                    }
                }
    ...
    const char* buf_display = (g.ActiveId == id && is_editable)
                                  ? edit_state.TempTextBuffer.Data
                                  : buf;
    buf                     = NULL;
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        if (edit_state.CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < edit_state.ScrollX)
                    ...
                    edit_state.ScrollX = (float)(int)(cursor_offset.x - size.x +
                                                      scroll_increment_x);
            }
            else
            {
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            float bg_offy_up =
                is_multiline ? 0.0f : -1.0f; // FIXME: those offsets should be
                                             // part of the style? they don't
                                             // play so well with multi-line
                                             // selection.
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f)
                        rect_size.x = (float)(int)(g.Font->GetCharAdvance(
                                                       (unsigned short)' ') *
                                                   0.50f); // So we can see
                                                           // selected empty
                                                           // lines
        ...
        bool cursor_is_visible =
            (!g.IO.OptCursorBlink) || (g.InputTextState.CursorAnim <= 0.0f) ||
        ...
        if (is_editable)
            g.OsImePosRequest = ImVec2(cursor_screen_pos.x - 1,
    ...
    else
    {
        ...
        if (is_multiline)
            text_size =
                ImVec2(size.x,
                       InputTextCalcTextLenAndLineCount(buf_display, &buf_end) *
                      ...
                      void*                 user_data)
{
    ...
    return InputTextEx(
        label, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
                          ...
                          ImGuiInputTextFlags extra_flags)
{
    ...
    if (step_ptr)
        PushItemWidth(ImMax(1.0f,
                            CalcItemWidth() -
                                (button_sz.x + style.ItemInnerSpacing.x) * 2));

    char buf[64];
    DataTypeFormatString(
        data_type, data_ptr, scalar_format, buf, IM_ARRAYSIZE(buf));
    ...
    if (step_ptr)
    {
                         ...
                         ImGuiButtonFlags_DontClosePopups))
        {
            DataTypeApplyOp(data_type,
                            '-',
                            data_ptr,
                            g.IO.KeyCtrl && step_fast_ptr ? step_fast_ptr
                                                          : step_ptr);
            value_changed = true;
        }
        SameLine(0, style.ItemInnerSpacing.x);
                       ...
                       ImGuiInputTextFlags extra_flags)
{
    ...
    if (decimal_precision < 0)
        strcpy(display_format, "%f"); // Ideally we'd have a minimum decimal
                                      // precision of 1 to visually denote that
                                      // this is a float, while hiding
                                      // non-significant digits? %f doesn't have
                                      // a minimum of 1
    else
        ImFormatString(display_format,
                       IM_ARRAYSIZE(display_format),
                       "%%.%df",
                       decimal_precision);
                       ...
                       ImGuiComboFlags flags)
{
    ...
    RenderTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y,
                          frame_bb.Min.y + style.FramePadding.y),
                   ImGuiDir_Down);
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImVec2 size_expected = CalcSizeAfterConstraint(
                popup_window, CalcSizeAutoFit(popup_window, size_contents));
            if (flags & ImGuiComboFlags_PopupAlignLeft)
                popup_window->AutoPosLastDirection = ImGuiDir_Left;
            ImVec2 pos =
                FindBestWindowPosForPopup(frame_bb.GetBL(),
                                          size_expected,
                  ...
                  int   popup_max_height_in_items)
{
    ...
    // The old Combo() API exposed "popup_max_height_in_items", however the new
    // more general BeginCombo() API doesn't, so we emulate it here.
    if (popup_max_height_in_items != -1 && !g.NextWindowData.SizeConstraintCond)
    {
        float popup_max_height =
            CalcMaxPopupHeightFromItemCount(popup_max_height_in_items);
        SetNextWindowSizeConstraints(ImVec2(0, 0),
                                     ImVec2(FLT_MAX, popup_max_height));
    }
...
{
    int         items_count = 0;
    const char* p = items_separated_by_zeros; // FIXME-OPT: Avoid computing
                                              // this, or at least only when
                                              // combo is open
    while (*p)
    {
        p += strlen(p) + 1;
                       ...
                       const ImVec2&        size_arg)
{
    ...
    float  w_draw         = ImMax(label_size.x,
                         window->Pos.x + max_x - window_padding.x -
...
{
    // Size default to hold ~7 items. Fractional number of items helps seeing
    // that we can scroll down/up without looking at scrollbar. However we don't
    // add +0.40f if items_count <= height_in_items. It is slightly dodgy,
    // because it means a dynamic list of items will make the widget resize
    // occasionally when it crosses that size. I am expecting that someone will
    // come and complain about this behavior in a remote future, then we can
                    ...
                    int   height_in_items)
{
    ...
    ImGuiListClipper clipper(
        items_count, GetTextLineHeightWithSpacing()); // We know exactly our
                                                      // line height here so we
                                                      // pass it as a minor
                                                      // optimization, but
                                                      // generally you don't
                                                      // need to.
                     ...
                     bool        enabled)
{
    ...
    ImGuiSelectableFlags flags = ImGuiSelectableFlags_MenuItem |
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x +=
            (float)(int)(style.ItemSpacing.x *
                         (-1.0f + 0.5f)); // -1 spacing to compensate the
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with regular window clipping rectangle as it is already set
    // to the area below. However we clip with window full rect. We remove 1
    // worth of rounding to Max.x to that text in long menus don't tend to
    // display over the lower-right rounded area, which looks particularly
    // glitchy.
    ImRect bar_rect = window->MenuBarRect();
    ImRect clip_rect(
        ImFloor(bar_rect.Min.x + 0.5f),
        ImFloor(bar_rect.Min.y + window->WindowBorderSize + 0.5f),
        ImFloor(ImMax(bar_rect.Min.x, bar_rect.Max.x - window->WindowRounding) +
                0.5f),
        ImFloor(bar_rect.Max.y + 0.5f));
    clip_rect.ClipWith(window->WindowRectClipped);
    PushClipRect(clip_rect.Min, clip_rect.Max, false);

    window->DC.CursorPos  = ImVec2(bar_rect.Min.x + window->DC.MenuBarOffsetX,
                                  bar_rect.Min.y); // + g.Style.FramePadding.y);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (menuset_is_open)
        g.NavWindow = window; // Odd hack to allow hovering across menus of a
                              // same menu-set (otherwise we wouldn't be able to
                              // hover parent)
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // the call to FindBestPopupWindowPos() in Begin()
        popup_pos =
    ...
    else
    {
        ...
        if (!enabled)
            PushStyleColor(ImGuiCol_Text,
                           g.Style.Colors[ImGuiCol_TextDisabled]);
        RenderTriangle(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w +
                                        g.FontSize * 0.30f,
                                    0.0f),
                       ImGuiDir_Right);
        if (!enabled)
    ...
    if (window->DC.LayoutType !=
        ImGuiLayoutType_Horizontal) // (window->Flags &
                                    // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
            ...
            g.OpenPopupStack[g.CurrentPopupStack.Size].ParentWindow == window)
        {
                    ...
                    g.OpenPopupStack[g.CurrentPopupStack.Size].Window)
            {
                ...
                float  extra            = ImClamp(fabsf(ta.x - tb.x) * 0.30f,
                                      5.0f,
                ...
                tb.y = ta.y + ImMax((tb.y - extra) - ta.y,
                                    -100.0f); // triangle is maximum 200 high to
                                              // limit the slope and the bias
                                              // toward large sub-menus //
        ...
        ImGuiWindowFlags flags =
            ...
            ((window->Flags &
              (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu))
                 ...
                 : ImGuiWindowFlags_ChildMenu);
        menu_is_open = BeginPopupEx(id, flags); // menu_is_open can be 'false'
                                                 ...
                                                 int    rounding_corners_flags)
{
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
    {
        ...
        window->DrawList->AddRectFilled(
            p_min, p_max, col_bg1, rounding, rounding_corners_flags);
    ...
    {
        window->DrawList->AddRectFilled(
            p_min, p_max, col, rounding, rounding_corners_flags);
    }
}
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & ImGuiColorEditFlags__InputsMask))); // Check only 1 option
                                                          // is selected
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & ImGuiColorEditFlags__DataTypeMask))); // Check only 1
                                                            // option is
                                                            // selected
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & ImGuiColorEditFlags__PickerMask))); // Check only 1 option
                                                          // is selected
                        ...
                        ImVec2              size)
{
    ...
    {
        float mid_x =
            (float)(int)((bb_inner.Min.x + bb_inner.Max.x) * 0.5f + 0.5f);
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(bb_inner.Min,
                                                 bb_inner.Max,
    ...
    if (g.ActiveId == id && BeginDragDropSource()) // NB: The ActiveId test is
                                                   // merely an optional
                                                   // micro-optimization
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_inputs)
    {
        if (RadioButton("RGB", (opts & ImGuiColorEditFlags_RGB) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__InputsMask) |
                   ImGuiColorEditFlags_RGB;
        if (RadioButton("HSV", (opts & ImGuiColorEditFlags_HSV) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__InputsMask) |
                   ImGuiColorEditFlags_HSV;
        if (RadioButton("HEX", (opts & ImGuiColorEditFlags_HEX) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__InputsMask) |
    ...
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs)
            Separator();
        if (RadioButton("0..255", (opts & ImGuiColorEditFlags_Uint8) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) |
                   ImGuiColorEditFlags_Uint8;
        if (RadioButton("0.00..1.00",
                        (opts & ImGuiColorEditFlags_Float) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) |
    ...
    if (BeginPopup("Copy"))
    {
        ...
        if (flags & ImGuiColorEditFlags_NoAlpha)
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), "0x%02X%02X%02X", cr, cg, cb);
        else
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), "0x%02X%02X%02X%02X", cr, cg, cb, ca);
        if (Selectable(buf))
            SetClipboardText(buf);
        EndPopup();
    }
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags,
                                    const float*        ref_col)
{
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            memcpy(&dummy_ref_col.x,
                   ref_col,
                   sizeof(float) *
                       (picker_flags & ImGuiColorEditFlags_NoAlpha ? 3 : 4));
            ImGui::ColorPicker4(
                "##dummypicker", &dummy_ref_col.x, picker_flags);
            ImGui::PopID();
        }
        ImGui::PopItemWidth();
    }
    if (allow_opt_alpha_bar)
    {
        if (allow_opt_picker)
            ImGui::Separator();
        ImGui::CheckboxFlags("Alpha Bar",
                             (unsigned int*)&g.ColorEditOptions,
                       ...
                       ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_last = ImMax(
            1.0f,
            (float)(int)(w_items_all - (w_item_one + style.ItemInnerSpacing.x) *
                                           (components - 1)));
    ...
    if (window->DC.LastItemRectHoveredRect &&
        BeginDragDropTarget()) // NB: The LastItemRectHoveredRect test is merely
                               // an optional micro-optimization
    {
...
// 'pos' is position of the arrow tip. half_sz.x is length from base to tip.
// half_sz.y is length on each side.
static void RenderArrow(ImDrawList* draw_list,
                        ImVec2      pos,
                        ImVec2      half_sz,
                        ImGuiDir    direction,
                        ImU32       col)
                         ...
                         const float*        ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags__PickerMask))
        flags |= ((g.ColorEditOptions & ImGuiColorEditFlags__PickerMask)
                 ...
                 ImGuiColorEditFlags__PickerMask;
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & ImGuiColorEditFlags__PickerMask))); // Check that only 1
                                                          // is selected
    if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_RGB ||
            ...
            value_changed |=
                ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_RGB);
        if (flags & ImGuiColorEditFlags_HSV ||
            (flags & ImGuiColorEditFlags__InputsMask) == 0)
            value_changed |=
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            ImVec2 gradient_p0(wheel_center.x + cosf(a0) * wheel_r_inner,
                               wheel_center.y + sinf(a0) * wheel_r_inner);
            ImVec2 gradient_p1(wheel_center.x + cosf(a1) * wheel_r_inner,
                               wheel_center.y + sinf(a1) * wheel_r_inner);
            ShadeVertsLinearColorGradientKeepAlpha(
                draw_list->VtxBuffer.Data + vert_start_idx,
                draw_list->VtxBuffer.Data + vert_end_idx,
                gradient_p0,
                gradient_p1,
                hue_colors[n],
                hue_colors[n + 1]);
        ...
        draw_list->AddCircleFilled(
            hue_cursor_pos, hue_cursor_rad, hue_color32, hue_cursor_segments);
        ...
        draw_list->AddTriangle(
            tra, trb, trc, IM_COL32(128, 128, 128, 255), 1.5f);
        sv_cursor_pos =
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        sv_cursor_pos.y =
            ImClamp((float)(int)(picker_pos.y +
    ...
    draw_list->AddCircleFilled(
        sv_cursor_pos, sv_cursor_rad, col32_no_alpha, 12);
    draw_list->AddCircle(
        sv_cursor_pos, sv_cursor_rad + 1, IM_COL32(128, 128, 128, 255), 12);
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(bar1_bb.Min,
                                             bar1_bb.Max,
                                             IM_COL32(0, 0, 0, 0),
        ...
        draw_list->AddRectFilledMultiColor(bar1_bb.Min,
                                           ...
                                           col32_no_alpha & ~IM_COL32_A_MASK);
        float bar1_line_y =
            (float)(int)(picker_pos.y + (1.0f - alpha) * sv_picker_size + 0.5f);
    ...
    return value_changed &&
           memcmp(backup_initial_col, col, components * sizeof(float));
}
...
void ImGui::Separator()
{
    ...
    if ((flags &
         (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical)) == 0)
        flags |= (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
                     ? ImGuiSeparatorFlags_Vertical
                     : ImGuiSeparatorFlags_Horizontal;
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & (ImGuiSeparatorFlags_Horizontal |
    ...
    ItemSize(ImVec2(0.0f, 0.0f)); // NB: we don't provide our width so that it
                                  // doesn't get feed back into AutoFit, we
    ...
    window->DrawList->AddLine(
        bb.Min, ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Separator));
                             ...
                             float         hover_extend)
{
    ...
    const ImU32 col = GetColorU32(held      ? ImGuiCol_SeparatorActive
                                  : hovered ? ImGuiCol_SeparatorHovered
                                            : ImGuiCol_Separator);
    window->DrawList->AddRectFilled(
        bb_render.Min, bb_render.Max, col, g.Style.FrameRounding);
...
void ImGui::EndGroup()
{
    ...
    if (group_data.AdvanceCursor)
    {
        window->DC.CurrentLineTextBaseOffset = ImMax(
            ...
            group_data.BackupCurrentLineTextBaseOffset); // FIXME: Incorrect, we
                                                         // should grab the base
                                                         // offset from the
                                                         // *first line* of the
                                                         // group but it is hard
                                                         // to obtain now.
    ...
    // copy it to LastItemId so IsItemActive() will be functional on the entire
    // group. It would be be neater if we replaced window.DC.LastItemId by e.g.
    // 'bool LastItemIsActive', but if you search for LastItemId you'll notice
    // it is only used in that context.
...
void ImGui::NewLine()
{
    ...
    window->DC.LayoutType                    = ImGuiLayoutType_Vertical;
    if (window->DC.CurrentLineHeight > 0.0f) // In the event that we are on a
                                             // line with items that is smaller
                                             // that FontSize high, we will
                                             // preserve its height.
        ItemSize(ImVec2(0, 0));
    else
...
void ImGui::NextColumn()
{
    ...
    if (++columns->Current < columns->Count)
    {
        ...
        window->DC.ColumnsOffsetX = GetColumnOffset(columns->Current) -
...
static float GetDraggedColumnOffset(ImGuiColumnsSet* columns, int column_index)
{
    ...
    IM_ASSERT(column_index > 0); // We cannot drag column 0. If you get this
                                 // assert you may have a conflict between the
                                 // ID of your columns and another widgets.
...
float ImGui::GetColumnWidth(int column_index)
{
    ...
    if (column_index < 0)
        column_index = columns->Current;
    return OffsetNormToPixels(columns,
                         ...
                         ImGuiColumnsFlags flags)
{
    ...
    const float content_region_width =
        (window->SizeContentsExplicit.x != 0.0f)
            ...
            : (window->Size.x - window->ScrollbarSizes.x);
    columns->MinX =
        window->DC.IndentX - g.Style.ItemSpacing.x; // Lock our horizontal range
    // column->MaxX = content_region_width - window->Scroll.x - ((window->Flags
    // & ImGuiWindowFlags_NoScrollbar) ? 0 : g.Style.ScrollbarSize);// -
    // window->WindowPadding().x;
...
void ImGui::EndColumns()
{
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_GrowParentContentsSize))
        window->DC.CursorMaxPos.x = ImMax(
            columns->StartMaxPosX,
            columns
                ->MaxX); // Restore cursor max pos, as columns don't grow parent
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            if (!(columns->Flags & ImGuiColumnsFlags_NoResize))
            {
                ...
                if (held &&
                    !(columns->Columns[n].Flags & ImGuiColumnsFlags_NoResize))
                    dragging_column = n;
            }
            ...
            window->DrawList->AddLine(
                ...
                ImVec2(xi, ImMin(y2, window->ClipRect.Max.y)),
                col);
        }
...
// [2017/12: This is currently the only public API, while we are working on
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            if (g.ActiveId == source_id) // Allow the underlying widget to
                                         // display/return hovered during the
                                         // mouse release frame, else we would
                                         // get a flicker.
    ...
    if (source_drag_active)
    {
        ...
        if (!(flags & ImGuiDragDropFlags_SourceNoPreviewTooltip))
        {
            ...
            // g.Style.Alpha * 0.60f); // This is better but e.g ColorButton
            // with checkboard has issue with transparent colors :(
            SetNextWindowPos(g.IO.MousePos);
            PushStyleColor(ImGuiCol_PopupBg,
                           GetStyleColorVec4(ImGuiCol_PopupBg) *
                               ...
                               ImGuiCond   cond)
{
    ...
    IM_ASSERT(type != NULL);
    IM_ASSERT(strlen(type) < IM_ARRAYSIZE(payload.DataType)); // Payload type
                                                              // can be at most
                                                              // 8 characters
                                                              // longs
                                                 ...
                                                 ImGuiDragDropFlags flags)
{
        ...
        payload.Preview)
    {
        ...
        if (push_clip_rect)
            window->DrawList->PushClipRectFullScreen();
        window->DrawList->AddRect(
            r.Min, r.Max, GetColorU32(ImGuiCol_DragDropTarget), 0.0f, ~0, 2.0f);
        if (push_clip_rect)
            window->DrawList->PopClipRect();
    }
    ...
    payload.Delivery =
        ...
        !IsMouseDown(g.DragDropMouseButton); // For extern drag sources
                                             // affecting os window focus, it's
                                             // easier to just test
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        ImGui::Checkbox("Show clipping rectangles when hovering an ImDrawCmd",
                        &show_clip_rects);
        ImGui::Separator();
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                ImDrawList* overlay_draw_list =
                    &GImGui->OverlayDrawList; // Render additional visuals into
                                              // the top-most draw list
                int elem_offset = 0;
                     ...
                     elem_offset += pcmd->ElemCount, pcmd++)
                {
                        ...
                        for (int i = elem_offset;
                            ...
                            vtxs_rect.Add(
                                draw_list
                                    ...
                                    .pos);
                        clip_rect.Floor();
                        overlay_draw_list->AddRect(clip_rect.Min,
                                                   clip_rect.Max,
                                                   IM_COL32(255, 255, 0, 255));
                        vtxs_rect.Floor();
                        overlay_draw_list->AddRect(vtxs_rect.Min,
                                                   vtxs_rect.Max,
                    ...
                    // Display individual triangles/vertices. Hover on to get
                    // the corresponding triangle highlighted.
                    ImGuiListClipper clipper(pcmd->ElemCount /
                                             3); // Manually coarse clip our
                                                 // print out of individual
                                                 // vertices to save CPU, only
                                                 // items that may be visible.
                    while (clipper.Step())
                             ...
                             prim++)
                        {
                            ...
                            if (ImGui::IsItemHovered())
                            {
                                ...
                                overlay_draw_list->Flags &=
                                    ~ImDrawListFlags_AntiAliasedLines; // Disable
                                                                       // AA on
                                                                       // triangle
                                                                       // outlines
                                                                       // at is
            ...
            static void NodeWindow(ImGuiWindow* window, const char* label)
            {
                ...
                ImGui::BulletText("Pos: (%.1f,%.1f), Size: (%.1f,%.1f), "
                                  "SizeContents (%.1f,%.1f)",
                                  window->Pos.x,
                                  window->Pos.y,
                                  window->Size.x,
                                  ...
                                  window->SizeContents.y);
                if (ImGui::IsItemHovered())
                    GImGui->OverlayDrawList.AddRect(window->Pos,
                                                    window->Pos + window->Size,
                                                    IM_COL32(255, 255, 0, 255));
                ...
                ImGui::BulletText("Active: %d, WriteAccessed: %d",
                                  window->Active,
        ...
        if (ImGui::TreeNode(
                "DrawList", "Active DrawLists (%d)", g.RenderDrawLists[0].Size))
        {
            for (int layer = 0; layer < IM_ARRAYSIZE(g.RenderDrawLists);
                 layer++)
        ...
        if (ImGui::TreeNode(
                "Popups", "Open Popups Stack (%d)", g.OpenPopupStack.Size))
        {
        ...
        if (ImGui::TreeNode("Basic state"))
        {
            ...
            ImGui::Text("HoveredRootWindow: '%s'",
                        g.HoveredRootWindow ? g.HoveredRootWindow->Name
                                            : "NULL");
            ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec)",
                        g.HoveredId,
                        g.HoveredIdPreviousFrame,
                        g.HoveredIdTimer); // Data is "in-flight" so depending
                                           // on when the Metrics window is
                                           // called we may see current frame
                                           // information or not
            ImGui::Text("ActiveId: 0x%08X/0x%08X (%.2f sec)",
                        g.ActiveId,
