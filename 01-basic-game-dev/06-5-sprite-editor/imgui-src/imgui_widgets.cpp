...
static ImVec2 InputTextCalcTextSizeW(const ImWchar*  text_begin,
                                     ...
                                     const ImWchar** remaining        = nullptr,
                                     ImVec2*         out_offset       = nullptr,
        ...
        style.FramePadding.y <
            window->DC
                .CurrentLineTextBaseOffset) // Try to vertically align buttons
                                            ...
                                            // (bit hacky, since it shouldn't be
                                            // a flag)
                          ...
                          ImGuiButtonFlags flags)
{
    ...
    RenderArrow(bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f),
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImRect bb(pos,
              pos + ImVec2(g.FontSize, g.FontSize) +
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    // visual noise on very small window and facilitate using the resize grab)
    float alpha = 1.0f;
    if ((axis == ImGuiAxis_Y) &&
        bb_height < g.FontSize + g.Style.FramePadding.y * 2.0f)
    {
        alpha = ImSaturate((bb_height - g.FontSize) /
                       ...
                       ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(
            frame_bb.Min,
            ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Max.y),
            frame_col,
            style.FrameRounding,
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImVec2 pos =
                FindBestWindowPosForPopupEx(frame_bb.GetBL(),
                                            size_expected,
...
static const ImGuiDataTypeInfo GDataTypeInfo[] = {
    { .Size     = sizeof(char),
      .PrintFmt = "%d",
      .ScanFmt  = "%d" }, // ImGuiDataType_S8
    { .Size = sizeof(unsigned char), .PrintFmt = "%u", .ScanFmt = "%u" },
    { .Size     = sizeof(short),
      .PrintFmt = "%d",
      .ScanFmt  = "%d" }, // ImGuiDataType_S16
    { .Size = sizeof(unsigned short), .PrintFmt = "%u", .ScanFmt = "%u" },
    { .Size     = sizeof(int),
      .PrintFmt = "%d",
      .ScanFmt  = "%d" }, // ImGuiDataType_S32
    { .Size = sizeof(unsigned int), .PrintFmt = "%u", .ScanFmt = "%u" },
#ifdef _MSC_VER
    ...
    { .Size     = sizeof(ImS64),
      .PrintFmt = "%lld",
      .ScanFmt  = "%lld" }, // ImGuiDataType_S64
    { .Size = sizeof(ImU64), .PrintFmt = "%llu", .ScanFmt = "%llu" },
#endif
    { .Size     = sizeof(float),
      .PrintFmt = "%f",
      .ScanFmt = "%f" }, // ImGuiDataType_Float (float are promoted to double in
                         // va_arg)
    { .Size     = sizeof(double),
      .PrintFmt = "%f",
      .ScanFmt  = "%lf" }, // ImGuiDataType_Double
};
...
static void DataTypeApplyOp(
    ImGuiDataType data_type, int op, void* output, void* arg1, const void* arg2)
{
    IM_ASSERT(op == '+' || op == '-');
    switch (data_type)
    {
                                    ...
                                    const char*   format)
{
             ...
             data_type == ImGuiDataType_U64)
    {
        ...
        // We don't bother handling support for legacy operators since they are
        // a little too crappy. Instead we will later implement a proper
                          ...
                          ImGuiDragFlags flags)
{
    ...
    const ImGuiAxis axis =
        (flags & ImGuiDragFlags_Vertical) ? ImGuiAxis_Y : ImGuiAxis_X;
    ...
    TYPE v_cur                         = *v;
    auto v_old_ref_for_accum_remainder = (FLOATTYPE)0.0f;
    ...
    if (is_power)
    {
        ...
        FLOATTYPE v_old_norm_curved =
            ImPow((FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min),
                  (FLOATTYPE)1.0f / power);
        FLOATTYPE v_new_norm_curved =
            v_old_norm_curved + (g.DragCurrentAccum / (v_max - v_min));
        v_cur =
            v_min + (TYPE)ImPow(ImSaturate((float)v_new_norm_curved), power) *
                        (v_max - v_min);
                         ...
                         ImGuiDragFlags flags)
{
    ...
    switch (data_type)
    {
        case ImGuiDataType_S8:
        {
            auto v32 = (ImS32) * (ImS8*)v;
            bool r   = DragBehaviorT<ImS32, ImS32, float>(
                ImGuiDataType_S32,
                &v32,
                v_speed,
                v_min ? *(const ImS8*)v_min : IM_S8_MIN,
                v_max ? *(const ImS8*)v_max : IM_S8_MAX,
                format,
                power,
                flags);
            if (r)
        ...
        case ImGuiDataType_U8:
        {
            auto v32 = (ImU32) * (ImU8*)v;
            bool r   = DragBehaviorT<ImU32, ImS32, float>(
                ImGuiDataType_U32,
                &v32,
                v_speed,
                v_min ? *(const ImU8*)v_min : IM_U8_MIN,
                v_max ? *(const ImU8*)v_max : IM_U8_MAX,
                format,
                power,
                flags);
            if (r)
        ...
        case ImGuiDataType_S16:
        {
            auto v32 = (ImS32) * (ImS16*)v;
            bool r   = DragBehaviorT<ImS32, ImS32, float>(
                ImGuiDataType_S32,
                &v32,
                v_speed,
                v_min ? *(const ImS16*)v_min : IM_S16_MIN,
                v_max ? *(const ImS16*)v_max : IM_S16_MAX,
                format,
                power,
                flags);
            if (r)
        ...
        case ImGuiDataType_U16:
        {
            auto v32 = (ImU32) * (ImU16*)v;
            bool r   = DragBehaviorT<ImU32, ImS32, float>(
                ImGuiDataType_U32,
                &v32,
                v_speed,
                v_min ? *(const ImU16*)v_min : IM_U16_MIN,
                v_max ? *(const ImU16*)v_max : IM_U16_MAX,
                format,
                power,
                flags);
            if (r)
        ...
        case ImGuiDataType_S32:
            return DragBehaviorT<ImS32, ImS32, float>(
                ...
                v_max ? *(const ImS32*)v_max : IM_S32_MAX,
                format,
                power,
                flags);
        case ImGuiDataType_U32:
            return DragBehaviorT<ImU32, ImS32, float>(
                ...
                v_max ? *(const ImU32*)v_max : IM_U32_MAX,
                format,
                power,
                flags);
        case ImGuiDataType_S64:
            return DragBehaviorT<ImS64, ImS64, double>(
                ...
                v_max ? *(const ImS64*)v_max : IM_S64_MAX,
                format,
                power,
                flags);
        case ImGuiDataType_U64:
            return DragBehaviorT<ImU64, ImS64, double>(
                ...
                v_max ? *(const ImU64*)v_max : IM_U64_MAX,
                format,
                power,
                flags);
        case ImGuiDataType_Float:
            return DragBehaviorT<float, float, float>(
                ...
                v_max ? *(const float*)v_max : FLT_MAX,
                format,
                power,
                flags);
        case ImGuiDataType_Double:
            return DragBehaviorT<double, double, double>(
                ...
                v_max ? *(const double*)v_max : DBL_MAX,
                format,
                power,
                flags);
        case ImGuiDataType_COUNT:
                       ...
                       float         power)
{
    ...
    if (power != 1.0f)
        IM_ASSERT(v_min != nullptr &&
                  v_max != nullptr); // When using a power curve the drag needs
    ...
    if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
    {
        ...
        return InputScalarAsWidgetReplacement(
            frame_bb, id, label, data_type, v, format);
    }
    ...
    const char* value_buf_end =
        value_buf +
        DataTypeFormatString(
            value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
                        ...
                        float         power)
{
    ...
    for (int i = 0; i < components; i++)
    {
        PushID(i);
        value_changed |=
            DragScalar("", data_type, v, v_speed, v_min, v_max, format, power);
...
{
    return DragScalar(
        label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, power);
}
...
{
    return DragScalar(
        label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max, format);
}
...
{
    return DragScalarN(
        label, ImGuiDataType_S32, v, 2, v_speed, &v_min, &v_max, format);
}
...
{
    return DragScalarN(
        label, ImGuiDataType_S32, v, 3, v_speed, &v_min, &v_max, format);
}
...
{
    return DragScalarN(
        label, ImGuiDataType_S32, v, 4, v_speed, &v_min, &v_max, format);
}
                                       ...
                                       float         linear_zero_pos)
{
    ...
    if (is_power)
    {
        if (v_clamped < 0.0f)
        {
            const float f = 1.0f - (float)((v_clamped - v_min) /
                                           (ImMin((TYPE)0, v_max) - v_min));
            return (1.0f - ImPow(f, 1.0f / power)) * linear_zero_pos;
        }
        else
        {
            const auto f = (float)((v_clamped - ImMax((TYPE)0, v_min)) /
                ...
                else
                {
                    if ((v_range >= -100.0f && v_range <= 100.0f) ||
                        ...
                        delta = ((delta < 0.0f) ? -1.0f : +1.0f) /
                                (float)v_range; // Gamepad/keyboard tweak speeds
                                                // in integer steps
                    else
                        delta /= 100.0f;
                }
                         ...
                         float         power)
{
    ...
    RenderNavHighlight(frame_bb, id);
    RenderFrame(
        frame_bb.Min, frame_bb.Max, frame_col, true, g.Style.FrameRounding);
                          ...
                          float         power)
{
    ...
    for (int i = 0; i < components; i++)
    {
        PushID(i);
        value_changed |=
            SliderScalar("", data_type, v, v_min, v_max, format, power);
        SameLine(0, g.Style.ItemInnerSpacing.x);
        PopID();
        PopItemWidth();
        v = (void*)((char*)v + type_size);
...
{
    return SliderScalar(
        label, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
                         ...
                         float       power)
{
    return SliderScalarN(
        label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, power);
}
...
{
    return SliderScalarN(
        label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power);
}
...
{
    return SliderScalarN(
        label, ImGuiDataType_Float, v, 4, &v_min, &v_max, format, power);
}
...
bool ImGui::SliderInt2(
    const char* label, int v[2], int v_min, int v_max, const char* format)
{
    return SliderScalarN(
        label, ImGuiDataType_S32, v, 2, &v_min, &v_max, format);
}

bool ImGui::SliderInt3(
    const char* label, int v[3], int v_min, int v_max, const char* format)
{
    return SliderScalarN(
        label, ImGuiDataType_S32, v, 3, &v_min, &v_max, format);
}

bool ImGui::SliderInt4(
    const char* label, int v[4], int v_min, int v_max, const char* format)
{
    return SliderScalarN(
        label, ImGuiDataType_S32, v, 4, &v_min, &v_max, format);
}
                         ...
                         float         power)
{
    return VSliderScalar(
        label, size, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
...
{
    return VSliderScalar(
        label, size, ImGuiDataType_S32, v, &v_min, &v_max, format);
}
                                         ...
                                         size_t      buf_size)
{
    ...
    if (fmt_end[0] == 0) // If we only have leading decoration, we don't need to
        ...
        return fmt_start;
    ImStrncpy(
        buf, fmt_start, ImMin((size_t)(fmt_end - fmt_start) + 1, buf_size));
    return buf;
}
...
// Create text input in place of an active drag/slider (used when doing a
                                           ...
                                           const char*   format)
{
    ...
    // On the first frame, g.ScalarAsInputTextId == 0, then on subsequent frames
    ...
    DataTypeFormatString(
        data_buf, IM_ARRAYSIZE(data_buf), data_type, data_ptr, format);
    ...
    bool value_changed = InputTextEx(
        label, data_buf, IM_ARRAYSIZE(data_buf), bb.GetSize(), flags);
    if (g.ScalarAsInputTextId == 0)
    {
                         ...
                         ImGuiInputTextFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        PushID(i);
        value_changed |=
            InputScalar("", data_type, v, step, step_fast, format, flags);
        SameLine(0, g.Style.ItemInnerSpacing.x);
        PopID();
        PopItemWidth();
        v = (void*)((char*)v + type_size);
                       ...
                       ImGuiInputTextFlags flags)
{
    ...
    return InputScalar(label,
                       ImGuiDataType_Float,
                       (void*)v,
                       (void*)(step > 0.0f ? &step : nullptr),
                       (void*)(step_fast > 0.0f ? &step_fast : nullptr),
                       format,
                       flags);
}
...
{
    return InputScalarN(
        label, ImGuiDataType_Float, v, 2, nullptr, nullptr, format, flags);
}
...
{
    return InputScalarN(
        label, ImGuiDataType_Float, v, 3, nullptr, nullptr, format, flags);
}
...
{
    return InputScalarN(
        label, ImGuiDataType_Float, v, 4, nullptr, nullptr, format, flags);
}
                       ...
                       ImGuiInputTextFlags flags)
{
    ...
    if (decimal_precision >= 0)
        ImFormatString(
            format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    if (decimal_precision >= 0)
        ImFormatString(
            format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
    return InputScalarN(
        label, ImGuiDataType_Float, v, 2, NULL, NULL, format, flags);
}
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    if (decimal_precision >= 0)
        ImFormatString(
            format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
    return InputScalarN(
        label, ImGuiDataType_Float, v, 4, NULL, NULL, format, flags);
}
...
{
    return InputScalarN(
        label, ImGuiDataType_S32, v, 2, nullptr, nullptr, "%d", flags);
}

bool ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags)
{
    return InputScalarN(
        label, ImGuiDataType_S32, v, 3, nullptr, nullptr, "%d", flags);
}

bool ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags)
{
    return InputScalarN(
        label, ImGuiDataType_S32, v, 4, nullptr, nullptr, "%d", flags);
}
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    return InputScalar(label,
                       ...
                       (void*)v,
                       (void*)(step > 0.0 ? &step : nullptr),
                       (void*)(step_fast > 0.0 ? &step_fast : nullptr),
                       format,
                       flags);
}
...
{
    return ImCharIsBlankW(c) || c == ',' || c == ';' || c == '(' || c == ')' ||
           c == '{' || c == '}' || c == '[' || c == ']' || c == '|';
}
                        ...
                        void*                  callback_user_data)
{
    ...
    const bool user_scroll_finish =
        is_multiline && state != nullptr && g.ActiveId == 0 &&
        g.ActiveIdPreviousFrame == GetScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active =
        is_multiline && state != nullptr &&
        g.ActiveId == GetScrollbarID(draw_window, ImGuiAxis_Y);
    ...
    const bool init_make_active = (focus_requested || user_clicked ||
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        state->CurLenW = ImTextStrFromUtf8(
            state->TextW.Data, buf_size, buf, nullptr, &buf_end);
        state->CurLenA =
        ...
        // widget
        // FIXME: For non-readonly widgets we might be able to require that
        // TextAIsValid && TextA == buf ? (untested) and discard undo stack if
        // user buffer has changed.
    ...
    {
        const bool will_render_cursor =
            (g.ActiveId == id) || (user_scroll_active);
        const bool will_render_selection =
            state->HasSelection() &&
            (RENDER_SELECTION_WHEN_INACTIVE || will_render_cursor);
        if (will_render_cursor || will_render_selection)
        {
            const char* buf_end = nullptr;
            state->TextW.resize(buf_size + 1);
            state->CurLenW = ImTextStrFromUtf8(
                state->TextW.Data, state->TextW.Size, buf, nullptr, &buf_end);
        ...
        if (io.InputQueueCharacters.Size > 0)
        {
            ...
            if (!ignore_inputs && !is_readonly && !user_nav_input_start)
                for (int n = 0; n < io.InputQueueCharacters.Size; n++)
                {
                    ...
                    auto c = (unsigned int)io.InputQueueCharacters[n];
                    if (InputTextFilterCharacter(
                            &c, flags, callback, callback_user_data))
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_paste =
            ...
            !is_readonly;
        const bool is_undo =
            ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_Z)) && !is_readonly &&
             is_undoable);
        const bool is_redo =
        ...
        else if (is_cut || is_copy)
        {
            ...
            if (io.SetClipboardTextFn)
            {
                ...
                const int clipboard_data_len =
                    ...
                    1;
                char* clipboard_data =
                    (char*)MemAlloc(clipboard_data_len * sizeof(char));
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                auto*     clipboard_filtered =
                    (ImWchar*)MemAlloc((clipboard_len + 1) * sizeof(ImWchar));
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
                          ...
                          ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ImGuiInputTextCallbackData callback_data;
                    memset(
                        &callback_data, 0, sizeof(ImGuiInputTextCallbackData));
        ...
        state->UserCallbackData = nullptr;
    }
    ...
    const char* buf_display_end        = nullptr;
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            const ImWchar* searches_input_ptr[2]      = { nullptr, nullptr };
            ...
            // // FIXME-OPT: Could use this when wchar_t are 16-bits
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                if (rect_pos.y < clip_rect.y)
                {
                    ...
                    // when wchar_t are 16-bits p = p ? p + 1 :
                    // text_selected_end;
                ...
                else
                {
                    ImVec2 rect_size = InputTextCalcTextSizeW(
                        p, text_selected_end, &p, nullptr, true);
                    if (rect_size.x <= 0.0f)
                        rect_size.x =
                            (float)(int)(g.Font->GetCharAdvance((ImWchar)' ') *
                                         0.50f); // So we can see selected empty
                                                 // lines
        ...
        // ImDrawList crash.
        buf_display =
            (!is_readonly && state->TextAIsValid) ? state->TextA.Data : buf;
        ...
        if (is_multiline ||
            ...
            draw_window->DrawList->AddText(g.Font,
                                           ...
                                           buf_display_end,
                                           0.0f,
                                           is_multiline ? nullptr : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if (!is_readonly)
                g.PlatformImePos = ImVec2(cursor_screen_pos.x - 1.0f,
    ...
    else
    {
        ...
        buf_display = (g.ActiveId == id && !is_readonly && state->TextAIsValid)
                          ? state->TextA.Data
                          : buf;
        if (is_multiline)
            text_size = ImVec2(size.x,
                         ...
                         const float*        ref_col)
{
    ...
    auto  bars_triangles_half_sz = (float)(int)(bars_width * 0.20f);
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            float cos_hue_angle = ImCos(-H * 2.0f * std::numbers::pi_v<float>);
            float sin_hue_angle = ImSin(-H * 2.0f * std::numbers::pi_v<float>);
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        {
            const float a0 =
                (n) / 6.0f * 2.0f * std::numbers::pi_v<float> - aeps;
            const float a1 =
                (n + 1.0f) / 6.0f * 2.0f * std::numbers::pi_v<float> + aeps;
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
    ...
    if ((flags & ImGuiColorEditFlags__PickerMask) == 0)
        flags |= ImGuiColorEditFlags__OptionsDefault &
                 ImGuiColorEditFlags__PickerMask;
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & ImGuiColorEditFlags__DisplayMask))); // Check only 1
                                                           // option is selected
    IM_ASSERT(ImIsPowerOfTwo((
                             ...
                             const char*        label_end)
{
    ...
    if (!is_leaf)
    {
        if (pressed)
        {
            ...
            if (g.DragDropActive &&
                is_open) // When using Drag and Drop "hold to open" we keep the
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            ...
            RenderTextClipped(
                text_pos, frame_bb.Max, label, label_end, &label_size);
...
// Tip: pass a non-visible label (e.g. "##dummy") then you can use the space to
// draw other text or image. But you need to make sure the ID is unique, e.g.
// enclose calls in PushID/PopID or use ##unique_id.
                       ...
                       const ImVec2&        size_arg)
{
    ...
    float  w_draw =
        ImMax(label_size.x, window->Pos.x + max_x - window_padding.x - pos.x);
    ImVec2 size_draw(
    ...
    auto  spacing_L = (float)(int)(style.ItemSpacing.x * 0.5f);
    auto  spacing_U = (float)(int)(style.ItemSpacing.y * 0.5f);
    ...
    RenderTextClipped(bb_inner.Min,
                      bb_inner.Max,
                      label,
                      nullptr,
                      &label_size,
                          ...
                          int         height_in_items)
{
    ...
    // We add +25% worth of item height to allow the user to see at a glance if
    // there are more items up/down, without looking at the scrollbar. We don't
    // add this extra bit if items_count <= height_in_items. It is slightly
    // dodgy, because it means a dynamic list of items will make the widget
    // resize occasionally when it crosses that size. I am expecting that
    // someone will come and complain about this behavior in a remote future,
    // then we can advise on a better solution.
...
{
    auto*       plot_data = (ImGuiPlotArrayGetterData*)data;
    const float v =
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open =
        Begin("##MainMenuBar", nullptr, window_flags) && BeginMenuBar();
    PopStyleVar(2);
...
void ImGui::EndMenuBar()
{
        ...
        (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
            ...
            g.NavMoveRequestForward == ImGuiNavForward_None)
        {
            ...
            FocusWindow(window);
            SetNavIDWithRectRel(
                window->NavLastIds[1], 1, window->NavRectRel[1]);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    // is going to be different! It is choosen by FindBestWindowPosForPopup().
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        popup_pos =
            ImVec2(pos.x - 1.0f - (float)(int)(style.ItemSpacing.x * 0.5f),
        ...
        window->DC.CursorPos.x +=
            (float)(int)(style.ItemSpacing.x *
                         (-1.0f +
                          0.5f)); // -1 spacing to compensate the spacing added
                                  // when Selectable() did a SameLine(). It
                                  // would also work to call SameLine()
                                  ...
                                  // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window &&
            g.OpenPopupStack.Size > g.BeginPopupStack.Size &&
            g.OpenPopupStack[g.BeginPopupStack.Size].ParentWindow == window &&
            !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            if (ImGuiWindow* next_window =
                    g.OpenPopupStack[g.BeginPopupStack.Size].Window)
            {
                // FIXME-DPI: Values should be derived from a master "scale"
                // factor.
...
void ImGui::EndMenu()
{
    ...
    // means that with the current code, a BeginMenu() from outside another menu
                     ...
                     bool        enabled)
{
    ...
    ImGuiSelectableFlags flags = ImGuiSelectableFlags_PressedOnRelease |
    ...
    else
    {
        ImVec2 shortcut_size =
            shortcut ? CalcTextSize(shortcut, nullptr) : ImVec2(0.0f, 0.0f);
        float w = window->MenuColumns.DeclColumns(
            label_size.x,
            shortcut_size.x,
...
// [SECTION] Widgets: BeginTabBar, EndTabBar, etc.
//-------------------------------------------------------------------------
// [BETA API] API may evolve! This code has been extracted out of the Docking
// branch, and some of the construct which are not used in Master may be left
// here to facilitate merging.
//-------------------------------------------------------------------------
// - BeginTabBar()
...
{
    const auto* a = (const ImGuiTabBarSortItem*)lhs;
    const auto* b = (const ImGuiTabBarSortItem*)rhs;
...
static ImGuiTabBarRef GetTabBarRefFromTabBar(ImGuiTabBar* tab_bar)
{
    ...
    if (g.TabBars.Contains(tab_bar))
        return { g.TabBars.GetIndex(tab_bar) };
    return { tab_bar };
}
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    return BeginTabBarEx(
        tab_bar, tab_bar_bb, flags | ImGuiTabBarFlags_IsFocused);
}
                          ...
                          ImGuiTabBarFlags flags)
{
    ...
    // When toggling back from ordered to manually-reorderable, shuffle tabs to
    // enforce the last visible order. Otherwise, the most recently inserted
    // tabs would move at the end of visible list which can be a little too
    ...
    // Draw separator
    const ImU32 col =
        GetColorU32((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive
                                                         : ImGuiCol_Tab);
    const float y = tab_bar->BarRect.Max.y - 1.0f;
    {
        const float separator_min_x =
            tab_bar->BarRect.Min.x - window->WindowPadding.x;
        const float separator_max_x =
            tab_bar->BarRect.Max.x + window->WindowPadding.x;
        window->DrawList->AddLine(
            ImVec2(separator_min_x, y), ImVec2(separator_max_x, y), col, 1.0f);
    }
    return true;
}
...
void ImGui::EndTabBar()
{
    ...
    if (tab_bar->VisibleTabWasSubmitted || tab_bar->VisibleTabId == 0 ||
        tab_bar_appearing)
        tab_bar->ContentsHeight =
            ImMax(window->DC.CursorPos.y - tab_bar->BarRect.Max.y, 0.0f);
    else
        window->DC.CursorPos.y =
            tab_bar->BarRect.Max.y + tab_bar->ContentsHeight;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (tab_list_popup_button)
        if (ImGuiTabItem* tab_to_select = TabBarTabListPopupButton(
                tab_bar)) // NB: Will alter BarRect.Max.x!
            scroll_track_selected_tab_id = tab_bar->SelectedTabId =
                tab_to_select->ID;
    ...
    ImGuiTabItem* most_recently_selected_tab = nullptr;
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        if (most_recently_selected_tab == nullptr ||
        ...
        tab->WidthContents =
            TabItemCalcSize(
                tab_name,
                (tab->Flags & ImGuiTabItemFlags_NoCloseButton) ? false : true)
                .x;

        width_total_contents +=
            (tab_n > 0 ? g.Style.ItemInnerSpacing.x : 0.0f) +
            tab->WidthContents;
    ...
    float       width_excess = (width_avail < width_total_contents)
                                   ? (width_total_contents - width_avail)
                                   : 0.0f;
        ...
        (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
    {
        ...
        if (tab_bar->Tabs.Size > 1)
            ImQsort(width_sort_buffer.Data,
                    (size_t)width_sort_buffer.Size,
        ...
        {
            while (tab_count_same_width < tab_bar->Tabs.Size &&
                   width_sort_buffer[0].Width ==
                       width_sort_buffer[tab_count_same_width].Width)
                tab_count_same_width++;
            float width_to_remove_per_tab_max =
                (tab_count_same_width < tab_bar->Tabs.Size)
                    ? (width_sort_buffer[0].Width -
                       width_sort_buffer[tab_count_same_width].Width)
                    : (width_sort_buffer[0].Width - 1.0f);
            float width_to_remove_per_tab =
                ImMin(width_excess / tab_count_same_width,
        ...
        for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
            tab_bar->Tabs[width_sort_buffer[tab_n].Index].Width =
    ...
    if (tab_bar->SelectedTabId == 0 && tab_bar->NextSelectedTabId == 0 &&
        most_recently_selected_tab != nullptr)
        scroll_track_selected_tab_id = tab_bar->SelectedTabId =
    ...
    const float scrolling_speed = (tab_bar->PrevFrameVisible + 1 < g.FrameCount)
                                      ? FLT_MAX
                                      : (g.IO.DeltaTime * g.FontSize * 70.0f);
    if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget)
        tab_bar->ScrollingAnim = ImLinearSweep(
            tab_bar->ScrollingAnim, tab_bar->ScrollingTarget, scrolling_speed);
...
{
    scrolling =
        ImMin(scrolling, tab_bar->OffsetMax - tab_bar->BarRect.GetWidth());
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    float tab_x2 = tab->Offset + tab->Width +
                   (order + 1 < tab_bar->Tabs.Size ? margin : 1.0f);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    bool         want_clip_rect = !avail_bar_rect.Contains(
        ImRect(window->DC.CursorPos,
               window->DC.CursorPos + ImVec2(scrolling_buttons_width, 0.0f)));
    if (want_clip_rect)
        PushClipRect(tab_bar->BarRect.Min,
                     tab_bar->BarRect.Max +
                         ImVec2(g.Style.ItemInnerSpacing.x, 0.0f),
                     true);

    ImGuiTabItem* tab_to_select = nullptr;
    ...
    window->DC.CursorPos =
        ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width,
    ...
    window->DC.CursorPos = ImVec2(
        tab_bar->BarRect.Max.x - scrolling_buttons_width + arrow_button_size.x,
        tab_bar->BarRect.Min.y);
    if (ArrowButtonEx("##>",
                      ImGuiDir_Right,
                      arrow_button_size,
            ...
            tab_to_select =
                &tab_bar->Tabs[(target_order >= 0 &&
                                target_order < tab_bar->Tabs.Size)
                                   ? target_order
                                   : selected_order]; // If we are at the end of
                                                      // the list, still scroll
                                                      // to make our tab visible
        }
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", nullptr, ImGuiComboFlags_NoPreview);
    PopStyleColor(2);
                         ...
                         ImGuiTabItemFlags flags)
{
    ...
    if (ret && !(flags & ImGuiTabItemFlags_NoPushId))
    {
        ...
        g.CurrentWindow->IDStack.push_back(
            tab->ID); // We already hashed 'label' so push into the ID stack
                      // directly instead of doing another hash through
                      // PushID(label)
    }
    return ret;
}
                      ...
                      ImGuiTabItemFlags flags)
{
    ...
    // render. We make a dummy call to ItemAdd() so that attempts to use a
    // contextual popup menu with an implicit ID won't use an older ID.
    if (p_open && !*p_open)
    {
        PushItemFlag(ImGuiItemFlags_NoNav | ImGuiItemFlags_NoNavDefaultFocus,
                     true);
        ItemAdd(ImRect(), id);
        PopItemFlag();
        return false;
    ...
    ImVec2 size = TabItemCalcSize(label, p_open != nullptr);
    ...
    // (We already handled layout and sizing using the previous known order, but
    // sizing is not affected by order!)
    ...
    window->DC.CursorPos =
        tab_bar->BarRect.Min +
        ImVec2((float)(int)tab->Offset - tab_bar->ScrollingAnim, 0.0f);
    ImVec2 pos = window->DC.CursorPos;
    ...
    bool want_clip_rect = (bb.Min.x < tab_bar->BarRect.Min.x) ||
                          (bb.Max.x >= tab_bar->BarRect.Max.x);
    if (want_clip_rect)
        PushClipRect(
            ImVec2(ImMax(bb.Min.x, tab_bar->BarRect.Min.x), bb.Min.y - 1),
            ImVec2(tab_bar->BarRect.Max.x, bb.Max.y),
            true);
    ...
    if (pressed || ((flags & ImGuiTabItemFlags_SetSelected) &&
                    !tab_contents_visible)) // SetSelected can only be passed on
                                            // explicit tab bar
        tab_bar->NextSelectedTabId = id;

    // Allow the close button to overlap unless we are dragging (in which case
    // we don't want any overlapping tabs to be hovered)
    if (!held)
        SetItemAllowOverlap();
    ...
    const ImGuiID close_button_id =
        p_open ? window->GetID((void*)((intptr_t)id + 1)) : 0;
    bool just_closed = TabItemLabelAndCloseButton(display_draw_list,
                                                  bb,
                                                  flags,
    ...
    // systems messes up with HoveredIdTimer)
...
// the function SetTabItemClosed() after BeginTabBar() and before any call to
// BeginTabItem()
void ImGui::SetTabItemClosed(const char* label)
{
    ...
    if (is_within_manual_tab_bar)
    {
        ImGuiTabBar* tab_bar = g.CurrentTabBar;
        IM_ASSERT(
            tab_bar->WantLayout); // Needs to be called AFTER BeginTabBar() and
                                  // BEFORE the first call to BeginTabItem()
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    return { ImMin(size.x, TabBarCalcMaxTabWidth()), size.y };
}
                              ...
                              ImU32             col)
{
    ...
    const float rounding =
        ImMax(0.0f, ImMin(g.Style.TabRounding, width * 0.5f - 1.0f));
    const float y1 = bb.Min.y + 1.0f;
    const float y2 = bb.Max.y - 1.0f;
    draw_list->PathLineTo(ImVec2(bb.Min.x, y2));
    draw_list->PathArcToFast(
        ImVec2(bb.Min.x + rounding, y1 + rounding), rounding, 6, 9);
    draw_list->PathArcToFast(
        ImVec2(bb.Max.x - rounding, y1 + rounding), rounding, 9, 12);
    ...
    if (g.Style.TabBorderSize > 0.0f)
    {
        ...
        draw_list->PathStroke(
            GetColorU32(ImGuiCol_Border), false, g.Style.TabBorderSize);
    }
}
                                       ...
                                       ImGuiID           close_button_id)
{
    ...
    if (flags & ImGuiTabItemFlags_UnsavedDocument)
    {
        text_pixel_clip_bb.Max.x -=
            CalcTextSize(TAB_UNSAVED_MARKER, nullptr, false).x;
    ...
    if (close_button_visible)
    {
        ...
        if (CloseButton(close_button_id,
                        ImVec2(bb.Max.x - frame_padding.x - close_button_sz,
                               bb.Min.y + frame_padding.y + close_button_sz),
                        close_button_sz))
    ...
    // text_pixel_clip_bb and !close_button_visible makes it tricky to abstract
    // at the moment
    const char* label_display_end = FindRenderedTextEnd(label);
    if (label_size.x > text_ellipsis_clip_bb.GetWidth())
    {
        ...
        float       label_size_clipped_x =
            g.Font
                ->CalcTextSizeA(g.FontSize,
                                text_ellipsis_clip_bb.GetWidth() -
                                    ellipsis_width + 1.0f,
                                0.0f,
                                label,
                                label_display_end,
        ...
        if (label_end == label &&
            label_end <
                label_display_end) // Always display at least 1 character if
                                   // there's no room for character + ellipsis
        {
            label_end =
                label + ImTextCountUtf8BytesFromChar(label, label_display_end);
            label_size_clipped_x =
                g.Font
                    ->CalcTextSizeA(g.FontSize, FLT_MAX, 0.0f, label, label_end)
                    .x;
        }
        while (label_end > label &&
               ImCharIsBlankA(label_end[-1])) // Trim trailing space
        {
            label_end--;
            label_size_clipped_x -=
                g.Font
                    ->CalcTextSizeA(
                        g.FontSize, FLT_MAX, 0.0f, label_end, label_end + 1)
                    .x; // Ascii blanks are always 1 byte
        }
        RenderTextClippedEx(draw_list,
                            text_pixel_clip_bb.Min,
                            text_pixel_clip_bb.Max,
                            label,
                            label_end,
                            &label_size,
                            ImVec2(0.0f, 0.0f));

        const float ellipsis_x =
            text_pixel_clip_bb.Min.x + label_size_clipped_x + 1.0f;
        if (!close_button_visible && ellipsis_x + ellipsis_width <= bb.Max.x)
            RenderPixelEllipsis(draw_list,
                                ImVec2(ellipsis_x, text_pixel_clip_bb.Min.y),
                                ellipsis_dot_count,
                                GetColorU32(ImGuiCol_Text));
    }
    else
