...
// available you probably want this to be available for reference. Thank you,
...
// pattern you will want to use in your code, but most of the real data you
...
#pragma clang diagnostic ignored                                               \
    "-Wexit-time-destructors" // warning : declaration requires an exit-time
                              ...
                              // static/globals.
#pragma clang diagnostic ignored                                               \
    "-Wunused-macros" // warning : warning: macro is not used // we define
...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    if (show_app_documents)
        ShowExampleAppDocuments(
            &show_app_documents); // Process the Document app next, as it may
                                  // also use a DockSpace()
    ...
    ImGui::PushItemWidth(ImGui::GetFontSize() *
                         -12); // Use fixed width for labels (by passing a
                               // negative value), the rest goes to widgets. We
                               // choose a width proportional to our font size.
    ...
    if (ImGui::BeginMenuBar())
    {
        ...
        if (ImGui::BeginMenu("Examples"))
        {
            ...
            ImGui::MenuItem(
                "Auto-resizing window", NULL, &show_app_auto_resize);
            ...
            ImGui::MenuItem(
                "Custom rendering", NULL, &show_app_custom_rendering);
    ...
    if (ImGui::CollapsingHeader("Help"))
    {
        ...
        ImGui::BulletText("Please see the ShowDemoWindow() code in "
                          "imgui_demo.cpp. <- you are here!");
        ImGui::BulletText("Please see the comments in imgui.cpp.");
        ImGui::BulletText("Please see the examples/ in application.");
        ImGui::BulletText("Enable 'io.ConfigFlags |= NavEnableKeyboard' for "
                          "keyboard controls.");
        ImGui::BulletText("Enable 'io.ConfigFlags |= NavEnableGamepad' for "
                          "gamepad controls.");
        ImGui::Separator();
    ...
    if (ImGui::CollapsingHeader("Configuration"))
    {
        ...
        if (ImGui::TreeNode("Configuration##2"))
        {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Required back-end to feed in gamepad inputs in "
                           "io.NavInputs[] and set io.BackendFlags |= "
                           "ImGuiBackendFlags_HasGamepad.\n\nRead instructions "
                           "in imgui.cpp for details.");
            ...
            ImGui::SameLine();
            ShowHelpMarker("Instruct back-end to not alter mouse cursor shape "
                           "and visibility.");
            ImGui::Checkbox("io.ConfigInputTextCursorBlink",
                            &io.ConfigInputTextCursorBlink);
            ImGui::SameLine();
            ShowHelpMarker("Set to false to disable blinking cursor, for users "
                           "who consider it distracting");
        ...
        {
            ShowHelpMarker(
                "Those flags are set by the back-ends (imgui_impl_xxx files) "
        ...
        if (ImGui::TreeNode("Capture/Logging"))
        {
            ...
            ImGui::LogButtons();
            ImGui::TextWrapped("You can also call ImGui::LogText() to output "
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
...

        {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Refer to the \"Combo\" section below for an "
...

        {
            ...
            ShowHelpMarker("USER:\nHold SHIFT or use mouse to select text.\n"
                           ...
                           "ESCAPE to revert.\n\nPROGRAMMER:\nYou can use the "
                           "ImGuiInputTextFlags_CallbackResize facility if you "
                           "need to wire InputText() to a dynamic string type. "
                           "See misc/cpp/imgui_stdlib.h for an example (this "
                           "is not demonstrated in imgui_demo.cpp).");

            ImGui::InputInt("input int", &i0);
            ImGui::SameLine();
            ShowHelpMarker("You can apply arithmetic operators +,*,/ on "
                           "numerical values.\n  e.g. [ 100 ], input \'*2\', "
                           "result becomes [ 200 ]\nUse +- to subtract.\n");
...

        {
            ...
            ImGui::DragFloat(
                "drag small float", &f2, 0.0001f, 0.0f, 0.0f, "%.06f ns");
        }

        {
            ...
            ImGui::SliderFloat(
                "slider float (curve)", &f2, -10.0f, 10.0f, "%.4f", 2.0f);
...

        {
            ...
            ShowHelpMarker(
                ...
                "and hold to use drag and drop.\nRight-click on the colored "
                "square to show options.\nCTRL+click on individual component "
                "to input value.\n");
    ...
    if (ImGui::TreeNode("Trees"))
    {
        ...
        {
            ShowHelpMarker("This is a more standard looking tree with "
                           "selectable nodes.\nClick to select, CTRL+Click to "
                           "toggle, click on arrows or double-click to open.");
            ...
            static int selection_mask =
                (1 << 2); // Dumb representation of what may be user-side
                          // selection state. You may carry selection state
                          // inside or outside your objects in whatever format
                          // you see fit.
            int node_clicked =
                -1; // Temporary storage of what node we have clicked to process
                    // selection at the end of the loop. May be a pointer to
                    // your own node type, etc.
            ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing,
                                ImGui::GetFontSize() *
                                    3); // Increase spacing to differentiate
                                        // leaves from expanded contents.
            for (int i = 0; i < 6; i++)
            {
                // Disable the default open on single-click behavior and pass in
                // Selected flag according to our selection state.
                ImGuiTreeNodeFlags node_flags =
                    ImGuiTreeNodeFlags_OpenOnArrow |
                    ImGuiTreeNodeFlags_OpenOnDoubleClick |
                    ((selection_mask & (1 << i)) ? ImGuiTreeNodeFlags_Selected
                                                 : 0);
                if (i < 3)
                {
                    // Node
            ...
            if (node_clicked != -1)
            {
                // Update selection state. Process outside of tree loop to avoid
                // visual inconsistencies during the clicking-frame.
    ...
    if (ImGui::TreeNode("Text"))
    {
        ...
        if (ImGui::TreeNode("Word Wrapping"))
        {
            ...
            ImGui::TextWrapped(
                ...
                "window. The current implementation for text wrapping follows "
                "simple rules suitable for English and possibly other "
                "languages.");
            ImGui::Spacing();
            ...
            ImGui::Text("The lazy dog is a good dog. This paragraph is made to "
        ...
        if (ImGui::TreeNode("UTF-8 Text"))
        {
            ...
            // (Needs a suitable font, try Noto, or Arial Unicode, or M+ fonts.
            ...
            // UTF-8 (e.g. Visual Studio save your file as 'UTF-8 without
            // signature')
            // - FOR THIS DEMO FILE ONLY, BECAUSE WE WANT TO SUPPORT OLD
            // COMPILERS, WE ARE *NOT* INCLUDING RAW UTF-8 CHARACTERS IN THIS
            // SOURCE FILE.
            ...
            ImGui::TextWrapped(
                "CJK text will only appears if the font was loaded with the "
                "appropriate CJK character ranges. Call "
                "io.Font->AddFontFromFileTTF() manually to load extra "
    ...
    if (ImGui::TreeNode("Images"))
    {
        ...
        ImGui::TextWrapped(
            ...
            "texture we have access to in this demo). Use the 'ImTextureID' "
            "type as storage to pass pointers or identifier to your own "
            "texture data. Hover the texture for a zoomed view!");

        // Here we are grabbing the font texture because that's the only one we
        // have access to inside the demo code. Remember that ImTextureID is
        // just storage for whatever you want it to be, it is essentially a
        // value that will be passed to the render function inside the ImDrawCmd
        // structure. If you use one of the default imgui_impl_XXXX.cpp
        // renderer, they all have comments at the top of their file to specify
        // what they expect to be stored in ImTextureID. (for example, the
        // imgui_impl_dx11.cpp renderer expect a 'ID3D11ShaderResourceView*'
        // pointer. The imgui_impl_glfw_gl3.cpp renderer expect a GLuint OpenGL
        // texture identifier etc.) If you decided that ImTextureID =
        // MyEngineTexture*, then you can pass your MyEngineTexture* pointers to
        // ImGui::Image(), and gather width/height through your own functions,
        // etc. Using ShowMetricsWindow() as a "debugger" to inspect the draw
        // data that are being passed to your render will help you debug issues
        // if you are confused about this. Consider using the lower-level
        // ImDrawList::AddImage() API, via
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (ImGui::ImageButton(my_tex_id,
                                   ImVec2(32, 32),
                                   ImVec2(0, 0),
    ...
    if (ImGui::TreeNode("Combo"))
    {
        ...
        // General BeginCombo() API, you have full control over your selection
        // data and display type. (your selection data could be an index, a
        // pointer to the object, an id for the object, a flag stored in the
        // object itself, etc.)
        const char* items[] = { "AAAA", "BBBB",    "CCCC", "DDDD",   "EEEE",
                                "FFFF", "GGGG",    "HHHH", "IIII",   "JJJJ",
                                "KKKK", "LLLLLLL", "MMMM", "OOOOOOO" };
        static const char* item_current =
            items[0]; // Here our selection is a single pointer stored outside
                      // the object.
    ...
    if (ImGui::TreeNode("Selectables"))
    {
        ...
        // information. When Selectable() has been clicked is returns true and
        // you can alter selection state accordingly.
        ...
        // as in real application your selection may be stored in a different
        // manner (in flags within objects, as an external list, etc).
        if (ImGui::TreeNode("Basic"))
        {
        ...
        {
            static bool selected[4 * 4] = { true,  false, false, false,
            ...
            for (int i = 0; i < 4 * 4; i++)
            {
                ImGui::PushID(i);
                if (ImGui::Selectable(
                        "Sailor", &selected[i], 0, ImVec2(50, 50)))
                {
                    // Note: We _unnecessarily_ test for both x/y and i here
                    // only to silence some static analyzer. The second part of
                    // each test is unnecessary.
                    int x = i % 4;
                    int y = i / 4;
                    if (x > 0)
                    {
        ...
        {
            ShowHelpMarker("Alignment applies when a selectable is larger than "
                           "its text content.\nBy default, Selectables uses "
                           "style.SelectableTextAlign but it can be overriden "
                           "on a per-item basis using PushStyleVar().");
    ...
    if (ImGui::TreeNode("Filtered Text Input"))
    {
        ...
        static char buf4[64] = "";
        ImGui::InputText(
            "uppercase", buf4, 64, ImGuiInputTextFlags_CharsUppercase);
        static char buf5[64] = "";
        ImGui::InputText(
            "no blank", buf5, 64, ImGuiInputTextFlags_CharsNoBlank);
        struct TextFilters
        {
        ...
        ShowHelpMarker("Display all characters as '*'.\nDisable clipboard cut "
                       "and copy.\nDisable logging.\n");
        ImGui::InputText(
            "password (clear)", bufpass, 64, ImGuiInputTextFlags_CharsNoBlank);
    ...
    if (ImGui::TreeNode("Multi-line Text Input"))
    {
        ...
        ShowHelpMarker(
            "You can use the ImGuiInputTextFlags_CallbackResize facility if "
            "you need to wire InputTextMultiline() to a dynamic string type. "
            "See misc/cpp/imgui_stdlib.h for an example. (This is not "
    ...
    if (ImGui::TreeNode("Plots Widgets"))
    {
        ...
        while (refresh_time < ImGui::GetTime()) // Create dummy data at fixed 60
                                                // hz rate for the demo
        {
        ...
        // indices. We probably want an API passing floats and user provide
        // sample rate/count.
        struct Funcs
        {
    ...
    if (ImGui::TreeNode("Color/Picker Widgets"))
    {
        ...
        ImGui::SameLine();
        ShowHelpMarker("Currently all this does is to lift the 0..1 limits on "
                       "dragging widgets.");
        int misc_flags =
        ...
        ShowHelpMarker(
            ...
            "slider/text inputs.\nWith the ImGuiColorEditFlags_NoLabel flag "
            "you can pass a non-empty label which will only be used for the "
            "tooltip and picker popup.");
        ...
        // Generate a dummy default palette. The palette will persist and can be
        // edited.
        ...
        if (ImGui::BeginPopup("mypicker"))
        {
            ...
            for (int n = 0; n < IM_ARRAYSIZE(saved_palette); n++)
            {
                ...
                // unless using ImGuiColorEditFlags_NoDragDrop)
        ...
        ImGui::ColorButton(
            "MyColor##3c", *(ImVec4*)&color, misc_flags, ImVec2(80, 80));
        ...
        ShowHelpMarker(
            ...
            "a display mode, but the user can change it with a "
            "right-click.\n\nColorPicker defaults to displaying RGB+HSV+Hex if "
            "you don't specify a display mode.\n\nYou can change the defaults "
        ...
        ShowHelpMarker(
            ...
            "options on a per-widget basis if needed, and the user can change "
            "non-forced ones with the options menu.\nWe don't have a getter to "
            "avoid encouraging you to persistently save values that aren't "
            "forward-compatible.");
...
// natural type maximum, hence the /2 below.
        ...
        ImGui::DragScalar("drag float ^2",
                          ...
                          2.0f);
        ImGui::SameLine();
        ShowHelpMarker("You can use the 'power' parameter to increase tweaking "
        ...
        ImGui::DragScalar("drag double",
                          ...
                          "%.10f grams",
                          1.0f);
        ImGui::DragScalar("drag double ^2",
                          ImGuiDataType_Double,
                          &f64_v,
                          0.0005f,
                          &f64_zero,
        ...
        ImGui::Text("Sliders");
        ImGui::SliderScalar(
            "slider s8 full", ImGuiDataType_S8, &s8_v, &s8_min, &s8_max, "%d");
        ImGui::SliderScalar(
            "slider u8 full", ImGuiDataType_U8, &u8_v, &u8_min, &u8_max, "%u");
        ImGui::SliderScalar("slider s16 full",
                            ImGuiDataType_S16,
                            &s16_v,
                            &s16_min,
                            &s16_max,
                            "%d");
        ImGui::SliderScalar("slider u16 full",
                            ImGuiDataType_U16,
                            &u16_v,
                            &u16_min,
                            &u16_max,
        ...
        ImGui::SliderScalar("slider s32 high",
                            ...
                            &s32_hi_b,
                            "%d");
        ImGui::SliderScalar("slider s32 full",
                            ImGuiDataType_S32,
                            &s32_v,
                            &s32_min,
                            &s32_max,
        ...
        ImGui::SliderScalar("slider u32 high",
                            ...
                            &u32_hi_b,
                            "%u");
        ImGui::SliderScalar("slider u32 full",
                            ImGuiDataType_U32,
                            &u32_v,
                            &u32_min,
                            &u32_max,
        ...
        ImGui::SliderScalar("slider u64 full",
                            ...
                            &u64_max,
                            "%I64u ms");
        ImGui::SliderScalar("slider float low",
                            ImGuiDataType_Float,
                            &f32_v,
                            &f32_zero,
                            &f32_one);
        ImGui::SliderScalar("slider float low^2",
                            ImGuiDataType_Float,
                            &f32_v,
                            &f32_zero,
                            &f32_one,
        ...
        ImGui::SliderScalar("slider double low",
                            ...
                            "%.10f grams",
                            1.0f);
        ImGui::SliderScalar("slider double low^2",
                            ImGuiDataType_Double,
                            &f64_v,
                            &f64_zero,
                            &f64_one,
        ...
        ImGui::InputScalar("input u32 hex",
                           ...
                           "%08X",
                           ImGuiInputTextFlags_CharsHexadecimal);
        ImGui::InputScalar("input s64",
                           ImGuiDataType_S64,
                           &s64_v,
                           inputs_step ? &s64_one : NULL);
        ImGui::InputScalar("input u64",
                           ImGuiDataType_U64,
                           &u64_v,
    ...
    if (ImGui::TreeNode("Vertical Sliders"))
    {
        ...
        const int    rows       = 3;
        const ImVec2 small_slider_size(18,
                                       (160.0f - (rows - 1) * spacing) / rows);
    ...
    {
        {
            ...
            // interaction. Also see the demo in Color Picker -> Palette demo.
            ...
            for (int n = 0; n < IM_ARRAYSIZE(names); n++)
            {
                ...
                if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None))
                {
                    ImGui::SetDragDropPayload(
                        "DND_DEMO_CELL",
                        &n,
                        sizeof(int)); // Set payload to carry the index of our
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
    {
        ...
        // functions. Note that the flags can be combined. (because BulletText
        // is an item itself and that would affect the output of IsItemHovered()
        ...
        if (embed_all_inside_a_child_window)
            ImGui::BeginChild(
                "outer_child", ImVec2(0, ImGui::GetFontSize() * 20), true);

        // Testing IsWindowFocused() function with its various flags. Note that
        ...
        // Testing IsWindowHovered() function with its various flags. Note that
        // the flags can be combined.
        ImGui::BulletText(
            "IsWindowHovered() = %d\n"
        ...
        // context menu (with BeginPopupContextItem) associated to the title bar
        // of a window.
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Child windows"))
    {
        ...
        // styles)
        // - Using SetCursorPos() to position the child window (because the
        // child window is an item from the POV of the parent window)
        ...
        //   section for more details about this.
        {
            ...
            ImGui::BeginChild(
                "blah", ImVec2(200, 100), true, ImGuiWindowFlags_None);
            for (int n = 0; n < 50; n++)
    ...
    if (ImGui::TreeNode("Basic Horizontal Layout"))
    {
        ...
        // Manually wrapping (we should eventually provide this as an automatic
        // layout feature, but for now you can do it manually)
    ...
    if (ImGui::TreeNode("Tabs"))
    {
        ...
        if (ImGui::TreeNode("Advanced & Close Button"))
        {
            ...
            // Tab Bar
            const char* names[4] = {
                "Artichoke", "Beetroot", "Celery", "Daikon"
            };
    ...
    {
        ShowHelpMarker("Using ImGui::BeginGroup()/EndGroup() to layout items. "
    ...
    {
        ShowHelpMarker("This is testing the vertical alignment that gets "
                       "applied on text to keep it aligned with widgets. Lines "
                       "only composed of text or \"small\" widgets fit in less "
                       "vertical spaces than lines with normal widgets.");
        ...
        ImGui::AlignTextToFramePadding(); // If your line starts with text, call
                                          // this to align it to upcoming
                                          // widgets.
        ...
        ImGui::AlignTextToFramePadding(); // Vertically align text node a bit
                                          // lower so it'll be vertically
                                          // centered with upcoming widget.
                                          // Otherwise you can use SmallButton
                                          // (smaller fit).
        bool node_open =
            ImGui::TreeNode("Node##2"); // Common mistake to avoid: if we want
                                        // to SameLine after TreeNode we need to
                                        // do it before we add child content.
        ImGui::SameLine(0.0f, spacing);
    ...
    {
        ShowHelpMarker("Use SetScrollHereY() or SetScrollFromPosY() to scroll "
                       "to a given position.");
        ...
        ImGui::SameLine(130);
        scroll_to |= ImGui::DragInt(
            "##pos_y", &scroll_to_px, 1.00f, 0, 9999, "Y = %d px");
        ImGui::PopItemWidth();
        if (scroll_to)
            track = false;
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ImGui::BeginChild(ImGui::GetID((void*)(intptr_t)i),
                              ImVec2(ImGui::GetWindowWidth() * 0.17f, 200.0f),
                              true);
            if (scroll_to)
                ImGui::SetScrollFromPosY(
                    ImGui::GetCursorStartPos().y + scroll_to_px, i * 0.25f);
    ...
    {
        ShowHelpMarker(
            "Horizontal scrolling for a window has to be enabled explicitly "
            "via the ImGuiWindowFlags_HorizontalScrollbar flag.\n\nYou may "
            "want to explicitly specify content width by calling "
        ...
        for (int line = 0; line < lines; line++)
        {
            ...
            // using basic Button+SameLine. If you want to create your own time
            // line for a real application you may be better off manipulating
            // the cursor position yourself, aka using
        ...
        {
            ImGui::BeginChild(
                "scrolling"); // Demonstrate a trick: you can use Begin to set
                              // yourself in the context of another window (here
    ...
    if (ImGui::TreeNode("Clipping"))
    {
        ...
        ImGui::TextWrapped(
            "On a per-widget basis we are occasionally clipping text CPU-side "
            "if it won't fit in its frame. Otherwise we are doing coarser "
            "clipping + passing a scissor rectangle to the renderer. The "
            "system is designed to try minimizing both execution and CPU/GPU "
            "rendering cost.");
...
static void ShowDemoWindowPopups()
{
    ...
    // being held by the programmer as we are used to with regular Begin()
    // calls.
    ...
    if (ImGui::TreeNode("Popups"))
    {
        ImGui::TextWrapped(
            ...
            "are behind the popup. Clicking outside the popup closes it.");
        ...
        // you may want to build a string using the "###" operator to preserve a
        // constant ID with a variable label)
    ...
    if (ImGui::TreeNode("Context menus"))
    {
        ...
        // BeginPopupContextItem() but without the Begin call. So here we will
        // make it that clicking on the text field with the right mouse button
        // (1) will toggle the visibility of the popup above.
        ImGui::Text(
            "(You can also right-click me to the same popup as above.)");
        ImGui::OpenPopupOnItemClick("item context menu", 1);

        // When used after an item that has an ID (here the Button), we can skip
        // providing an ID to BeginPopupContextItem(). BeginPopupContextItem()
        // will use the last item ID as the popup ID. In addition here, we want
        // to include your editable label inside the button label. We use the
        // ### operator to override the ID (read FAQ about ID for details)
    ...
    if (ImGui::TreeNode("Modals"))
    {
        ImGui::TextWrapped("Modal windows are like popups but the user cannot "
                           "close them by clicking outside the window.");
    ...
    if (ImGui::TreeNode("Menus inside a regular window"))
    {
        ...
        // differentiate the parent of this menu from the parent of the various
        // popup menus above. To do so we are encloding the items in a
        // PushID()/PopID() block to make them two different menusets. If we
        // don't, opening any popup above and hovering our menu here would open
        // it. This is because once a menu is active, we allow to switch to a
        // sibling menu by just hovering on it, which is the desired behavior
        // for regular menus.
        ImGui::PushID("foo");
...
static void ShowDemoWindowColumns()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            sprintf(label, "%04d", i);
            if (ImGui::Selectable(
                    label, selected == i, ImGuiSelectableFlags_SpanAllColumns))
                selected = i;
    ...
    */
    ...
    if (ImGui::TreeNode("Horizontal Scrolling"))
    {
        ...
        ImGuiListClipper clipper(
            ITEMS_COUNT); // Also demonstrate using the clipper for large list
        while (clipper.Step())
        {
...
static void ShowDemoWindowMisc()
{
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        ImGui::Text(
            "NavActive: %d, NavVisible: %d", io.NavActive, io.NavVisible);
        ...
        if (ImGui::TreeNode("Dragging"))
        {
            ...
            if (ImGui::IsItemActive())
            {
                ...
                draw_list->AddLine(io.MouseClickedPos[0],
                                   io.MousePos,
                ...
                ImGui::SameLine();
                ImGui::Text("Raw (%.1f, %.1f), WithLockThresold (%.1f, %.1f), "
                            "MouseDelta (%.1f, %.1f)",
                            value_raw.x,
                            value_raw.y,
        ...
        {
            const char* mouse_cursors_names[] = { "Arrow",      "TextInput",
                                                  "Move",       "ResizeNS",
                                                  "ResizeEW",   "ResizeNESW",
                                                  "ResizeNWSE", "Hand" };
            ...
            for (int i = 0; i < ImGuiMouseCursor_COUNT; i++)
            {
                char label[32];
                sprintf(
                    label, "Mouse cursor %d: %s", i, mouse_cursors_names[i]);
                ImGui::Bullet();
...
void ImGui::ShowAboutWindow(bool* p_open)
{
    ...
    if (show_config_info)
    {
        ...
        ImGui::Text("style.ItemSpacing: %.2f,%.2f",
                    style.ItemSpacing.x,
                    style.ItemSpacing.y);
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    if (ImGui::Button("Revert Ref"))
        style = *ref;
    ImGui::SameLine();
    ShowHelpMarker("Save/Revert in local non-persistent storage. Default "
                   "Colors definition are not affected. Use \"Export Colors\" "
                   "below to save them somewhere.");
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
    {
        if (ImGui::BeginTabItem("Sizes"))
        {
            ...
            ImGui::SliderFloat2("FramePadding",
                                ...
                                "%.0f");
            ImGui::SliderFloat2(
                "ItemSpacing", (float*)&style.ItemSpacing, 0.0f, 20.0f, "%.0f");
            ...
            ImGui::SliderFloat2("TouchExtraPadding",
                                ...
                                "%.0f");
            ImGui::SliderFloat(
                "IndentSpacing", &style.IndentSpacing, 0.0f, 30.0f, "%.0f");
            ImGui::SliderFloat(
                "ScrollbarSize", &style.ScrollbarSize, 1.0f, 20.0f, "%.0f");
            ImGui::SliderFloat(
                "GrabMinSize", &style.GrabMinSize, 1.0f, 20.0f, "%.0f");
            ImGui::Text("Borders");
            ImGui::SliderFloat("WindowBorderSize",
                               ...
                               "%.0f");
            ImGui::SliderFloat(
                "ChildBorderSize", &style.ChildBorderSize, 0.0f, 1.0f, "%.0f");
            ImGui::SliderFloat(
                "PopupBorderSize", &style.PopupBorderSize, 0.0f, 1.0f, "%.0f");
            ImGui::SliderFloat(
                "FrameBorderSize", &style.FrameBorderSize, 0.0f, 1.0f, "%.0f");
            ImGui::SliderFloat(
                "TabBorderSize", &style.TabBorderSize, 0.0f, 1.0f, "%.0f");
            ImGui::Text("Rounding");
            ImGui::SliderFloat(
                "WindowRounding", &style.WindowRounding, 0.0f, 12.0f, "%.0f");
            ImGui::SliderFloat(
                "ChildRounding", &style.ChildRounding, 0.0f, 12.0f, "%.0f");
            ImGui::SliderFloat(
                "FrameRounding", &style.FrameRounding, 0.0f, 12.0f, "%.0f");
            ImGui::SliderFloat(
                "PopupRounding", &style.PopupRounding, 0.0f, 12.0f, "%.0f");
            ImGui::SliderFloat("ScrollbarRounding",
                               ...
                               "%.0f");
            ImGui::SliderFloat(
                "GrabRounding", &style.GrabRounding, 0.0f, 12.0f, "%.0f");
            ImGui::SliderFloat(
                "TabRounding", &style.TabRounding, 0.0f, 12.0f, "%.0f");
            ...
            ImGui::SliderFloat2("ButtonTextAlign",
                                ...
                                "%.2f");
            ImGui::SameLine();
            ShowHelpMarker("Alignment applies when a button is larger than its "
                           "text content.");
            ...
            ImGui::SameLine();
            ShowHelpMarker(
                "Adjust if you cannot see the edges of your screen (e.g. on a "
        ...
        if (ImGui::BeginTabItem("Colors"))
        {
            ...
            ImGui::SameLine();
            ShowHelpMarker(
                "In the color list:\nLeft-click on colored square to open "
        ...
        if (ImGui::BeginTabItem("Fonts"))
        {
            ...
            ShowHelpMarker("Read FAQ and misc/fonts/README.txt for details on "
                           "font loading.");
            ImGui::PushItemWidth(120);
            for (int i = 0; i < atlas->Fonts.Size; i++)
            {
                ...
                if (font_details_opened)
                {
                    ...
                    ImGui::SameLine();
                    ShowHelpMarker(
                        "Note than the default embedded font is NOT meant to "
                        "be scaled.\n\nFont are currently rendered into "
                        "bitmaps at a given size at the time of building the "
                        "atlas. You may oversample them to get some "
                        "flexibility with scaling. You can also render at "
                        "multiple sizes and select which one to use at "
                        "runtime.\n\n(Glimmer of hope: the atlas system should "
                        "hopefully be rewritten in the future to make scaling "
                        "more natural and automatic.)");
                    ImGui::InputFloat(
                        "Font offset", &font->DisplayOffset.y, 1, 1, "%.0f");
                    ...
                    ImGui::Text("Fallback character: '%c' (%d)",
                                font->FallbackChar,
                                font->FallbackChar);
                    const float surface_sqrt =
                        sqrtf((float)font->MetricsTotalSurface);
                    ...
                    for (int config_i = 0; config_i < font->ConfigDataCount;
                        ...
                        if (const ImFontConfig* cfg =
                            ...
                            ImGui::BulletText("Input %d: \'%s\', Oversample: "
                                              "(%d,%d), PixelSnapH: %d",
                                              config_i,
                                              cfg->Name,
                                              cfg->OversampleH,
                                              ...
                                              cfg->PixelSnapH);
                    if (ImGui::TreeNode(
                            "Glyphs", "Glyphs (%d)", font->Glyphs.Size))
                    {
                        // Display all glyphs of the fonts in separate pages of
                        // 256 characters
                        for (int base = 0; base < 0x10000; base += 256)
                        {
                            ...
                            for (int n = 0; n < 256; n++)
                                count += font->FindGlyphNoFallback(
                                             ...
                                             : 0;
                            if (count > 0 &&
                                ImGui::TreeNode((void*)(intptr_t)base,
                                                "U+%04X..U+%04X (%d %s)",
                                                base,
                                                base + 255,
                                                count,
            ...
            if (ImGui::DragFloat("this window scale",
                                 &window_scale,
                                 0.005f,
                                 0.3f,
                                 2.0f,
...
struct ExampleAppConsole
{
    ...
    ExampleAppConsole()
    {
        ...
        Commands.push_back(
            "CLASSIFY"); // "classify" is only here to provide an example of
...
static void ShowExampleAppLog(bool* p_open)
{
    ...
    // We take advantage of the fact that multiple calls to Begin()/End() are
    // appending to the same window. Most of the contents of the window will be
    // added by the log.Draw() call.
            ...
            if (node_open)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (i >= 5)
                            ImGui::InputFloat(
                                "##value", &dummy_members[i], 1.0f);
                        else
                            ImGui::DragFloat(
                                "##value", &dummy_members[i], 0.01f);
                        ImGui::PopItemWidth();
                        ImGui::NextColumn();
                    }
                    ImGui::PopID();
...
static void ShowExampleAppAutoResize(bool* p_open)
{
    if (!ImGui::Begin("Example: Auto-resizing window",
                      p_open,
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    if (type == 3)
        ImGui::SetNextWindowSizeConstraints(ImVec2(400, -1),
                                            ImVec2(500, -1)); // Width 400-500
    if (type == 4)
        ImGui::SetNextWindowSizeConstraints(ImVec2(-1, 400),
                                            ImVec2(-1, 500)); // Height 400-500
    if (type == 5)
        ImGui::SetNextWindowSizeConstraints(
            ImVec2(0, 0),
    ...
    if (ImGui::Begin("Example: Constrained Resize", p_open, flags))
    {
        ...
        for (int i = 0; i < display_lines; i++)
            ImGui::Text("%*sHello, sailor! Making this line long enough for "
                        "the example.",
                        i * 4,
                        "");
    }
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::ColorEdit4("Color", &col.x);
    {
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            draw_list->AddCircle(
                ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 6, th);
            x += sz + spacing; // Hexagon
            draw_list->AddCircle(
                ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 20, th);
            x += sz + spacing; // Circle
            ...
            draw_list->AddTriangle(ImVec2(x + sz * 0.5f, y),
                                   ...
                                   th);
            x += sz + spacing;
            draw_list->AddLine(ImVec2(x, y), ImVec2(x + sz, y), col32, th);
            x += sz + spacing; // Horizontal line (note: drawing a filled
                               // rectangle will be faster!)
            draw_list->AddLine(ImVec2(x, y), ImVec2(x, y + sz), col32, th);
            x += spacing; // Vertical line (note: drawing a filled rectangle
                          // will be faster!)
            draw_list->AddLine(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, th);
        ...
        draw_list->AddCircleFilled(
            ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 6);
        x += sz + spacing; // Hexagon
        draw_list->AddCircleFilled(
            ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 32);
        x += sz + spacing; // Circle
...
struct MyDocument
{
    ...
    bool Open; // Set when the document is open (in this demo, we keep an array
               // of all available documents to simplify the demo)
...
void ShowExampleAppDocuments(bool* p_open)
{
    ...
    if (ImGui::BeginMenuBar())
    {
        if (ImGui::BeginMenu("File"))
        {
            ...
            if (ImGui::MenuItem(
                    "Close All Documents", NULL, false, open_count > 0))
                for (int doc_n = 0; doc_n < app.Documents.Size; doc_n++)
    ...
    // Submit Tab Bar and Tabs
    {
        ...
        if (ImGui::BeginTabBar("##tabs", tab_bar_flags))
        {
            ...
            for (int doc_n = 0; doc_n < app.Documents.Size; doc_n++)
            {
                ...
                bool visible =
                    ImGui::BeginTabItem(doc->Name, &doc->Open, tab_flags);
