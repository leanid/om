                             ...
                             const void*      libc_ptr)
{
    ...
    std::printf(
        "Q: куда попадут новые сегменты при dlopen дополнительной .so?\n"
        "A: в mmap-область [M] - туда же, где уже лежат наш mmap-кусок\n"
        "   и все load-time библиотеки (libc и др.). mmap-область растёт\n"
        "   ВНИЗ от верха user space: новые сегменты ставятся НИЖЕ уже\n"
        "   занятых, в дыру H~~~M со стороны M. А [heap] растёт навстречу\n"
        "   вверх: дыра между H и M - общая «взлётная полоса», и ядро\n"
        "   никогда не положит mmap внутрь кучи (и наоборот). Столкновение\n"
        "   было бы ENOMEM, но при 128 ТиБ это нереально. (Стеки потоков,\n"
        "   кстати, тоже берутся из mmap-области.)\n"
        "   Живой пример - в ../libs/: там dlopen печатает diff maps, и\n"
        "   новые сегменты .so появляются именно на адресах 0x7f...,\n"
        "   рядом с libc.\n\n");
}
