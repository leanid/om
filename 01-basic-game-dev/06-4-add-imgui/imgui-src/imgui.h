...
// Dear ImGui in your codebase. Get latest version at
...
ImDrawListFlags, ImDrawList, ImDrawData)
...
struct ImDrawChannel; // Temporary storage for ImDrawList ot output draw
                      // commands out of order, used by
...
typedef unsigned short
    ImWchar; // A single U16 character for keyboard input/display. We encode
...
// (Inside a namespace so you can add extra functions in your own separate file.
...
IMGUI_API void ShowMetricsWindow(
    bool* p_open =
        NULL); // create metrics/debug window. display Dear ImGui internals:
               // draw commands (with individual draw calls and vertices),
...
//   regardless of its return value! [this is due to legacy reason and is
//   inconsistent with most other functions such as BeginMenu/EndMenu,
//   BeginPopup/EndPopup, etc.
//    where the EndXXX call should only be called if the corresponding BeginXXX
...
//   its return value [this is due to legacy reason and is inconsistent with
//   most other functions such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc.
//   where the EndXXX call should only be called if the corresponding BeginXXX
//   function returned true.]
...
IMGUI_API void SetNextWindowBgAlpha(
    float alpha); // set next window background color alpha. helper to easily
                  // modify ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use
...
IMGUI_API void SameLine(
    ...
    float spacing_w =
        -1.0f); // call between widgets or groups to layout them horizontally. X
...
IMGUI_API void SetCursorPosX(
    float local_x); //    GetWindowPos() + GetCursorPos() ==
...
IMGUI_API bool InvisibleButton(
    const char*   str_id,
    const ImVec2& size); // button behavior without the visuals, frequently
...
IMGUI_API bool SliderFloat(
    ...
    float       v_max,
    const char* format = "%.3f",
    float power = 1.0f); // adjust format to decorate the value with a prefix or
                         // a suffix for in-slider labels or unit display. Use
...
// - If you want to use InputText() with a dynamic string type such as
// std::string or your own, see misc/cpp/imgui_stdlib.h
...
// right-clicked to open an option menu.)
// - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float*
// v', the array syntax is just a way to document the number of elements that
// are expected to be accessible. You can the pass the address of a first float
// element out of a contiguous structure, e.g. &myvector.x
...
// pressing ESCAPE.
// - Their visibility state (~bool) is held internally by imgui instead of being
// held by the programmer as we are used to with regular Begin() calls.
//   User can manipulate the visibility state by calling OpenPopup().
// (*) One can use IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup) to
// bypass it and detect hovering even when normally blocked by a popup. Those
// three properties are connected. The library needs to hold their visibility
// state because it can close popups at any time.
...
IMGUI_API bool BeginPopup(
    ...
    ImGuiWindowFlags flags =
        0); // return true if the popup is open, and you can start outputting to
            // it. only call EndPopup() if BeginPopup() returns true!
...
IMGUI_API bool OpenPopupOnItemClick(
    const char* str_id = NULL,
    int         mouse_button =
        1); // helper to open popup when clicked on last item (note: actually
...
IMGUI_API bool IsPopupOpen(
    const char* str_id); // return true if the popup is open at the current
...
IMGUI_API void SetTabItemClosed(
    const char*
        tab_or_docked_window_label); // notify TabBar or Docking system of a
                                     ...
                                     // reduce visual flicker on reorderable tab
                                     // bars). For tab-bar: call after
...
IMGUI_API const char* GetStyleColorName(
    ImGuiCol idx); // get a string corresponding to the enum value (for display,
                   // saving, etc.).
...
IMGUI_API bool IsMousePosValid(
    const ImVec2* mouse_pos = NULL); // by convention we use (-FLT_MAX,-FLT_MAX)
                                     // to denote that there is no mouse
...
IMGUI_API ImVec2 GetMouseDragDelta(
    ...
    float lock_threshold =
        -1.0f); // return the delta from the initial clicking position. This is
...
IMGUI_API void SaveIniSettingsToDisk(
    const char*
        ini_filename); // this is automatically called (if io.IniFilename is not
                       // empty) a few seconds after any modification that
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_UnsavedDocument =
        1 << 20, // Append '*' to title without affecting the ID, as a
                 ...
                 // is deferred by one frame to allow code to cancel the closure
                 // (with a confirmation popup, etc.) without flicker.
    ...
    // 1 << 17,  // --> Set io.ConfigWindowsResizeFromEdges and make sure mouse
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_UnsavedDocument =
        1 << 0, // Append '*' to title without affecting the ID, as a
                ...
                // selected on closure and closure is deferred by one frame to
                // allow code to undo it without flicker.
    ImGuiTabItemFlags_SetSelected =
        1 << 1, // Trigger flag to programatically make the tab selected when
    ...
    ImGuiTabItemFlags_NoCloseWithMiddleMouseButton =
        1 << 2, // Disable behavior of closing tabs (that are submitted with
                ...
                // this behavior on user's side with if (IsItemHovered() &&
                // IsMouseClicked(2)) *p_open = false.
    ImGuiTabItemFlags_NoPushId = 1 << 3 // Don't call PushID(tab->ID)/PopID() on
...
enum ImGuiColorEditFlags_
{
    ...
    // application defaults using SetColorEditOptions(). The intent is that you
    // probably don't want to override them in most of your calls, let the user
    // choose via the option menu and/or call SetColorEditOptions() during
    // startup.
...
// Enumateration for ImGui::SetWindow***(), SetNextWindow***(),
// SetNextTreeNode***() functions Represent a condition. Important: Treat as a
// regular enum! Do NOT combine multiple values using binary operators! All the
// functions above treat 0 as a shortcut to ImGuiCond_Always.
enum ImGuiCond_
{
...
// it so our code runs fast in debug). You generally do NOT need to care or use
// this ever. But we need to make it available in imgui.h because some of our
// data structures are relying on it. Important: clear() frees memory, resize(0)
// keep the allocated buffer. We use resize(0) a lot to intentionally recycle
// allocated buffers across frames and amortize our costs. Important: our
// implementation does NOT call C++ constructors/destructors, we treat
// everything as raw data! This is intentional but be extra mindful of that, do
// NOT use this class as a std::vector replacement in your own code! Many of the
...
struct ImGuiStyle
{
    ...
    ImVec2 SelectableTextAlign; // Alignment of selectable text when selectable
                                // is larger than text. Defaults to (0.0f, 0.0f)
                                // (top-left aligned).
...
struct ImGuiIO
{
    ...
    const char* IniFilename; // = "imgui.ini"    // Path to .ini file. Set NULL
                             // to disable automatic .ini loading/saving, if
    ...
    int KeyMap[ImGuiKey_COUNT]; // <unset>          // Map of indices into the
                                ...
                                // your "native" keyboard state.
    float KeyRepeatDelay; // = 0.250f         // When holding a key/button, time
    ...
    void* UserData; // = NULL           // Store your own data for retrieval by
                    // callbacks.

    ImFontAtlas* Fonts; // <auto>           // Load, rasterize and pack one or
    ...
    bool
        ConfigWindowsResizeFromEdges; // = true           // Enable resizing of
                                      ...
                                      // (This used to be the a per-window
                                      // ImGuiWindowFlags_ResizeFromAnySide
                                      // flag)
    bool ConfigWindowsMoveFromTitleBarOnly; // = false       // [BETA] Set to
                                            // true to only allow moving windows
                                            // when clicked+dragged from the
                                            // title bar. Windows without a
                                            // title bar are not affected.
    ...
    // Optional: Platform/Renderer back-end name (informational only! will be
    // displayed in About Window) + User data for back-end/wrappers to store
    // their own stuff.
    ...
    bool
        KeysDown[512]; // Keyboard keys that are pressed (ideally left in the
    ...
    float NavInputs[ImGuiNavInput_COUNT]; // Gamepad inputs. Cleared back to
                                          // zero by EndFrame(). Keyboard keys
                                          // will be auto-mapped and be written
                                          // here by NewFrame().
...
typedef ImGuiInputTextCallback
    ImGuiTextEditCallback; // OBSOLETE in 1.63 (from Aug 2018): made the names
                           // consistent
...
// allocated memory via the placement "new(ptr0) Type()" syntax. Defining a
...
#define IMGUI_ONCE_UPON_A_FRAME                                                \
    ...
    if (imgui_oaf) // OBSOLETED in 1.51, will remove!
#endif
...
//-----------------------------------------------------------------------------
// Draw List API (ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel,
// ImDrawListFlags, ImDrawList, ImDrawData) Hold a series of drawing commands.
// The user provides a renderer for ImDrawData which essentially contains an
// array of ImDrawList.
//-----------------------------------------------------------------------------
...
// Draw callback may be useful for example to: A) Change your GPU render state,
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedLines =
        1 << 0, // Lines are anti-aliased (*2 the number of triangles for 1.0f
                // wide line, otherwise *3 the number of triangles)
    ImDrawListFlags_AntiAliasedFill = 1
                                      << 1 // Filled shapes have anti-aliased
                                           // edges (*2 the number of vertices)
};
    ...
    IMGUI_API void ScaleClipRects(
        const ImVec2&
            fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd.
                       ...
                       // scale than ImGui expects, or if there is a difference
                       // between your window resolution and framebuffer
                       // resolution.
};
...
struct ImFontAtlas
{
    ...
    int TexGlyphPadding; // Padding between glyphs within texture in pixels.
                         ...
                         // on bilinear filtering you may set this to 0.
...
struct ImFont
{
    ...
    ImVec2 DisplayOffset; // 8     // in  // = (0,0)    // Offset font rendering
                          // by xx pixels

    // Members: Cold ~32/40 bytes
    ImFontAtlas* ContainerAtlas;    // 4-8   // out //            // What we has
                                    // been loaded into
    const ImFontConfig* ConfigData; // 4-8   // in  //            // Pointer
                                    // within ContainerAtlas->ConfigData
    short
        ConfigDataCount; // 2     // in  // ~ 1        // Number of ImFontConfig
                         // involved in creating this font. Bigger than 1 when
                         // merging multiple font sources into one ImFont.
    ImWchar FallbackChar; // 2     // in  // = '?'      // Replacement glyph if
                          // one isn't found. Only set via SetFallbackChar()
