...
typedef int ImGuiSliderFlags; // -> enum ImGuiSliderFlags_       // Flags: for
                              // SliderBehavior()
typedef int ImGuiDragFlags;   // -> enum ImGuiDragFlags_         // Flags: for
                              // DragBehavior()
...
// variety of types: signed/unsigned int/long long float/double (Exceptionally
// using templates here but we could also redefine them for variety of types)
...
// Helper: ImPool<>. Basic keyed storage for contiguous instances,
// slow/amortized insertion, O(1) indexable, O(Log N) queries by ID over a
// dense/hot buffer, Honor constructor/destructor. Add/remove invalidate all
...
template <typename T> struct IMGUI_API ImPool
{
    ...
    {
        return (p >= Data.Data && p < Data.Data + Data.Size);
    }
    void Clear()
    {
...
// and 2D, and the maintenance of some branches/patches)
struct ImVec1
{
    float x;
    ImVec1() { x = 0.0f; }
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    int     CurLenW,
        CurLenA; // we need to maintain our buffer length in both UTF-8 and
                 // wchar format. UTF-8 len is valid even if TextA is not.
    ...
    bool TextAIsValid; // temporary UTF8 buffer is not initially valid before we
                       // make the widget active (until then we pull the data
                       // from user argument)
...
{
    ImGuiTabBar* Ptr; // Either field can be set, not both. Dock node tab bars
...
struct ImGuiContext
{
    ...
    bool FrameScopePushedImplicitWindow; // Set by NewFrame(), cleared by
                                         // EndFrame()
    ...
    bool ActiveIdAllowOverlap; // Active widget allows another widget to steal
                               ...
                               // but not always)
    bool
        ActiveIdHasBeenPressed; // Track whether the active id led to a press
                                // (this is to allow changing between
                                // PressOnClick and PressOnRelease without
                                // pressing twice). Used by range_select branch.
    ...
    ImVector<ImGuiPopupRef> BeginPopupStack; // Which level of BeginPopup() we
    ...
    ImGuiID
        NavJustMovedToSelectScopeId; // Just navigated to this select scope id
                                     // (result of a successfully MoveRequest).
    ImGuiID
        NavNextActivateId; // Set by ActivateItem(), queued until next frame.
    ImGuiInputSource NavInputSource; // Keyboard or Gamepad mode? THIS WILL ONLY
                                     // BE None or NavGamepad or NavKeyboard.
    ...
    // [This is unused in this branch, but left here to facilitate
    // merging/syncing multiple branches]
    ImGuiID MultiSelectScopeId;

    // Platform support
    ImVec2 PlatformImePos; // Cursor position request & last passed to the OS
                           // Input Method Editor
        ...
        : OverlayDrawList(NULL)
    {
        ...
        NavId = NavActivateId = NavActivateDownId = NavActivatePressedId =
            NavInputId                            = 0;
        NavJustTabbedId = NavJustMovedToId = NavJustMovedToSelectScopeId =
            NavNextActivateId              = 0;
...
struct IMGUI_API ImGuiWindow
{
    ...
    short BeginOrderWithinParent; // Order within immediate parent window, if we
                                  // are a child window. Otherwise 0.
    short BeginOrderWithinContext; // Order within entire imgui context. This is
...
enum ImGuiTabItemFlagsPrivate_
{
    ImGuiTabItemFlags_NoCloseButton =
        1 << 20 // Store whether p_open is set or not, which we need to
                // recompute WidthContents during layout.
};
...
struct ImGuiTabItem
{
    ...
    int NameOffset;        // When Window==NULL, offset to name within parent
                           // ImGuiTabBar::TabsNames
    float Offset;          // Position relative to beginning of tab
    float Width;           // Width currently displayed
    float WidthContents;   // Width of actual contents, stored during
                           // BeginTabItem() call
...
struct ImGuiTabBar
{
    ...
    float  OffsetNextTab; // Distance from BarRect.Min.x, incremented with each
                          // BeginTabItem() call, not used if
    ...
    const char* GetTabName(const ImGuiTabItem* tab) const
    {
        IM_ASSERT(tab->NameOffset != -1 &&
                  tab->NameOffset < TabsNames.Buf.Size);
...
IMGUI_API bool IsPopupOpen(
    ImGuiID id); // Test for id within current popup stack level (currently
...
IMGUI_API void          TabBarQueueChangeTabOrder(ImGuiTabBar*        tab_bar,
...
IMGUI_API void RenderRectFilledRangeH(ImDrawList*   draw_list,
                                      ...
                                      float         x_end_norm,
                                      float         rounding);
IMGUI_API void RenderPixelEllipsis(ImDrawList* draw_list,
                                   ImVec2      pos,
                                   int         count,
                                   ImU32       col);
...
extern void ImGuiTestEngineHook_ItemAdd(ImGuiContext* ctx,
                                        const ImRect& bb,
                                        ImGuiID       id);
...
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS)                       \
    ImGuiTestEngineHook_ItemInfo(                                              \
        &g, _ID, _LABEL, _FLAGS) // Register status flags
#else
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS)                       \
    do                                                                         \
    {                                                                          \
