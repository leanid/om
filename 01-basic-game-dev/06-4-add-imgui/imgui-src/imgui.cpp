...
// License). This library is free but I need your support to sustain development
// and maintenance. Businesses: you can support continued maintenance and
// development via support contracts or sponsoring, see docs/README.
// Individuals: you can support continued maintenance and development via
// donations or Patreon https://www.patreon.com/imgui.
...
application?
  ...
  - How can I have multiple widgets with the same label or with an empty label?
...
Korean, Cyrillic?
  - How can I interact with standard C++ types (such as std::string and
std::vector)?
  - How can I use the drawing facilities without an ImGui window? (using
ImDrawList API)
  - How can I use Dear ImGui on a platform that doesn't have a mouse or a
keyboard? (input share, remoting, gamepad)
...
// [SECTION] MISC HELPERS/UTILITIES (Maths, String, Format, Hash, File
functions)
...
/*
 ...
 build system. It is recommended you build and statically link the .cpp files as
 part of your project and not as shared library (DLL).
 ...
 pcmd->TextureId.
                 // The vast majority of draw calls will use the imgui texture
 ...
 them to 32-bits in imconfig.h if your engine doesn't support 16-bits indices.
 ...
 - The examples/ folders contains many actual implementation of the pseudo-codes
 above.
 - When calling NewFrame(), the 'io.WantCaptureMouse', 'io.WantCaptureKeyboard'
 and 'io.WantTextInput' flags are updated. They tell you if Dear ImGui intends
 to use your inputs. When a flag is set you want to hide the corresponding
 inputs from the rest of your application. In every cases you need to pass on
 the inputs to imgui. Refer to the FAQ for more information.
 - Please read the FAQ below!. Amusingly, it is called a FAQ because people
 frequently run into the same issues!
 ...
 calculation gives you a zero value, replace it with a dummy small value!
 ...
 ImFontGlyphRangesBuilder. Keep redirection typedef (will obsolete).
 ...
 Vulkan,  etc.). old binding will still work as is, however prefer using the
 separated bindings as they will be updated to be multi-viewport conformant.
                       when adopting new bindings follow the main.cpp code of
 your preferred examples/ folder to know which functions to call.
 ...
 - 2017/09/26 (1.52) - renamed ImFont::Glyph to ImFontGlyph. Keep redirection
 ...
 GPU>; io.Fonts->TexId = YourTextureIdentifier; } you now have more flexibility
 to load multiple TTF fonts and manage the texture buffer for internal needs. it
 is now recommended that you sample the font texture with bilinear
 interpolation. (1.30) - added texture identifier in ImDrawCmd passed to your
 ...
 that you clicked in the void to unfocus its own windows. Note: The
 ...
 Those identifiers are stored and passed as ImTextureID (void*) value.
 ...
 "OK") End(); Begin("MyOtherWindow"); Button("OK");          // Label = "OK", ID
 = hash of ("MyOtherWindow", "OK") End();
 ...
 No visible label, just a checkbox!
 ...
 ID stack. Remember that ID are formed from the concatenation of _everything_
 pushed into the ID stack. At each level of the stack we store the seed used for
 items at this level of the ID stack.
 ...
 (..., "node", my_ptr, "Click") PopID(); PopID();
   ...
   - Tree nodes implicitly creates a scope for you by calling PushID().
       ...
       Button("Click");       // Label = "Click",  ID = hash of (..., "Click")
       if (TreeNode("node"))  // <-- this function call will do a PushID() for
 you (unless instructed not to, with a special flag)
       {
         Button("Click");     // Label = "Click",  ID = hash of (..., "node",
 "Click") TreePop();
       }
 ...
 A: Use the font atlas to load the TTF/OTF file you want:
    ...
    Default is ProggyClean.ttf, monospace, rendered at size 13, embedded in dear
 imgui's source code. (Tip: monospace fonts are convenient because they allow to
 facilitate horizontal alignment directly at the string level.) (Read the
 'misc/fonts/README.txt' file for more details about font loading.)
 ...
 strings. You may want to see ImFontConfig::GlyphMinAdvanceX to make your icon
 look monospace to facilitate alignment. (Read the 'misc/fonts/README.txt' file
 for more details about icons font loading.)

 Q: How can I load multiple fonts?
 A: Use the font atlas to pack them into a single texture:
    (Read the 'misc/fonts/README.txt' file and the code in ImFontAtlas for more
 details.)
 ...
 io.Fonts->AddFontFromFileTTF("myfontfile2.ttf", size_in_pixels);
      ...
      io.Fonts->AddFontFromFileTTF("fontawesome-webfont.ttf", 16.0f, &config,
 ranges); // Merge icon font io.Fonts->AddFontFromFileTTF("myfontfile.ttf",
 ...
 std::vector)? A: - Being highly portable (bindings for several languages,
 frameworks, programming style, obscure or older platforms/compilers), and
 aiming for compatibility & performance suitable for every modern real-time game
 engines, dear imgui does not use any of std C++ types. We use raw types (e.g.
 char* instead of std::string) because they adapt to more use cases.
    - To use ImGui::InputText() with a std::string or any resizable string
 class, see misc/cpp/imgui_stdlib.h.
    - To use combo boxes and list boxes with std::vector or any other data
 structure: the BeginCombo()/EndCombo() API lets you iterate and submit items
 yourself, so does the ListBoxHeader()/ListBoxFooter() API. Prefer using them
 over the old and awkward Combo()/ListBox() api.
    - Generally for most high-level types you should be able to access the
 underlying data type. You may write your own one-liner wrappers to facilitate
 user code (tip: add new functions in ImGui:: namespace from your code).
    - Dear ImGui applications often need to make intensive use of strings. It is
 expected that many of the strings you will pass to the API are raw literals
 (free in C/C++) or allocated in a manner that won't incur a large cost on your
 application. Please bear in mind that using std::string on applications with
 large amount of UI may incur unsatisfactory performances. Modern
 implementations of std::string often include small-string optimization (which
 is often a local buffer) but those are not configurable and not the same across
 implementations.
    - If you are finding your UI traversal cost to be too large, make sure your
 string usage is not leading to excessive amount of heap allocations. Consider
 using literals, statically sized buffers and your own helper functions. A
 common pattern is that you will need to build lots of strings on the fly, and
 their maximum length can be easily be scoped ahead. One possible implementation
 of a helper to facilitate printf-style building of strings:
 https://github.com/ocornut/Str This is a small helper where you can instance
 strings with configurable local buffers length. Many game engines will provide
 similar or better string helpers.

 Q: How can I use the drawing facilities without an ImGui window? (using
 ImDrawList API) A: - You can create a dummy window. Call Begin() with the
 NoBackground | NoDecoration | NoSavedSettings | NoInputs flags. (The
 ImGuiWindowFlags_NoDecoration flag itself is a shortcut for NoTitleBar |
 NoResize | NoScrollbar | NoCollapse) Then you can retrieve the ImDrawList* via
 ...
 (gamepad, input share, remote display) A: - You can control Dear ImGui with a
 gamepad. Read about navigation in "Using gamepad/keyboard navigation controls".
      (short version: map gamepad inputs into the io.NavInputs[] array + set
 io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad)
    - You can share your computer mouse seamlessly with your
 console/tablet/phone using Synergy (https://symless.com/synergy) This is the
 preferred solution for developer productivity. In particular, the
 "micro-synergy-client" repository
 (https://github.com/symless/micro-synergy-client) has simple and portable
 source code (uSynergy.c/.h) for a small embeddable client that you can use on
 any platform to connect to your host computer, based on the Synergy 1.x
 protocol. Make sure you download the Synergy 1 server on your computer. Console
 SDK also sometimes provide equivalent tooling or wrapper for Synergy-like
 protocols.
    - You may also use a third party solution such as Remote ImGui
 (https://github.com/JordiRos/remoteimgui) which sends the vertices to render
 over the local network, allowing you to use Dear ImGui even on a screen-less
 machine.
    - For touch inputs, you can increase the hit box of widgets (via the
 style.TouchPadding setting) to accommodate for the lack of precision of touch
 inputs, but it is recommended you use a mouse or gamepad to allow optimizing
 ...
 look at the Wiki, read docs/TODO.txt and see how you want to help and can help!
    - Businesses: convince your company to fund development via support
 contracts/sponsoring! This is among the most useful thing you can do for dear
 imgui.
    - Individuals: you can also become a Patron (http://www.patreon.com/imgui)
 or donate on PayPal! See README.
 ...
 gallery threads (github.com/ocornut/imgui/issues/1902). Visuals are ideal as
...
static void FreeWrapper(void* ptr0, void* user_data)
{
    IM_UNUSED(user_data);
    IM_UNUSED(ptr0);
    IM_ASSERT(0);
...
ImGuiStyle::ImGuiStyle()
{
    ...
    SelectableTextAlign = ImVec2(
        0.0f,
        0.0f); // Alignment of selectable text when button is larger than text.
    DisplayWindowPadding = ImVec2(
        19,
        19); // Window position are clamped to be visible within the display
                         ...
                         int         padding_bytes)
{
    ...
    if (padding_bytes > 0)
        memset(
            (void*)(((char*)file_data) + file_size), 0, (size_t)padding_bytes);
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo ==
        0) // Step 0: the clipper let you process the first element, regardless
           // of it being visible or not, so we can measure the element height.
    {
        DisplayStart = 0;
        DisplayEnd   = 1;
        StartPosY    = ImGui::GetCursorPosY();
    ...
    if (StepNo ==
        3) // Step 3: the clipper validate that we have reached the expected Y
...
void ImGui::RenderBullet(ImVec2 pos)
{
    ...
    window->DrawList->AddCircleFilled(
        pos, g.FontSize * 0.20f, GetColorU32(ImGuiCol_Text), 8);
}
...
// FIXME-NAV: The existence of SetNavID/SetNavIDWithRectRel/SetFocusID is
// incredibly messy and confusing and needs some explanation or refactoring.
...
void ImGui::MarkItemEdited(ImGuiID id)
{
    ...
    IM_UNUSED(
        id); // Avoid unused variable warnings when asserts are compiled out.
...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
    ...
    window->DC.CursorPos.x = (float)(int)(window->Pos.x + window->DC.Indent.x +
                                          window->DC.ColumnsOffset.x);
    window->DC.CursorPos.y = (float)(int)(window->DC.CursorPos.y + line_height +
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags)
{
    ...
    // Special handling for the dummy item after Begin() which represent the
    // title bar or tab. When the window is collapsed (SkipItems==true) that
    // last item will never be overwritten so we need to detect the case.
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    if (g.IO.MouseClicked[0])
    {
        if (g.HoveredRootWindow != NULL)
        {
            ...
            if (g.IO.ConfigWindowsMoveFromTitleBarOnly &&
                ...
                if (!g.HoveredRootWindow->TitleBarRect().Contains(
    ...
    {
        // Find the top-most window between HoveredWindow and the front most
        // Modal Window. This is where we can trim the popup stack.
...
static void ImGui::UpdateMouseInputs()
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        if (g.IO.MouseClicked[i]) // Clicking any mouse button reactivate mouse
                                  // hovering which may have been deactivated by
...
void ImGui::UpdateMouseWheel()
{
    ...
    if (g.IO.MouseWheelH != 0.0f && scroll_allowed && !g.IO.KeyCtrl)
    {
        ...
        SetWindowScrollX(scroll_window,
                         scroll_window->Scroll.x -
...
void ImGui::NewFrame()
{
    ...
    // Perform simple check: the beta io.ConfigWindowsResizeFromEdges option
    ...
    // fallback is particularly important as it avoid ImGui:: calls from
    // crashing.
                                  ...
                                  ImGuiWindow*            window)
{
    ...
    if (window->Active)
    {
        ...
        if (count > 1)
            ImQsort(window->DC.ChildWindows.begin(),
                    (size_t)count,
                    sizeof(ImGuiWindow*),
                    ChildWindowComparer);
    ...
    if (g.CurrentWindowStack.Size != 1)
    {
        if (g.CurrentWindowStack.Size > 1)
        {
            IM_ASSERT(g.CurrentWindowStack.Size == 1 &&
                      "Mismatched Begin/BeginChild vs End/EndChild calls: did "
                      "you forget to call End/EndChild?");
        ...
        else
        {
            IM_ASSERT(g.CurrentWindowStack.Size == 1 &&
                      "Mismatched Begin/BeginChild vs End/EndChild calls: did "
                      "you call End/EndChild too much?");
        }
    }
...
void ImGui::Render()
{
        ...
        if (windows_to_render_front_most[n] &&
            IsWindowActiveAndVisible(
                windows_to_render_front_most
                    [n])) // NavWindowingTarget is always temporarily displayed
                          // as the front-most window
            AddRootWindowToDrawData(windows_to_render_front_most[n]);
    g.DrawDataBuilder.FlattenIntoSingleLayer();

    // Draw software mouse cursor if requested
    if (g.IO.MouseDrawCursor)
        RenderMouseCursor(&g.OverlayDrawList,
                          g.IO.MousePos,
    ...
    // (Legacy) Call the Render callback function. The current prefer way is to
    // let the user retrieve GetDrawData() and call the render function
    // themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
static void FindHoveredWindow()
{
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        // Those seemingly unnecessary extra tests are because the code here is
        // a little different in viewport/docking branches.
...
// This is locked and return 0.0f until the mouse moves past a distance
// threshold at least once. NB: This is only valid if IsMousePosValid().
// Back-ends in theory should always keep mouse position valid when dragging
                                ...
                                ImGuiWindowFlags flags)
{
    ...
    if (size.x <= 0.0f)
        size.x = ImMax(content_avail.x + size.x,
                       4.0f); // Arbitrary minimum child size (0.0f causing too
                              // much issues)
    if (size.y <= 0.0f)
...
static ImVec2 CalcSizeContents(ImGuiWindow* window)
{
    ...
    if (window->Hidden && window->HiddenFramesForResize == 0 &&
        window->HiddenFramesRegular > 0)
        return window->SizeContents;
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    else
    {
        ...
        if (is_popup ||
            is_menu) // Popups and menus bypass style.WindowMinSize by default,
                     // but we give then a non-zero minimum size to facilitate
                     // understanding problematic cases (e.g. empty popups)
...
static void CalcResizePosSizeFromAnyCorner(ImGuiWindow*  window,
                                           ...
                                           ImVec2*       out_size)
{
    ImVec2 pos_min = ImLerp(
        corner_target, window->Pos, corner_norm); // Expected window upper-left
                                      ...
                                      ImU32         resize_grip_col[4])
{
         ...
         resize_grip_n++)
    {
        ...
        // GetOverlayDrawList(window)->AddRect(resize_rect.Min, resize_rect.Max,
        ...
        else if (held)
        {
            ...
            ImVec2 corner_target =
                ...
                ImLerp(grip.InnerDir * grip_hover_outer_size,
                       ...
                       grip.CornerPosN); // Corner of the window corresponding
                                         // to our corner grip
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        ButtonBehavior(border_rect,
                       ...
                       &held,
                       ImGuiButtonFlags_FlattenChildren);
        // GetOverlayDrawList(window)->AddRect(border_rect.Min, border_rect.Max,
        // IM_COL32(255, 255, 0, 255));
        if ((hovered &&
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Ignore zero-sized display explicitly to avoid losing positions if a
        // window manager reports zero-sized window when initializing or
        // minimizing.
                ...
                ImVec2 size_for_clamping =
                    ((g.IO.ConfigWindowsMoveFromTitleBarOnly) &&
                     !(window->Flags & ImGuiWindowFlags_NoTitleBar))
                        ...
                        : window->Size;
                window->Pos = ImMax(window->Pos + size_for_clamping, padding) -
                              size_for_clamping;
        ...
        const int resize_grip_count =
            g.IO.ConfigWindowsResizeFromEdges ? 2 : 1; // 4
        const float grip_draw_size = (float)(int)ImMax(
            g.FontSize * 1.35f,
        ...
        if (dim_bg_for_window_list && window == g.NavWindowingTargetAnim)
        {
            ...
            if (!bb.Contains(viewport_rect)) // Avoid drawing if the window
                                             // covers all the viewport anyway
                window->DrawList->AddRectFilled(
                    bb.Min,
                    bb.Max,
        ...
        else
        {
            ...
            if (flags & ImGuiWindowFlags_MenuBar)
            {
                ...
                menu_bar_rect.ClipWith(
                    window->Rect()); // Soft clipping, in particular child
                                     // window don't have minimum size covering
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            if (flags & ImGuiWindowFlags_UnsavedDocument)
            {
                ImVec2 marker_pos =
                    ImVec2(
                        ImMax(text_r.Min.x,
                              text_r.Min.x + (text_r.GetWidth() - text_size.x) *
                ...
                RenderTextClipped(marker_pos + off,
                                  ...
                                  NULL,
                                  NULL,
                                  ImVec2(0, style.WindowTitleAlign.y),
                                  &clip_rect);
            }
        }
        ...
        */
        ...
        window->InnerMainRect.Min.y = title_bar_rect.Max.y +
        ...
        window->InnerClipRect.Min.x =
            ImFloor(0.5f + window->InnerMainRect.Min.x +
                    ImMax(0.0f,
                          ImFloor(window->WindowPadding.x * 0.5f -
                                  window->WindowBorderSize)));
        ...
        // We fill last item data based on Title Bar, in order for
...
void ImGui::FocusWindow(ImGuiWindow* window)
{
    ...
    if (window->Flags &
        ImGuiWindowFlags_Popup) // FIXME: This statement should be unnecessary.
...
static const ImGuiStyleVarInfo GStyleVarInfo[] = {
    ...
    { ImGuiDataType_Float,
      1,
      (ImU32)IM_OFFSETOF(
          ImGuiStyle, ScrollbarRounding) }, // ImGuiStyleVar_ScrollbarRounding
    { ImGuiDataType_Float,
      1,
      (ImU32)IM_OFFSETOF(ImGuiStyle,
...
// programmaticaly. If you want a window to never be focused, you may use the
// e.g. NoInputs flag.
...
void ImGui::BeginTooltip()
{
    ...
    if (g.DragDropWithinSourceOrTarget)
    {
        ...
        ImVec2 tooltip_pos =
            g.IO.MousePos +
            ImVec2(16 * g.Style.MouseCursorScale, 8 * g.Style.MouseCursorScale);
...
bool ImGui::IsPopupOpen(const char* str_id)
{
    ImGuiContext& g = *GImGui;
    return g.OpenPopupStack.Size > g.BeginPopupStack.Size &&
           g.OpenPopupStack[g.BeginPopupStack.Size].PopupId ==
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window)
{
    ...
    if (ref_window)
    {
             ...
             popup_count_to_keep++)
        {
            ...
            for (int m = popup_count_to_keep;
                 m < g.OpenPopupStack.Size && !popup_or_descendent_has_focus;
                 m++)
    ...
    {
        // IMGUI_DEBUG_LOG("ClosePopupsOverWindow(%s) ->
...
void ImGui::ClosePopupToLevel(int remaining, bool apply_focus_to_window_under)
{
    ...
    // FIXME: This code is faulty and we may want to eventually to replace or
    // remove the 'apply_focus_to_window_under=true' path completely. Instead of
    // using g.OpenPopupStack[remaining-1].Window etc. we should find the
    // highest root window that is behind the popups we are closing. The current
    // code will set focus to the parent of the popup window which is incorrect.
    // It rarely manifested until now because UpdateMouseMovingWindowNewFrame()
    // would call FocusWindow() again on the clicked window, leading to a chain
    // of focusing A (clicked window) then B (parent window of the popup) then A
    // again. However if the clicked window has the _NoMove flag set we would be
    // left with B focused. For now, we have disabled this path when called from
    // ClosePopupsOverWindow() because the users of ClosePopupsOverWindow()
    // don't need to alter focus anyway, but we should inspect and fix this
    // properly.
...
// status is owned by imgui (and manipulated with e.g. OpenPopup) so the actual
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu)
    {
        ...
        if (parent_window->DC.MenuBarAppending)
            ...
            r_avoid =
                ImRect(parent_window->Pos.x + horizontal_overlap,
                       -FLT_MAX,
    ...
    if (window->Flags & ImGuiWindowFlags_Popup)
    {
        ImRect r_avoid = ImRect(window->Pos.x - 1,
                                window->Pos.y - 1,
                                window->Pos.x + 1,
                                window->Pos.y + 1);
        return FindBestWindowPosForPopupEx(window->Pos,
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip)
    {
        ...
        if (!g.NavDisableHighlight && g.NavDisableMouseHover &&
            ...
            r_avoid = ImRect(
                ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 16, ref_pos.y + 8);
        else
            r_avoid = ImRect(
                ref_pos.x - 16,
                ref_pos.y - 8,
                ...
                ref_pos.y +
                    24 *
                        sc); // FIXME: Hard-coded based on mouse cursor shape
                             // expectation. Exact dimension not very important.
        ImVec2 pos = FindBestWindowPosForPopupEx(ref_pos,
                                                 window->Size,
                                                 &window->AutoPosLastDirection,
                                                 r_outer,
                                                 r_avoid);
        if (window->AutoPosLastDirection == ImGuiDir_None)
            pos =
                ref_pos +
                ImVec2(2,
                       2); // If there's not enough room, for tooltip we prefer
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    NavClampRectToVisibleAreaForMoveDir(
        g.NavMoveClipDir, cand, window->ClipRect);

    // Compute distance between boxes
    // FIXME-NAV: Introducing biases for vertical navigation, needs to be
    // removed.
    float dbx = NavScoreItemDistInterval(
        cand.Min.x, cand.Max.x, curr.Min.x, curr.Max.x);
    ...
    else
    {
        ...
        quadrant =
            (window->DC.LastItemId < g.NavId) ? ImGuiDir_Left : ImGuiDir_Right;
    }
    ...
    if (ImGui::IsMouseHoveringRect(cand.Min, cand.Max))
    {
        ...
        draw_list->AddRectFilled(cand.Max - ImVec2(4, 4),
                                 cand.Max + ImGui::CalcTextSize(buf) +
                                     ImVec2(4, 4),
                                 IM_COL32(40, 0, 0, 150));
        draw_list->AddText(g.IO.FontDefault, 13.0f, cand.Max, ~0U, buf);
    }
    else if (g.IO.KeyCtrl) // Hold to preview score in matching quadrant. Press
                           // C to rotate.
    {
        ...
        {
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), "%.0f/%.0f", dist_box, dist_center);
            ImDrawList* draw_list = ImGui::GetOverlayDrawList(window);
            draw_list->AddRectFilled(
                cand.Min, cand.Max, IM_COL32(255, 0, 0, 200));
            draw_list->AddText(g.IO.FontDefault,
                               13.0f,
                               cand.Min,
                                  ...
                                  const ImGuiID id)
{
        ...
        !(item_flags & ImGuiItemFlags_NoNav))
    {
        ...
        if ((g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet) &&
            ...
            if (ImClamp(nav_bb.Max.y,
                        window->ClipRect.Min.y,
                        window->ClipRect.Max.y) -
                    ImClamp(nav_bb.Min.y,
                            window->ClipRect.Min.y,
                            window->ClipRect.Max.y) >=
...
static void NavRestoreLayer(ImGuiNavLayer layer)
{
    ...
    if (layer == 0 && g.NavWindow->NavLastIds[0] != 0)
        ImGui::SetNavIDWithRectRel(
            g.NavWindow->NavLastIds[0], layer, g.NavWindow->NavRectRel[0]);
    else
...
static ImVec2 ImGui::NavCalcPreferredRefPos()
{
    ...
    else
    {
        ...
        // arbitrary position around the bottom left of the currently navigated
        // item.
        ...
        return ImFloor(ImClamp(
            ...
            visible_rect.Min,
            visible_rect
                .Max)); // ImFloor() is important because non-integer mouse
                        // position application in back-end might be lossy and
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
    ...
    if (mode == ImGuiInputReadMode_Down)
        return g.IO.NavInputs[n]; // Instant, read analog input (0.0f..1.0f, as
                                  // provided by user)

    const float t = g.IO.NavInputsDownDuration[n];
    if (t < 0.0f &&
        mode == ImGuiInputReadMode_Released) // Return 1.0f when just released,
                                             // no repeat, ignore analog input.
        return (g.IO.NavInputsDownDurationPrev[n] >= 0.0f ? 1.0f : 0.0f);
    if (t < 0.0f)
        return 0.0f;
    if (mode == ImGuiInputReadMode_Pressed) // Return 1.0f when just pressed, no
    ...
    if (mode == ImGuiInputReadMode_Repeat)
        return (float)CalcTypematicPressedRepeatAmount(
            t,
            t - g.IO.DeltaTime,
            g.IO.KeyRepeatDelay * 0.80f,
            g.IO.KeyRepeatRate * 0.80f);
    if (mode == ImGuiInputReadMode_RepeatSlow)
        return (float)CalcTypematicPressedRepeatAmount(
            t,
            t - g.IO.DeltaTime,
            g.IO.KeyRepeatDelay * 1.00f,
                                         ...
                                         const ImRect& item_rect)
{
    ...
    // GetOverlayDrawList(window)->AddRect(window_rect.Min, window_rect.Max,
...
#define NAV_MAP_KEY(_KEY, _NAV_INPUT)                                          \
    ...
    {                                                                          \
        ...
        if (g.IO.KeyAlt &&
            !g.IO.KeyCtrl) // AltGR is Alt+Ctrl, also even on keyboards without
                           // AltGR we don't want Alt+Ctrl to open menu.
            g.IO.NavInputs[ImGuiNavInput_KeyMenu_] = 1.0f;
#undef NAV_MAP_KEY
    }
    memcpy(g.IO.NavInputsDownDurationPrev,
        ...
        (!g.NavDisableHighlight || g.NavInitRequestFromMove))
    {
        ...
        if (g.NavInitRequestFromMove)
            SetNavIDWithRectRel(
                g.NavInitResultId, g.NavLayer, g.NavInitResultRectRel);
        else
        ...
        !g.NavWindowingTarget)
    {
        ...
        if (scroll_dir.x != 0.0f && window->ScrollbarX)
        {
            SetWindowScrollX(
                window,
                ImFloor(window->Scroll.x + scroll_dir.x * scroll_speed));
        ...
        if (scroll_dir.y != 0.0f)
        {
            SetWindowScrollY(
                window,
                ImFloor(window->Scroll.y + scroll_dir.y * scroll_speed));
            g.NavMoveFromClampedRefRect = true;
        }
    }
    ...
    if (g.NavWindow)
    {
        for (int layer = 0; layer < 2; layer++)
            GetOverlayDrawList(g.NavWindow)
                ->AddRect(g.NavWindow->Pos + g.NavWindow->NavRectRel[layer].Min,
                          g.NavWindow->Pos + g.NavWindow->NavRectRel[layer].Max,
                          IM_COL32(255, 200, 0, 255));
    } // [DEBUG]
    if (g.NavWindow)
    {
        ImU32  col = (!g.NavWindow->Hidden) ? IM_COL32(255, 0, 255, 255)
                                            : IM_COL32(255, 0, 0, 255);
        ImVec2 p   = NavCalcPreferredRefPos();
        char   buf[32];
        ImFormatString(buf, 32, "%d", g.NavLayer);
        GetOverlayDrawList(g.NavWindow)->AddCircleFilled(p, 3.0f, col);
        GetOverlayDrawList(g.NavWindow)
            ->AddText(NULL, 13.0f, p + ImVec2(8, -4), col, buf);
    }
#endif
}
...
static void ImGui::NavUpdateMoveResult()
{
    ...
    if (g.NavMoveResultLocal.ID == 0 && g.NavMoveResultOther.ID == 0)
    {
        // In a situation when there is no results but NavId != 0, re-enable the
    ...
    if (g.NavId != result->ID)
    {
        ...
        // happen with ImGuiNavMoveFlags_AllowCurrentNavId)
        ...
        if (page_up_held != page_down_held) // If either (not both) are pressed
        {
                ...
                window->DC.NavHasScroll)
            {
                ...
                if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
                    SetWindowScrollY(window,
                                     window->Scroll.y -
                                         window->InnerClipRect.GetHeight());
...
void ImGui::NavUpdateWindowingList()
{
    ...
    SetNextWindowPos(
        g.IO.DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
                            ...
                            const char*   text_end)
{
    ...
    if (g.LogDepthRef >
        window->DC.TreeDepth) // Re-adjust padding if we have popped out of our
                              // starting depth
    ...
    for (;;)
    {
        ...
        // trailing \n to allow a subsequent item on the same line to be
        // captured.
        ...
        {
            // An empty "" string at a different Y position should output a
            // carriage return.
            LogText(IM_NEWLINE);
            break;
        }
                                            ...
                                            const char* name)
{
    ImGuiWindowSettings* settings =
        ImGui::FindWindowSettings(ImHashStr(name, 0));
    if (!settings)
                                           ...
                                           ImGuiTextBuffer*      buf)
{
    ...
    for (int i = 0; i != g.SettingsWindows.Size; i++)
    {
        ...
        if (const char* p = strstr(
                name, "###")) // Skip to the "###" marker if any. We don't skip
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    ImGui::Text("%d active windows (%d visible)",
    ...
    struct Funcs
    {
                                 ...
                                 const char*  label)
        {
            ...
            ImDrawList* overlay_draw_list =
                GetOverlayDrawList(window); // Render additional visuals into
                 ...
                 elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                if (show_draw_cmd_clip_rects && ImGui::IsItemHovered())
                {
                    ...
                    for (int i = elem_offset;
                         ...
                         i++)
                        vtxs_rect.Add(
                            draw_list->VtxBuffer[idx_buffer ? idx_buffer[i] : i]
                                .pos);
                    clip_rect.Floor();
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label)
        {
            ...
            ImGui::BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)",
                              window->Scroll.x,
                              GetWindowScrollMaxX(window),
                              window->Scroll.y,
                              GetWindowScrollMaxY(window));
            ImGui::BulletText(
                ...
                for (int n = 0; n < window->ColumnsStorage.Size; n++)
                {
                            ...
                            columns->Flags))
                    {
                        ImGui::BulletText(
                            ...
                            columns->MinX,
                            columns->MaxX);
                        for (int column_n = 0; column_n < columns->Columns.Size;
                             column_n++)
                            ImGui::BulletText(
        ...
        {
            // Standalone tab bars (not associated to docking/windows
            // functionality) currently hold no discernible strings.
            char        buf[256];
            char*       p       = buf;
            ...
            ImFormatString(
                ...
                "TabBar (%d tabs)%s",
                tab_bar->Tabs.Size,
                (tab_bar->PrevFrameVisible < ImGui::GetFrameCount() - 2)
                    ? " *Inactive*"
                    : "");
    ...
    if (ImGui::TreeNode("DrawList",
                        "Active DrawLists (%d)",
                        g.DrawDataBuilder.Layers[0].Size))
    {
        for (int i = 0; i < g.DrawDataBuilder.Layers[0].Size; i++)
            Funcs::NodeDrawList(
                NULL, g.DrawDataBuilder.Layers[0][i], "DrawList");
        ImGui::TreePop();
    }
    ...
    if (g.IO.KeyCtrl && show_window_begin_order)
    {
        for (int n = 0; n < g.Windows.Size; n++)
        {
            ...
            char buf[32];
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), "%d", window->BeginOrderWithinContext);
            float       font_size         = ImGui::GetFontSize() * 2;
            ImDrawList* overlay_draw_list = GetOverlayDrawList(window);
            overlay_draw_list->AddRectFilled(window->Pos,
                                             window->Pos +
