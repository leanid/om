...
namespace om {
    ...
    matrix::matrix()
            : row0(1.0f, 0.f), row1(0.f, 1.f), row2(0.f, 0.f) {
    }
    ...
    sound_buffer_impl::sound_buffer_impl(std::string_view path,
        ...
        if (file_audio_spec.channels != device_audio_spec.channels ||
            ...
            if (0 != convert_status) {
                ...
                message << "failed to convert WAV byte stream: "
                        << SDL_GetError();
    ...
    private:
        GLuint compile_shader(GLenum shader_type, std::string_view src) {
            ...
            if (compiled_status == 0) {
                ...
                glGetShaderInfoLog(shader_id, info_len, NULL,
                                   info_chars.data());
                OM_GL_CHECK();
...
/// return true if more events in queue
    ...
    static void initialize_internal(std::string_view title,
...

        {
            ...
            {
                const SDL_DisplayMode *dispale_mode = SDL_GetCurrentDisplayMode(
                        1);
                if (!dispale_mode) {
                    std::cout << "can't get current display mode: "
            ...
            window = SDL_CreateWindow(
                    title.data(), window_size_w, window_size_h,
                    ::SDL_WINDOW_OPENGL);
            ...
            if (num_audio_devices > 0) {
                default_audio_device_name =
                        SDL_GetAudioDeviceName(num_audio_devices - 1,
                                               SDL_FALSE);
        ...
        already_exist = true;
        // example:
        // on my Samsung S23 Ultra path to files
        // /storage/emulated/0/Android/data/org.home.leo/files/test_output_file.txt
        std::string internal_path = SDL_AndroidGetExternalStoragePath();
        std::cout << "internal_path: " << internal_path << std::endl;

        {
            std::ofstream out(internal_path + "/test_output_file.txt", std::ios::binary);
            std::cout << "is file opened: " << std::boolalpha << out.is_open()
                      << std::endl;
            out << "hello world!\n" << std::endl;
        }

        std::cout << "test_output_file.txt just write to disk" << std::endl;

        {
            std::ifstream in(internal_path + "/test_output_file.txt", std::ios::binary);
            std::cout << "is file opened: " << std::boolalpha << in.is_open()
                      << std::endl;
            std::string line;
            std::getline(in, line);
            std::cout << "content from file [test_output_file.txt] " << line
                      << std::endl;
        }
    }
    ...
    void
    initialize(std::string_view title, const window_mode &desired_window_mode) {
