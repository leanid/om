...
template <class T>
DOCTEST_CONSTEXPR_FUNC T&& forward(
    typename types::remove_reference<T>::type&& t) DOCTEST_NOEXCEPT
{
    return static_cast<T&&>(t);
}
...
struct has_global_insertion_operator<
    T,
    decltype(::operator<<(declval<std::ostream&>(), declval<const T&>()),
...
DOCTEST_INTERFACE bool decomp_assert(assertType::Enum at,
                                     const char*      file,
                                     int              line,
                                     const char*      expr,
                                     const Result&    result);
...
template <int comparison, typename L, typename R>
DOCTEST_NOINLINE bool binary_assert(assertType::Enum at,
                                    const char*      file,
                                    int              line,
                                    const char*      expr,
...
template <typename L>
DOCTEST_NOINLINE bool unary_assert(assertType::Enum at,
                                   const char*      file,
                                   int              line,
                                   const char*      expr,
...
#define DOCTEST_TYPE_TO_STRING_AS(str, ...)                                    \
    namespace doctest                                                          \
    {                                                                          \
    template <> inline String toString<__VA_ARGS__>()                          \
    {                                                                          \
        return str;                                                            \
    }                                                                          \
    }                                                                          \
...
#define DOCTEST_SUBCASE(name)                                                  \
    if (const doctest::detail::Subcase& DOCTEST_ANONYMOUS(                     \
            DOCTEST_ANON_SUBCASE_) DOCTEST_UNUSED =                            \
...
#define DOCTEST_WARN_THROWS_WITH(expr, with, ...)                              \
    ...
    {                                                                          \
        static_assert(false,                                                   \
                      "Exception translation is not available when doctest "   \
                      "is disabled.");                                         \
        return false;                                                          \
    }()
...
bool decomp_assert(assertType::Enum at,
                   const char*      file,
                   int              line,
                   const char*      expr,
                   const Result&    result)
