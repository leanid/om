...
SampleAnalysis analyse(const IConfig& cfg, FDuration* first, FDuration* last)
{
    ...
    else
    {
        ...
        return SampleAnalysis{ CATCH_MOVE(samples),
...
public: // IMutableRegistryHub
    ...
    {
        m_testCaseRegistry.registerTest(CATCH_MOVE(testInfo),
                                        CATCH_MOVE(invoker));
    }
...
public:
        ...
        , m_context{ config, CATCH_MOVE(reporter) }
    {
        ...
        m_tests = createShard(
            m_tests, m_config->shardCount(), m_config->shardIndex());
    }
...
Session::Session()
{
    ...
    if (!exceptions.empty())
    {
        ...
        for (const auto& ex_ptr : exceptions)
        {
            ...
            {
                errStream->stream()
                    << TextFlow::Column(ex.what()).indent(2) << '\n';
            }
        }
    }
#endif
    ...
    SourceLineInfo const& _lineInfo)
{
    return Detail::make_unique<TestCaseInfo>(
        _className, nameAndTags, _lineInfo);
}
...
std::map<std::string, std::string> const& ReporterConfig::customOptions() const
{
                                     ...
                                     std::string&&     message)
{
    m_resultCapture.handleMessage(
        m_assertionInfo, resultType, CATCH_MOVE(message), m_reaction);
}
        ...
        if (isOptPrefix(next[0]))
        {
            ...
            else
            {
                if (next.size() > 1 && next[1] != '-' && next.size() > 2)
                {
                    ...
                    {
                        m_tokenBuffer.push_back(
                            { TokenType::Option, next.substr(i, 1) });
                    }
                }
                else
                {
...
void FatalConditionHandler::disengage_platform() noexcept
{
        ...
        topLevelExceptionFilter)
    {
        Catch::cerr()
            ...
            << " The filter was restored, but might be rolled back "
               "unexpectedly.";
    }
...
public:
    ...
    std::string getContents()
    {
        ...
        while (current_pos > 0)
        {
            auto read_characters = std::fread(
                buffer, 1, std::min(buffer_size, current_pos), m_file);
...
Optional<ReporterSpec> parseReporterSpec(StringRef reporterSpec)
{
    ...
    for (size_t i = 1; i < parts.size(); ++i)
    {
        ...
        else if (key[0] == 'X')
        {
            ...
            auto ret =
                kvPairs.emplace(std::string(kv.key), std::string(kv.value));
            if (!ret.second)
            {
...
RunContext::RunContext(IConfig const* _config, IEventListenerPtr&& reporter)
    ...
    , m_outputRedirect(
          makeOutputRedirect(m_reporter->getPreferences().shouldRedirectStdOut))
...
void RunContext::handleFatalErrorCondition(StringRef message)
{
    ...
    // Since this is a fatal error, we have not had and won't have the
    // opportunity to destruct them properly
    while (!m_activeSections.empty())
    {
        auto           nl = m_activeSections.back()->nameAndLocation();
        SectionEndInfo endInfo{
            SectionInfo(CATCH_MOVE(nl.location), CATCH_MOVE(nl.name)), {}, 0.0
        };
...
Section::~Section()
{
    if (m_sectionIncluded)
    {
        SectionEndInfo endInfo{ CATCH_MOVE(m_info),
                                m_assertions,
                                          ...
                                          const_iterator end) const
{
    ...
    // 0xff back to 'm'
    auto str = std::string(
        begin == this->begin() ? m_string.begin() : begin.m_it, end.m_it);
    ...
    for (std::size_t idx = 0; idx < m_str.size(); ++idx)
    {
        ...
        switch (c)
        {
            ...
            default:
                ...
                {
                    unsigned char nc =
                        static_cast<unsigned char>(m_str[idx + n]);
                    valid &= ((nc & 0xC0) == 0x80);
...
XmlWriter::ScopedElement& XmlWriter::ScopedElement::writeText(StringRef text,
                                                              XmlFormatting fmt)
{
...
struct BySectionInfo
{
    ...
    bool operator()(
        Detail::unique_ptr<CumulativeReporterBase::SectionNode> const& node)
        const
    {
...
void JsonReporter::testCaseEnded(TestCaseStats const& tcStats)
{
...

    {
        ...
        // We do not write the test case totals, because there will always be
        // just one test case here.
