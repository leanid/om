...
// mask themselves as GCC should be ignored.
    ...
    !defined(__CUDACC__) && !defined(__LCC__) && !defined(__NVCOMPILER)
#define CATCH_INTERNAL_START_WARNINGS_SUPPRESSION _Pragma("GCC diagnostic push")
#define CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION _Pragma("GCC diagnostic pop")

// This only works on GCC 9+. so we have to also add a global suppression of
// Wparentheses for older versions of GCC.
...
#define CATCH_INTERNAL_SUPPRESS_UNUSED_VARIABLE_WARNINGS                       \
    _Pragma("diag_suppress declared_but_not_referenced")
#endif
...
#if defined(CATCH_PLATFORM_WINDOWS) || defined(CATCH_PLATFORM_PLAYSTATION) ||  \
    defined(__CYGWIN__) || defined(__QNX__) || defined(__EMSCRIPTEN__) ||      \
    defined(__DJGPP__) || defined(__OS400__)
...
// Assume that some platforms do not support getenv.
    ...
    defined(CATCH_PLATFORM_PLAYSTATION) || defined(_GAMING_XBOX)
...
#if defined(CATCH_INTERNAL_CONFIG_GETENV) &&                                   \
    !defined(CATCH_INTERNAL_CONFIG_NO_GETENV) &&                               \
    !defined(CATCH_CONFIG_NO_GETENV) && !defined(CATCH_CONFIG_GETENV)
...
#if defined(CATCH_PLATFORM_WINDOWS) && !defined(CATCH_CONFIG_COLOUR_WIN32) &&  \
...
struct ResultWas
{
    enum OfType
    {
        ...
        Info    = 1,
        Warning = 2,
        // TODO: Should explicit skip be considered "not OK" (cf. isOk)? I.e.,
        // should it have the failure bit?
        ExplicitSkip = 4,
...
template <typename Duration = Benchmark::FDuration> struct BenchmarkStats;
...
public:
    ...
    virtual IGeneratorTracker* acquireGeneratorTracker(
        StringRef generatorName, SourceLineInfo const& lineInfo) = 0;
...
public:
    ...
    virtual void registerListener(
        Detail::unique_ptr<EventListenerFactory> factory)                 = 0;
    ...
    virtual void registerTranslator(
        Detail::unique_ptr<IExceptionTranslator>&& translator)         = 0;
...
namespace Benchmark
{
template <typename Type> struct Estimate
{
    Type   point;
    Type   lower_bound;
    Type   upper_bound;
...
template <class Dummy> struct BenchmarkStats
{
    BenchmarkInfo info;
...
inline auto invoke_deoptimized(Fn&& fn, Args&&... args)
    -> std::enable_if_t<!std::is_same<void, decltype(fn(args...))>::value>
{
...
template <typename Func, typename... U>
using FunctionReturnType =
    std::remove_reference_t<std::remove_cv_t<std::result_of_t<Func(U...)>>>;
#endif
...
struct ChronometerModel final : public ChronometerConcept
{
    ...
    {
        return std::chrono::duration_cast<std::chrono::nanoseconds>(finished -
                                                                    started);
    }
...
template <typename Fun> repeater<std::decay_t<Fun>> repeat(Fun&& fun)
{
...
namespace Benchmark
{
template <typename Result> struct Timing
{
    IDuration elapsed;
    Result    result;
    int       iterations;
...
TimingOf<Fun, Args...> measure(Fun&& fun, Args&&... args)
{
    ...
    auto&& r =
        Detail::complete_invoke(CATCH_FORWARD(fun), CATCH_FORWARD(args)...);
    auto end   = Clock::now();
...
template <typename Clock, typename Fun>
TimingOf<Fun, run_for_at_least_argument_t<Clock, Fun>> run_for_at_least(
    IDuration how_long, const int initial_iterations, Fun&& fun)
{
...
template <typename Clock> std::vector<double> resolution(int k)
{
    ...
    {
        deltas.push_back(
            static_cast<double>((times[idx] - times[idx - 1]).count()));
    }
...
EnvironmentEstimate estimate_clock_cost(FDuration resolution)
{
    ...
    auto&& r =
        run_for_at_least<Clock>(clock_cost_estimation_time, iters, time_clock);
...
template <typename Clock> Environment measure_environment()
{
#if defined(__clang__)
...
#pragma clang diagnostic pop
    ...
    env = Catch::Detail::make_unique<Environment>(
...
struct Benchmark
{
    ...
    template <typename Clock = default_clock> void run()
    {
        ...
        CATCH_TRY
        {
            ...
            BenchmarkInfo info{
                CATCH_MOVE(name),           plan.estimated_duration.count(),
            ...
            auto analysis = Detail::analyse(
                *cfg, samples.data(), samples.data() + samples.size());
            BenchmarkStats<> stats{
                CATCH_MOVE(info),  CATCH_MOVE(analysis.samples),
                analysis.mean,     analysis.standard_deviation,
    ...
    template <
        typename Fun,
        std::enable_if_t<!Detail::is_related<Fun, Benchmark>::value, int> = 0>
...
#define CATCH_BENCHMARK(...)                                                   \
    INTERNAL_CATCH_BENCHMARK(                                                  \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_BENCHMARK_),                \
        INTERNAL_CATCH_GET_1_ARG(__VA_ARGS__, , ),                             \
        INTERNAL_CATCH_GET_2_ARG(__VA_ARGS__, , ))
#define CATCH_BENCHMARK_ADVANCED(name)                                         \
    INTERNAL_CATCH_BENCHMARK_ADVANCED(                                         \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_BENCHMARK_), name)
...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
    ...
    T const& stored_object() const { return *reinterpret_cast<T const*>(data); }
#if defined(__GNUC__) && __GNUC__ <= 6
...
template <typename T>
std::enable_if_t<std::is_enum<T>::value, std::string> convertUnstreamable(
    T const& value)
{
...
template <typename T, typename = void> struct StringMaker
{
    ...
    static std::enable_if_t<::Catch::Detail::IsStreamInsertable<Fake>::value,
                            std::string>
...
template <size_t SZ> struct StringMaker<char[SZ]>
{
    static std::string convert(char const* str)
    {
...
template <size_t SZ> struct StringMaker<signed char[SZ]>
{
...
template <size_t SZ> struct StringMaker<unsigned char[SZ]>
{
...
{
    static std::string convert(const std::nullopt_t&) { return "{ }"; }
};
} // namespace Catch
...
#define INTERNAL_CATCH_INFO(macroName, log)                                    \
    const Catch::ScopedMessage INTERNAL_CATCH_UNIQUE_NAME(scopedMessage)(      \
...
struct is_unary_function<
    F,
    Catch::Detail::void_t<decltype(std::declval<F>()(fake_arg()))>>
    : std::true_type
{
};
...
public:
    ...
    template <typename U> static auto ok(U&& value) -> BasicResult
    {
...
public:
    ...
    Detail::InternalParseResult parse(
        std::string const&, Detail::TokenStream tokens) const override;
};
...
public:
    ...
    template <typename T> friend Parser operator|(Parser const& p, T&& rhs)
    {
        Parser temp(p);
        temp |= rhs;
        return temp;
    ...
    template <typename T> friend Parser operator|(Parser&& p, T&& rhs)
    {
        p |= CATCH_FORWARD(rhs);
        return CATCH_MOVE(p);
    }
    ...
    using ParserBase::parse;
    Detail::InternalParseResult parse(
        std::string const& exeName, Detail::TokenStream tokens) const override;
};
...
#define CATCH_DEFINE_COMPARABLE_TRAIT(id, op)                                  \
    ...
    template <typename T>                                                      \
    struct is_##id##_0_comparable<T, void_t<decltype(std::declval<T>() op 0)>> \
        : std::true_type                                                       \
    {                                                                          \
    };
...
template <typename T>
struct capture_by_value : std::integral_constant<bool, std::is_arithmetic<T>{}>
{
};

#if defined(CATCH_CONFIG_CPP20_COMPARE_OVERLOADS)
template <> struct capture_by_value<std::strong_ordering> : std::true_type
{
};
template <> struct capture_by_value<std::weak_ordering> : std::true_type
{
};
template <> struct capture_by_value<std::partial_ordering> : std::true_type
{
};
#endif
...
public:
    ...
    constexpr ITransientExpression& operator=(ITransientExpression const&) =
        default;
...
public:
    constexpr BinaryExpr(bool      comparisonResult,
                         LhsT      lhs,
                         StringRef op,
                         RhsT      rhs)
...
public:
    ...
    template <typename T> constexpr void handleExpr(ExprLhs<T> const& expr)
    {
...
#define CATCH_INTERNAL_STRINGIFY(...)                                          \
    "Disabled by CATCH_CONFIG_DISABLE_STRINGIFICATION"_catch_sr
#endif
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                 \
    ...
    { /* NOLINT(bugprone-infinite-loop) */                                     \
        /* The expression should not be evaluated, but warnings should         \
         * hopefully be checked */                                             \
...
template <typename C> class TestInvokerFixture : public ITestInvoker
{
...
public:
    ...
    void prepareTestCase() override { m_fixture = Detail::make_unique<C>(); }

    void tearDownTestCase() override { m_fixture.reset(); }
...
template <typename C>
Detail::unique_ptr<ITestInvoker> makeTestInvokerFixture(
...
#define INTERNAL_CATCH_TESTCASE(...)                                           \
    INTERNAL_CATCH_TESTCASE2(                                                  \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEST_), __VA_ARGS__)

#else // ^^ !CATCH_CONFIG_EXPERIMENTAL_STATIC_ANALYSIS_SUPPORT | vv
...
#elif defined(CATCH_CONFIG_PREFIX_ALL) &&                                      \
    defined(CATCH_CONFIG_DISABLE) // ^^ prefixed, implemented | vv prefixed,
                                  // disabled
...
#define CATCH_SCENARIO_METHOD(className, ...)                                  \
    INTERNAL_CATCH_TESTCASE_METHOD_NO_REGISTRATION(                            \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEST_), className)
...
#elif !defined(CATCH_CONFIG_PREFIX_ALL) &&                                     \
    !defined(CATCH_CONFIG_DISABLE) // ^^ prefixed, disabled | vv unprefixed,
                                   // implemented
...
#elif !defined(CATCH_CONFIG_PREFIX_ALL) &&                                     \
    defined(CATCH_CONFIG_DISABLE) // ^^ unprefixed, implemented | vv unprefixed,
                                  // disabled
...
#if defined(__GNUC__)
// We need to silence "empty __VA_ARGS__ warning", and using just _Pragma does
// not work
...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                  \
    ...
    template <template <typename...> class Final,                              \
              template <INTERNAL_CATCH_REMOVE_PARENS(                          \
                  signature)> class... Containers,                             \
              typename... Types>                                               \
    struct create<Final,                                                       \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION(Name, Tags, ...)     \
    INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(                       \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_),            \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_),            \
        Name,                                                                  \
        Tags,                                                                  \
        typename TestType,                                                     \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION(              \
    ...
    INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION_2(                \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_CLASS_),      \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_),            \
        ClassName,                                                             \
        Name,                                                                  \
        Tags,                                                                  \
        ...
        {                                                                      \
            ...
            (void)expander{ (                                                  \
                Catch::AutoReg(                                                \
                    ...
                    Catch::StringRef(),                                        \
                    Catch::NameAndTags{                                        \
                        Name " - " INTERNAL_CATCH_STRINGIZE(TmplList) " - " +  \
                            std::to_string(index),                             \
                        Tags }),                                               \
                index++)... }; /* NOLINT */                                    \
        }                                                                      \
...
template <typename T> class IGenerator : public GeneratorUntypedBase
{
...
public:
    ...
    bool     next() { return m_generator->countedNext(); }
};
...
GeneratorWrapper<T> values(std::initializer_list<T> values)
{
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<FixedValuesGenerator<T>>(values));
}
...
template <typename T> class Generators : public IGenerator<T>
{
    ...
    void add_generator(T const& val) { m_generators.emplace_back(value(val)); }
    void add_generator(T&& val)
    {
        m_generators.emplace_back(value(CATCH_MOVE(val)));
    }
    template <typename U>
    std::enable_if_t<!std::is_same<std::decay_t<U>, T>::value> add_generator(
        U&& val)
    {
...
{
    return makeGenerators(value(T(CATCH_FORWARD(val))),
                          CATCH_FORWARD(moreGenerators)...);
}
...
#define CATCH_INTERNAL_GENERATOR_STRINGIZE(...)                                \
    CATCH_INTERNAL_GENERATOR_STRINGIZE_IMPL(__VA_ARGS__)

#define GENERATE(...)                                                          \
    Catch::Generators::generate(CATCH_INTERNAL_GENERATOR_STRINGIZE(            \
...
template <typename T> class TakeGenerator final : public IGenerator<T>
{
...
{
    return GeneratorWrapper<T>(Catch::Detail::make_unique<TakeGenerator<T>>(
        target, CATCH_MOVE(generator)));
}
...
{
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<FilterGenerator<T, Predicate>>(
            CATCH_FORWARD(pred), CATCH_MOVE(generator)));
}

template <typename T> class RepeatGenerator final : public IGenerator<T>
{
    static_assert(!std::is_same<T, bool>::value,
...
{
    return GeneratorWrapper<T>(Catch::Detail::make_unique<RepeatGenerator<T>>(
        repeats, CATCH_MOVE(generator)));
}
...
{
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<MapGenerator<T, U, Func>>(
            CATCH_FORWARD(function), CATCH_MOVE(generator)));
}
                                       ...
                                       GeneratorWrapper<T>&& generator)
{
    return GeneratorWrapper<std::vector<T>>(
        Catch::Detail::make_unique<ChunkGenerator<T>>(size,
...
#define CATCH_CONFIG_INTERNAL_MSVC_UMUL128
    ...
    !defined(CATCH_CONFIG_NO_UINT128) && !defined(CATCH_CONFIG_UINT128)
...
template <std::size_t> struct SizedUnsignedType;
#define SizedUnsignedTypeHelper(TYPE)                                          \
    template <> struct SizedUnsignedType<sizeof(TYPE)>                         \
    {                                                                          \
        using type = TYPE;                                                     \
    }
...
template <typename T> struct ExtendedMultResult
{
    T              upper;
    T              lower;
 ...
 */
constexpr ExtendedMultResult<std::uint64_t> extendedMultPortable(
...
inline ExtendedMultResult<std::uint64_t> extendedMult(std::uint64_t lhs,
                                                      std::uint64_t rhs)
{
...
constexpr ExtendedMultResult<UInt> extendedMult(UInt lhs, UInt rhs)
{
    ...
    return { static_cast<UInt>(result >> (CHAR_BIT * sizeof(UInt))),
...
fillBitsFrom(Generator& gen)
{
    ...
    static_assert(Generator::min() == 0 &&
                  ...
                  "Generator must be able to output all numbers in its result "
                  "type (effectively it must be a random bit generator)");
...
{
    static_assert(sizeof(OriginalType) == sizeof(UnsignedType),
 ...
 */
template <typename IntegerType> class uniform_integer_distribution
{
    static_assert(std::is_integral<IntegerType>::value, "...");

    using UnsignedIntegerType =
        Detail::SizedUnsignedType_t<sizeof(IntegerType)>;
    ...
    // How many different values are there in [a, b]. a == b => 1, can be 0 for
    // distribution over all values in the type.
    ...
    static constexpr UnsignedIntegerType computeDistance(IntegerType a,
                                                         IntegerType b)
    {
    ...
    static constexpr UnsignedIntegerType computeRejectionThreshold(
...
public:
    ...
    template <typename Generator> constexpr result_type operator()(Generator& g)
    {
 ...
 * Assumes `a < b`.
 */
template <typename FloatType> FloatType gamma(FloatType a, FloatType b)
{
    ...
    const auto gamma_down =
        b - Catch::nextafter(b, -std::numeric_limits<FloatType>::infinity());

    return gamma_up < gamma_down ? gamma_down : gamma_up;
}

template <typename FloatingPoint> struct DistanceTypePicker;
template <> struct DistanceTypePicker<float>
{
    using type = std::uint32_t;
};
template <> struct DistanceTypePicker<double>
{
    using type = std::uint64_t;
};

template <typename T> using DistanceType = typename DistanceTypePicker<T>::type;
...
DistanceType<FloatType> count_equidistant_floats(FloatType a,
                                                 FloatType b,
                                                 FloatType distance)
{
    assert(a <= b);
    ...
    const auto s = bg - ag;
    const auto err =
        (std::fabs(a) <= std::fabs(b)) ? -ag - (s - bg) : bg - (s + ag);
 ...
 * Frederic Goualard.
 */
template <typename FloatType> class uniform_floating_point_distribution
{
    static_assert(std::is_floating_point<FloatType>::value, "...");
    static_assert(!std::is_same<FloatType, long double>::value,
                  "We do not support long double due to inconsistent behaviour "
                  "between platforms");
...
public:
    ...
    uniform_floating_point_distribution(FloatType a, FloatType b)
        ...
        , m_floats_in_range(
              Detail::count_equidistant_floats(m_a, m_b, m_ulp_magnitude))
        , m_int_dist(0, m_floats_in_range)
        , m_max_steps_in_one_go(
              Detail::calculate_max_steps_in_one_go(m_ulp_magnitude))
...
template <>
class RandomFloatingGenerator<long double> final
...
template <typename T>
std::enable_if_t<std::is_integral<T>::value, GeneratorWrapper<T>> random(T a,
                                                                         T b)
{
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<RandomIntegerGenerator<T>>(
            a, b, Detail::getSeed()));
}

template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, GeneratorWrapper<T>> random(
    T a, T b)
{
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<RandomFloatingGenerator<T>>(
            a, b, Detail::getSeed()));
}
...
GeneratorWrapper<T> range(T const& start, T const& end, T const& step)
{
    ...
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<RangeGenerator<T>>(start, end, step));
}
...
template <typename T> GeneratorWrapper<T> range(T const& start, T const& end)
{
    ...
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<RangeGenerator<T>>(start, end));
}
...
public:
    ...
    //! Called _every time_ a TEST_CASE is entered, including repeats (due to
    //! sections)
     ...
     * NOT called for test cases that are explicitly skipped using the `SKIP`
     * macro.
     *
    ...
    //! Writes out information about provided reporters using reporter-specific
    //! format
    virtual void listReporters(
        std::vector<ReporterDescription> const& descriptions) = 0;
    //! Writes out the provided listeners descriptions using reporter-specific
    //! format
    ...
    //! Writes out information about the provided tags using reporter-specific
    //! format
...
public:
    ...
    // TODO: this exists only for adding filenames to test cases -- let's expose
    // this in a saner way later
...
//! Wrapper over `std::getenv` that compiles on UWP (and always returns nullptr
//! there)
...
constexpr bool is_permutation(ForwardIter1    first_1,
                              ...
                              Comparator      cmp)
{
    // TODO: no optimization for stronger iterators, because we would also have
    // to constrain on sentinel vs not sentinel types
    ...
    // We need to handle case where at least one of the ranges has no more
    // elements
...
public:
    ...
    template <typename T> JsonArrayWriter& write(T const& value)
    {
...
private:
    template <typename T> JsonArrayWriter& writeImpl(T const& value)
    {
...
public:
    ...
    // TODO: Do we want to check that redirect is not active before retrieving
    // the output?
...
public:
    ...
    std::vector<Detail::unique_ptr<EventListenerFactory>> const& getListeners()
        const;
};
...
struct NameAndLocationRef
{
    ...
    constexpr NameAndLocationRef(StringRef name_, SourceLineInfo location_)
        : name(name_)
        , location(location_)
    {
    }
...
public:
    TrackerBase(NameAndLocation&& nameAndLocation,
                TrackerContext&   ctx,
                ITracker*         parent);
...
class SectionTracker : public TrackerBase
{
    ...
    // Note that lifetime-wise we piggy back off the name stored in the
    // `ITracker` parent`. Currently it allocates owns the name, so this is
    // safe. If it is later refactored to not own the name, the name still has
    // to outlive the `ITracker` parent, so this should still be safe.
    StringRef m_trimmed_name;

public:
    SectionTracker(NameAndLocation&& nameAndLocation,
                   TrackerContext&   ctx,
                   ITracker*         parent);
...
public: // IResultCapture
    ...
    IGeneratorTracker* acquireGeneratorTracker(
        StringRef generatorName, SourceLineInfo const& lineInfo) override;
...
template <typename Container>
Container createShard(Container const&  container,
                      ...
                      std::size_t const shardIndex)
{
    ...
    const std::size_t endIndex = (shardIndex + 1) * shardSize +
                                 (std::min)(shardIndex + 1, leftoverTests);

    auto startIterator =
        std::next(container.begin(), static_cast<std::ptrdiff_t>(startIndex));
    auto endIterator =
        std::next(container.begin(), static_cast<std::ptrdiff_t>(endIndex));
...
public:
    ...
    std::vector<TestCaseHandle> const& getAllTestsSorted(
...
public:
    ...
    public:
        ...
        ScopedElement& writeText(StringRef     text,
...
template <typename ArgT, typename MatcherT>
constexpr MatchExpr<ArgT, MatcherT> makeMatchExpr(ArgT&&          arg,
...
#define INTERNAL_CHECK_THAT(macroName, matcher, resultDisposition, arg)        \
    ...
    {                                                                          \
        ...
        {                                                                      \
            catchAssertionHandler.handleExpr(                                  \
                Catch::makeMatchExpr(arg, matcher));                           \
        }                                                                      \
        INTERNAL_CATCH_CATCH(catchAssertionHandler)                            \
        catchAssertionHandler.complete();                                      \
    } while (false)
...
template <typename T> class MatcherBase : public MatcherUntypedBase
{
public:
...
template <typename ArgT> class MatchAllOf final : public MatcherBase<ArgT>
{
...
MatchAllOf<ArgT> operator&&(MatchAllOf<ArgT> const&  lhs,
                            MatcherBase<ArgT> const& rhs) = delete;
//! lvalue overload is intentionally deleted, users should
//! not be trying to compose stored composition matchers
template <typename ArgT>
MatchAllOf<ArgT> operator&&(MatcherBase<ArgT> const& lhs,
                            MatchAllOf<ArgT> const&  rhs) = delete;

template <typename ArgT> class MatchAnyOf final : public MatcherBase<ArgT>
{
    std::vector<MatcherBase<ArgT> const*> m_matchers;
...
MatchAnyOf<ArgT> operator||(MatcherBase<ArgT> const& lhs,
                            MatchAnyOf<ArgT> const&  rhs) = delete;

template <typename ArgT> class MatchNotOf final : public MatcherBase<ArgT>
{
...
template <typename T>
Detail::MatchAllOf<T> operator&&(MatcherBase<T> const& lhs,
                                 MatcherBase<T> const& rhs)
{
    return Detail::MatchAllOf<T>{} && lhs && rhs;
}
template <typename T>
Detail::MatchAnyOf<T> operator||(MatcherBase<T> const& lhs,
                                 MatcherBase<T> const& rhs)
{
...
std::array<void const*, N + M> array_cat(std::array<void const*, N>&& lhs,
...
std::array<void const*, N + 1> array_cat(std::array<void const*, N>&& lhs,
                                         void const*                  rhs)
{
...
template <std::size_t N>
std::array<void const*, N + 1> array_cat(void const*                  lhs,
                                         std::array<void const*, N>&& rhs)
{
...
template <typename T>
using is_generic_matcher =
    std::is_base_of<Catch::Matchers::MatcherGenericBase,
                    std::remove_cv_t<std::remove_reference_t<T>>>;

template <typename... Ts>
using are_generic_matchers =
    Catch::Detail::conjunction<is_generic_matcher<Ts>...>;

template <typename T>
using is_matcher =
    std::is_base_of<Catch::Matchers::MatcherUntypedBase,
                    std::remove_cv_t<std::remove_reference_t<T>>>;

template <std::size_t N, typename Arg>
bool match_all_of(Arg&&,
...
template <typename T,
          ...
          std::size_t Idx,
          std::size_t... Indices>
bool match_any_of(Arg&&                             arg,
                  std::array<void const*, N> const& matchers,
                  std::index_sequence<Idx, Indices...>)
{
    return static_cast<T const*>(matchers[Idx])->match(arg) ||
           match_any_of<MatcherTs...>(
               arg, matchers, std::index_sequence<Indices...>{});
}
...
std::string describe_multi_matcher(
    StringRef                                            combine,
    std::array<void const*, sizeof...(MatcherTs)> const& matchers,
    std::index_sequence<Idx...>)
{
    ...
    return describe_multi_matcher(combine,
                                  descriptions.data(),
...
public:
    ...
    explicit MatchAllOfGeneric(
        std::array<void const*, sizeof...(MatcherTs)> matchers)
        : m_matchers{ matchers }
    {
    }

    template <typename Arg> bool match(Arg&& arg) const
    {
        return match_all_of<MatcherTs...>(
            arg, m_matchers, std::index_sequence_for<MatcherTs...>{});
    }

    std::string describe() const override
    {
        return describe_multi_matcher<MatcherTs...>(
    ...
    {
        return MatchAllOfGeneric<MatcherTs..., MatchersRHS...>{ array_cat(
            CATCH_MOVE(lhs.m_matchers), CATCH_MOVE(rhs.m_matchers)) };
    }
    ...
    operator&&(MatchAllOfGeneric<MatcherTs...>&& lhs, MatcherRHS const& rhs)
    {
        return MatchAllOfGeneric<MatcherTs..., MatcherRHS>{ array_cat(
    ...
    operator&&(MatcherLHS const& lhs, MatchAllOfGeneric<MatcherTs...>&& rhs)
    {
        return MatchAllOfGeneric<MatcherLHS, MatcherTs...>{ array_cat(
            static_cast<void const*>(std::addressof(lhs)),
...
public:
    ...
    {
        return MatchAnyOfGeneric<MatcherTs..., MatchersRHS...>{ array_cat(
            CATCH_MOVE(lhs.m_matchers), CATCH_MOVE(rhs.m_matchers)) };
    }
    ...
    operator||(MatchAnyOfGeneric<MatcherTs...>&& lhs, MatcherRHS const& rhs)
    {
        return MatchAnyOfGeneric<MatcherTs..., MatcherRHS>{ array_cat(
            CATCH_MOVE(lhs.m_matchers),
            static_cast<void const*>(std::addressof(rhs))) };
    }
    ...
    operator||(MatcherLHS const& lhs, MatchAnyOfGeneric<MatcherTs...>&& rhs)
    {
        return MatchAnyOfGeneric<MatcherLHS, MatcherTs...>{ array_cat(
            static_cast<void const*>(std::addressof(lhs)),
            CATCH_MOVE(rhs.m_matchers)) };
    }
};
...
public:
    ...
    explicit MatchNotOfGeneric(MatcherT const& matcher)
        : m_matcher{ matcher }
    {
    }
...
std::enable_if_t<Detail::are_generic_matchers<MatcherLHS, MatcherRHS>::value,
...
template <typename MatcherT>
std::enable_if_t<Detail::is_generic_matcher<MatcherT>::value,
...
std::enable_if_t<Detail::is_generic_matcher<MatcherLHS>::value,
...
std::enable_if_t<Detail::is_generic_matcher<MatcherRHS>::value,
...
//! Meta-matcher for checking that an element in a range matches a specific
//! matcher
template <typename Matcher>
...
//! Creates a matcher that checks whether a range contains element matching a
//! matcher
template <typename Matcher>
std::enable_if_t<Detail::is_matcher<Matcher>::value,
...
//! Creates a matcher that checks whether a std derived exception has the
//! provided message
...
template <typename StringMatcherType>
ExceptionMessageMatchesMatcher<StringMatcherType> MessageMatches(
    StringMatcherType&& matcher)
{
    return { CATCH_FORWARD(matcher) };
}
...
//! Creates a matcher that accepts doubles within certain relative range of
//! target
WithinRelMatcher WithinRel(double target, double eps);
//! Creates a matcher that accepts doubles within 100*DBL_EPS relative range of
//! target
WithinRelMatcher WithinRel(double target);
//! Creates a matcher that accepts doubles within certain relative range of
//! target
WithinRelMatcher WithinRel(float target, float eps);
//! Creates a matcher that accepts floats within 100*FLT_EPS relative range of
//! target
...
PredicateMatcher<T, Pred> Predicate(Pred&&             predicate,
                                    std::string const& description = "")
{
    static_assert(is_callable<Pred(T)>::value,
                  "Predicate not callable with argument T");
    static_assert(std::is_same<bool, FunctionReturnType<Pred, T>>::value,
...
// Matcher for checking that at least one element in range matches a given
// matcher.
template <typename Matcher>
...
template <typename Matcher> AllMatchMatcher<Matcher> AllMatch(Matcher&& matcher)
{
    return { CATCH_FORWARD(matcher) };
}

// Creates a matcher that checks whether no element in a range matches a
// matcher.
template <typename Matcher>
...
// Creates a matcher that checks whether any element in a range matches a
// matcher.
template <typename Matcher> AnyMatchMatcher<Matcher> AnyMatch(Matcher&& matcher)
{
    return { CATCH_FORWARD(matcher) };
}
...
public:
    ...
    template <typename RangeLike> constexpr bool match(RangeLike&& rng) const
    {
...
constexpr std::enable_if_t<!Detail::is_matcher<RangeLike>::value,
...
constexpr RangeEqualsMatcher<RangeLike, Equality> RangeEquals(
    RangeLike&& range, Equality&& predicate)
{
...
StringContainsMatcher ContainsSubstring(
    std::string const& str, CaseSensitive caseSensitivity = CaseSensitive::Yes);
...
RegexMatcher Matches(std::string const& regex,
                     CaseSensitive      caseSensitivity = CaseSensitive::Yes);
...
class VectorContainsElementMatcher final
...
template <typename T, typename AllocComp, typename AllocMatch>
class UnorderedEqualsMatcher final
...
//! Creates a matcher that matches vectors that contain all elements in
//! `comparator`
template <typename T,
          typename AllocComp  = std::allocator<T>,
          typename AllocMatch = AllocComp>
ContainsMatcher<T, AllocComp, AllocMatch> Contains(
    std::vector<T, AllocComp> const& comparator)
{
    return ContainsMatcher<T, AllocComp, AllocMatch>(comparator);
}

//! Creates a matcher that matches vectors that contain `comparator` as an
//! element
...
//! Creates a matcher that matches vectors that are exactly equal to
//! `comparator`
template <typename T,
          typename AllocComp  = std::allocator<T>,
          typename AllocMatch = AllocComp>
EqualsMatcher<T, AllocComp, AllocMatch> Equals(
    std::vector<T, AllocComp> const& comparator)
{
...
template <typename T,
          typename AllocComp  = std::allocator<T>,
          typename AllocMatch = AllocComp>
ApproxMatcher<T, AllocComp, AllocMatch> Approx(
    std::vector<T, AllocComp> const& comparator)
{
    return ApproxMatcher<T, AllocComp, AllocMatch>(comparator);
}
//! Creates a matcher that matches vectors that is equal to `target` modulo
//! permutation
template <typename T,
          typename AllocComp  = std::allocator<T>,
          typename AllocMatch = AllocComp>
UnorderedEqualsMatcher<T, AllocComp, AllocMatch> UnorderedEquals(
...
//! Represents either an assertion or a benchmark result to be handled by
//! cumulative reporter later
...
public:
    ...
    //! Customization point: called after last test finishes (testRunEnded has
    //! been handled)
...
protected:
    //! Should the cumulative base store the assertion expansion for successful
    //! assertions?
    bool m_shouldStoreSuccesfulAssertions = true;
    //! Should the cumulative base store the assertion expansion for failed
    //! assertions?
...
public: // IEventListener
    ...
    void assertionEnded(AssertionStats const& assertionStats) override;
    void sectionEnded(SectionStats const& sectionStats) override;
    void testCasePartialEnded(TestCaseStats const& testStats,
                              uint64_t             partNumber) override;
    void testCaseEnded(TestCaseStats const& testCaseStats) override;
    void testRunEnded(TestRunStats const& testRunStats) override;
...
template <typename T, typename = void> struct has_description : std::false_type
{
};
