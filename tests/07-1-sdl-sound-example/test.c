...
int main(int argc, char* argv[])
{
    ...
    playMusic("highlands.wav", SDL_MIX_MAXVOLUME);
    ...
    playMusic("road.wav", SDL_MIX_MAXVOLUME);
    SDL_Delay(1000);
    ...
    playSound("door2.wav", SDL_MIX_MAXVOLUME / 2);
    SDL_Delay(2000);

    /* Caching sound example, create, play from Memory, clear */

    Audio * sound = createAudio("door1.wav", 0, SDL_MIX_MAXVOLUME / 2);
    playSoundFromMemory(sound, SDL_MIX_MAXVOLUME);
    SDL_Delay(2000);

    Audio * music = createAudio("highlands.wav", 1, SDL_MIX_MAXVOLUME);
