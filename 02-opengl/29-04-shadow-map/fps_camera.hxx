...
inline void fps_camera::rotate(const float delta_yaw, const float delta_pitch)
{
    ...
    pitch = pitch = std::clamp(pitch, -89.f, 89.f);
...
inline void fps_camera::zoom(const float zoom)
{
    ...
    fovy_ = fovy_ = std::clamp(fovy_, 1.f, 45.f);
}
...
{
    aspect_ = v;
    aspect_ = aspect_ = std::clamp(aspect_, 0.1f, 10.f);
}
...
{
    z_near_ = v;
    z_near_ = z_near_ = std::clamp(z_near_, 0.01f, 10000.f);
}
...
{
    z_far_ = v;
    z_far_ = z_far_ = std::clamp(z_far_, z_near_, 10000.f);
}
