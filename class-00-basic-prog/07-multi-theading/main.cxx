...
int main([[maybe_unused]] int argc, [[maybe_unused]] char *argv[])
{
    ...
    auto print_chars_unsafe = [&out_file](const string_view &str, size_t num_of_iter)
        {
    ...
    std::transform(begin(print_patterns), end(print_patterns), begin(threads),
    ...
    const size_t expected_size_of_file =
        std::accumulate(begin(print_patterns), end(print_patterns), size_t{ 0 },
                [](size_t value, const string_view &right)
                {
                    const size_t num_of_chars_for_pattern = right.size() * num_of_iterations;
    ...
    {
        cout << "file_size(" << file_size << ") == expected_size_of_file(" << expected_size_of_file << ")";
    }
    else
    {
        cout << "file_size(" << file_size << ") != expected_size_of_file(" << expected_size_of_file << ")";
