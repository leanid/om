...
struct scene
{
    ...
    gles30::shader refraction_shader;
    ...
    gles30::mesh cube_sky_refraction;
...
scene::scene()
    ...
    , refraction_shader("res/refraction.vsh", "res/refraction.fsh")
    ...
    , cube_metal{ create_mesh(cube_vertices, sizeof(cube_vertices) / 4 / 8,
                              { &tex_metal }) }
    , cube_skybox{ create_mesh(cube_vertices, sizeof(cube_vertices) / 4 / 8,
                               { &tex_cubemap }) }
    , cube_sky_refraction{ create_mesh(
...
void scene::render(float delta_time)
{
    ...
    refraction_shader.set_uniform("camera_pos", camera.position());
    render_mesh(refraction_shader, camera, cube_sky_refraction,
                glm::vec3(0.f, 1.f, 0.f), scale, properties,
                render_options::only_pro_view_model);

    glDepthFunc(GL_LEQUAL);
    render_mesh(skybox_shader, camera, cube_skybox, camera.position(), 1.f,
                properties, render_options::only_pro_view);
    glDepthFunc(GL_LESS);
}
